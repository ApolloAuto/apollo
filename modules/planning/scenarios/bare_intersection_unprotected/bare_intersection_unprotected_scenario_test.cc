...
TEST_F(BareIntersectionUnprotectedScenarioTest, Init) {
  ...
  scenario_.reset(new BareIntersectionUnprotectedScenario());
  scenario_->Init(injector, "BARE_INTERSECTION_UNPROTECTED");
  EXPECT_EQ(scenario_->Name(), "BARE_INTERSECTION_UNPROTECTED");
}
