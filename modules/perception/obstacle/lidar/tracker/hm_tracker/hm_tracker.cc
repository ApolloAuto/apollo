...
bool HmObjectTracker::Init() {
    ...
    if (matcher_method_ == "hungarian_matcher") {
        if (!HungarianMatcher::set_max_match_distance(
                    max_match_dist)) {
            return false;
        }
    }
    ...
    if (!TrackObjectDistance::set_weight_location_dist(weight_location_dist)) {
        return false;
    }
    ...
    if (!TrackObjectDistance::set_weight_direction_dist(
                weight_direction_dist)) {
        return false;
    }
    ...
    if (!TrackObjectDistance::set_weight_bbox_size_dist(
                weight_bbox_size_dist)) {
        return false;
    }
    ...
    if (!TrackObjectDistance::set_weight_point_num_dist(
                weight_point_num_dist)) {
        return false;
    }
    ...
    if (!TrackObjectDistance::set_weight_histogram_dist(
                weight_histogram_dist)) {
        return false;
    }
    ...
    ObjectTrack::set_filter_method(filter_method_);
    ...
    if (filter_method_ == "kalman_filter") {
        ...
        if (!model_config->GetValue("centroid_initial_velocity_variance",
            ...
            AERROR << "centroid_initial_velocity_variance not found."
                   << name();
            return false;
        }
...
bool HmObjectTracker::Track(const std::vector<ObjectPtr>& objects,
    ...
    Eigen::Vector3f cur_location(velo2world_pose(0, 3),
            velo2world_pose(1, 3), velo2world_pose(2, 3));
    _ref_translation = cur_location - _ref_location;
    _ref_location = cur_location;
    _ref_orientation = (velo2world_pose *
            Eigen::Vector4d(1, 0, 0, 0)).head(3).cast<float>();
    Eigen::Vector3f ref_velocity = _ref_translation / time_diff;
...
bool HmObjectTracker::Initialize(const std::vector<ObjectPtr>& objects,
    ...
    _ref_location = Eigen::Vector3f(velo2world_pose(0, 3),
            velo2world_pose(1, 3), velo2world_pose(2, 3));
    _ref_translation = Eigen::Vector3f(0, 0, 0);
    Eigen::Vector4d dir = velo2world_pose * Eigen::Vector4d(1, 0, 0, 0);
    _ref_orientation = Eigen::Vector3f(dir[0], dir[1], dir[2]);
...
void HmObjectTracker::DecomposeForegroundBackgroundObjects(
    ...
    for (size_t i = 0; i < objects.size(); i++) {
        if (objects[i]->object_ptr->is_background) {
            objects[i]->object_ptr->track_id =
                ObjectTrack::get_next_track_id();
...
void HmObjectTracker::UpdateAssignedTracks(
    ...
    for (int i = 0; i < assignments.size(); i++) {
        ...
        tracks[track_id]->update_with_object(new_objects[obj_id], time_diff);
    }
}
...
void HmObjectTracker::UpdateUnassignedTracks(
    ...
    for (int i = 0; i < unassigned_tracks.size(); i++) {
        ...
        tracks[track_id]->update_without_object(
...
void HmObjectTracker::CollectTrackedResults(
    ...
    for (size_t i = 0; i < tracks.size(); i++) {
        if (tracks[i]->_consecutive_invisible_count > 0) {
            continue;
        }
        ObjectPtr obj(new Object);
        TrackedObjectPtr result_obj = tracks[i]->_current_object;
        ...
        obj->track_id = tracks[i]->_idx;
        obj->tracking_time = tracks[i]->_period;
        obj->type = result_obj->type;
        obj->center = result_obj->center.cast<double>() -
...
void HmObjectTracker::ComputeTrackIdsForRecentObjects(
    ...
    for (size_t i = 0; i < tracks.size(); i++) {
        if (tracks[i]->_consecutive_invisible_count == 0) {
            int obj_id = tracks[i]->_current_object->object_ptr->id;
            recent_obj_2_track[obj_id] = tracks[i]->_idx;
        }
    }

    for (size_t i = 0; i < background_objects_.size(); i++) {
        int obj_id = background_objects_[i]->object_ptr->id;
        recent_obj_2_track[obj_id] =
            background_objects_[i]->object_ptr->track_id;
    }
