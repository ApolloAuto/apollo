...
void ClassifyBySimple::Init(const ClassifyParam& model_config, const int gpu_id) {
    AINFO << "Enter Classify init";
    net_inputs_.clear();
    net_outputs_.clear();
    AINFO << "clear success";
    net_inputs_ = inference::GetBlobNames(model_config.info().inputs());
    net_outputs_ = inference::GetBlobNames(model_config.info().outputs());
    ...
    std::string proto_file = GetModelFile(model_path, model_config.info().proto_file().file());
    AINFO << "proto_file " << proto_file;

    std::string weight_file = GetModelFile(model_path, model_config.info().weight_file().file());
    ...
    AINFO << "input_reshape: " << input_reshape[net_inputs_[0]][0] << ", " << input_reshape[net_inputs_[0]][1] << ", "
          << input_reshape[net_inputs_[0]][2] << ", " << input_reshape[net_inputs_[0]][3];
...
void ClassifyBySimple::Perform(const camera::TrafficLightFrame* frame, std::vector<base::TrafficLightPtr>* lights) {
...
void ClassifyBySimple::Prob2Color(const float* out_put_data, float threshold, base::TrafficLightPtr light) {
    int max_color_id = 0;
