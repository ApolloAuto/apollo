...
class ZhongyunMessageManagerTest : public ::testing::Test {
 public:
  ZhongyunMessageManagerTest() : manager_() {}
  virtual void SetUp() {}
...
TEST_F(ZhongyunMessageManagerTest, GetSendProtocols) {
  EXPECT_TRUE(manager_.GetMutableProtocolDataById(Brakecontrola4::ID) !=
              nullptr);
  EXPECT_TRUE(manager_.GetMutableProtocolDataById(Gearcontrola1::ID) !=
              nullptr);
  EXPECT_TRUE(manager_.GetMutableProtocolDataById(Parkingcontrola5::ID) !=
              nullptr);
  EXPECT_TRUE(manager_.GetMutableProtocolDataById(Steeringcontrola2::ID) !=
              nullptr);
  EXPECT_TRUE(manager_.GetMutableProtocolDataById(Torquecontrola3::ID) !=
              nullptr);
}
