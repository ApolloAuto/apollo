...
float HungarianMatcher::_s_max_match_distance = 4.0f;
...
bool HungarianMatcher::set_max_match_distance(
        float max_match_distance) {
    if (max_match_distance >= 0) {
        _s_max_match_distance = max_match_distance;
        return true;
    } else {
    ...
    for (int i = 0; i < track_components.size(); i++) {
        if (track_components[i].empty() || obj_components[i].empty()) {
            ...
            if (association_mat(track_components[i][0], obj_components[i][0])
                ...
                float association_score = association_mat(
                        track_components[i][0], obj_components[i][0]);
        ...
        assign_objects_to_tracks_using_null_tracks(mat_loc,
                max_dist, assignments_loc,
...
void HungarianMatcher::compute_associate_matrix(
        const std::vector<ObjectTrackPtr>& tracks,
        const std::vector<Eigen::VectorXf>& tracks_predict,
    ...
    for (size_t i = 0; i < tracks.size(); ++i) {
        for (size_t j = 0; j < new_objects.size(); ++j) {
            association_mat(i, j) = compute_track_object_distance(
                    tracks[i], tracks_predict[i], new_objects[j], time_diff);
        }
    }
}

float HungarianMatcher::compute_track_object_distance(
    ...
    return TrackObjectDistance::compute_distance(track, track_predict,
            new_object, time_diff);
}

void HungarianMatcher::compute_connected_components(
...
void HungarianMatcher::assign_objects_to_tracks(
        const Eigen::MatrixXf& association_mat,
...
void HungarianMatcher::assign_objects_to_tracks_using_null_tracks(
        const Eigen::MatrixXf& association_mat,
        const double max_distance,
        std::vector<TrackObjectPair>& assignments,
        std::vector<int>& unassigned_tracks,
