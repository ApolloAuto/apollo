...
#ifndef MODULES_DRIVERS_LIDAR_VELODYNE_POINTCLOUD_RAWDATA_H_
#define MODULES_DRIVERS_LIDAR_VELODYNE_POINTCLOUD_RAWDATA_H_
...
static const float CHANNEL_TDURATION =
...
static const float VLP16_BLOCK_TDURATION =
    (VLP16_NUM_SEQS_PER_BLOCK * SEQ_TDURATION);

/** Special Definitions for HDL32 support **/
static const float HDL32_CHANNEL_TDURATION =
    1.152f;  // [µs] From Application Note: HDL-32E Packet Structure and Timing
             // Defition
...
static const int VLS128_BLOCK_DATA_SIZE =
    VLS128_NUM_CHANS_PER_BLOCK * CHANNEL_SIZE;
...
static const int NUM_CHANS_PER_PACKET =
    (NUM_CHANS_PER_BLOCK * NUM_BLOCKS_PER_PACKET);
...
class RawData {
 ...
 private:
  ...
  Config config_;
  float previous_block_corrected_azimuth;    // for support of different scan
                                             // patterns
  float previous_block_corrected_elevation;  // record the scan profile endpoint
                                             // in previous packet

  /**
   * Calibration file
   */
  apollo::drivers::lidar_velodyne::Calibration calibration_;
...
POINT_CLOUD_REGISTER_POINT_STRUCT(apollo::drivers::lidar_velodyne::PointXYZIRB,
                                  (float, x, x)(float, y, y)(float, z, z)(
                                      float, intensity,
                                      intensity)(uint16_t, ring,
                                                 ring)(uint16_t, block, block))

#endif  // MODULES_DRIVERS_LIDAR_VELODYNE_POINTCLOUD_RAWDATA_H_
