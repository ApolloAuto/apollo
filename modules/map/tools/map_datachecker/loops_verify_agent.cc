...
LoopsVerifyAgent::LoopsVerifyAgent(std::shared_ptr<JSonConf> sp_conf, std::shared_ptr<PoseCollectionAgent> sp_pose_collection_agent) {
    _sp_conf = sp_conf;
    _sp_pose_collection_agent = sp_pose_collection_agent;
    //_have_result = false;
}

grpc::Status LoopsVerifyAgent::process_grpc_request(grpc::ServerContext *context, LOOPS_VERIFY_REQUEST_TYPE *request, LOOPS_VERIFY_RESPONSE_TYPE *response) {
...
void LoopsVerifyAgent::StartVerify(LOOPS_VERIFY_REQUEST_TYPE *request, LOOPS_VERIFY_RESPONSE_TYPE *response)
{
    ...
    double loops_to_check = (double)get_loops_to_check(request);
...
void LoopsVerifyAgent::CheckVerify(LOOPS_VERIFY_REQUEST_TYPE *request, LOOPS_VERIFY_RESPONSE_TYPE *response)
{
    AINFO << "call CheckVerify";
    if( get_state() == LoopsVerifyAgentState::IDLE/* && _have_result == false */) {
    ...
    if ( _sp_laps_checker == nullptr ) {
        ...
        response->set_progress(0.0);
        response->set_code(ErrorCode::SUCCESS); // TODO enrich error code
        return;
    }
    ...
    if ( std::abs(1.0 - progress) < 1e-8 ) {
        ...
        adu::workers::collection::LoopResult *loop_result = response->mutable_loop_result();

        loop_result->set_loop_num((double)lap);
        ...
        if ( data_type == DataType::MAP_CHECKOUT ) {
            if ( is_reached ) {
                loop_result->set_loop_num((double)_sp_conf->laps_number_additional);
            } else {
                loop_result->set_loop_num(lap - _sp_conf->laps_number >= 0 ? double(lap - _sp_conf->laps_number) : 0.0);
            }
        }
    }
}

void LoopsVerifyAgent::StopVerify(LOOPS_VERIFY_REQUEST_TYPE *request, LOOPS_VERIFY_RESPONSE_TYPE *response) {
...
std::shared_ptr<std::vector<std::pair<double, double>>> LoopsVerifyAgent::get_verify_range(LOOPS_VERIFY_REQUEST_TYPE *request) {
    std::shared_ptr<std::vector<std::pair<double, double>>> sp_range(new std::vector<std::pair<double, double>>());
    int range_size = request->range_size();
    for ( int i = 0; i < range_size; i++ ) {
        const ::adu::workers::collection::VerifyRange& range = request->range(i);
        sp_range->push_back( std::make_pair(range.start_time(), range.end_time()) );
    }
    return sp_range;
}

size_t LoopsVerifyAgent::get_loops_to_check(LOOPS_VERIFY_REQUEST_TYPE *request) {
...
double LoopsVerifyAgent::get_range_index(std::shared_ptr<std::vector<std::pair<double, double>>> sp_range, std::vector<bool>& range_index, std::shared_ptr<std::vector<FramePose>> sp_vec_poses) {
    if ( sp_range == nullptr ) {
    ...
    for( size_t i = 0; i < size; i++ ) {
        if ( range[i].first >= range[i].second ) {
            AINFO << "range error, [" << std::to_string(range[i].first) << "," << std::to_string(range[i].second) << "]";
            fprintf(stderr, "range error, [%lf,%lf]\n", range[i].first, range[i].second);
            continue; // TODO add error code?
        }
    ...
    AINFO << "[get_range_index] min_time:" << std::to_string(min_time) << ", max_time" << std::to_string(max_time);
    ...
    if (size == 0 || max_time <= 0) {
        ...
        if (sp_vec_poses->size() > 0) {
            ...
            int index_size = int(max_time - min_time + 1);
        ...
        for( int i = 0; i < (int)size; i++ ) {
            int start_time = int(range[i].first - min_time);
            int end_time = int(range[i].second - min_time);
...
int LoopsVerifyAgent::get_poses_to_check(std::shared_ptr<std::vector<std::pair<double, double>>> sp_range, std::vector<FramePose>& poses) {
    if ( _sp_pose_collection_agent == nullptr ) {
        AINFO << "error, sp_pose_collection_agent is null";
        return -1;
    }
    std::shared_ptr<std::vector<FramePose>> sp_vec_poses = _sp_pose_collection_agent->get_poses();
    ...
    if ( min_time == std::numeric_limits<double>::max() || range_index.size() == 0 ) {
    ...
    for( size_t i = 0; i < pose_size; i++ ) {
        int time = int(vec_poses[i].time_stamp - min_time);
        if ( time >= (int)range_size ) {
            break;
        }
...
int LoopsVerifyAgent::do_start_verify(std::shared_ptr<std::vector<std::pair<double, double>>> sp_range, double loops_to_check) {

    clock_t start = clock();
//    set_state(LoopsVerifyAgentState::RUNNING);
    //_have_result = false;
    std::vector<FramePose> all_poses;
    get_poses_to_check(sp_range, all_poses); // 2

    _sp_laps_checker = std::shared_ptr<LapsChecker>(new LapsChecker(all_poses, (int)loops_to_check, _sp_conf));
    _sp_laps_checker->check(); // 3

    //_have_result = true;
    //set_state(LoopsVerifyAgentState::IDLE);
    double duration = ( double(clock() - start) + 0.0 ) / CLOCKS_PER_SEC;
