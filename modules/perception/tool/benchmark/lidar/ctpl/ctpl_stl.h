...
class Queue {
 public:
  ...
  bool pop(T &v) {  // NOLINT
    std::unique_lock<std::mutex> lock(this->mutex);
    if (this->q.empty()) { return false; }    v = this->q.front();
    this->q.pop();
    return true;
  }
  bool empty() {
...
class thread_pool {
 public:
  ...
  void stop(bool isWait = false) {
    if (!isWait) {
      if (this->isStop) { return; }      this->isStop = true;
      for (int i = 0, n = this->size(); i < n; ++i) {
        *this->flags[i] = true;  // command the threads to stop
      }
      this->clear_queue();  // empty the queue
    } else {
      if (this->isDone || this->isStop) { return; }      this->isDone = true;  // give the waiting threads a command to finish
    }
    {
      std::unique_lock<std::mutex> lock(this->mutex);
