...
bool OpenSpaceRoiUtil::FormulateBoundaryConstraints(
  ...
  auto &obstacles_edges_num_vec =
      *open_space_info->mutable_obstacles_edges_num();
...
void OpenSpaceRoiUtil::GetRoiXYBoundary(
  ...
  *XYBoundary = {std::numeric_limits<double>::max(),
...
void OpenSpaceRoiUtil::TransformByOriginPoint(
    const common::math::Vec2d &origin_point, const double heading,
...
bool OpenSpaceRoiUtil::LoadObstacles(
    const double filtering_distance, const double perception_obstacle_buffer,
    const std::vector<double> &xy_boundary, Frame *const frame,
...
bool OpenSpaceRoiUtil::FilterOutObstacle(const double filtering_distance,
                                         const Frame &frame,
  ...
  if (obstacle_center_xy.x() < xy_boundary[0] ||
      obstacle_center_xy.x() > xy_boundary[1] ||
      obstacle_center_xy.y() < xy_boundary[2] ||
      obstacle_center_xy.y() > xy_boundary[3]) {
    return true;
  }
...
bool OpenSpaceRoiUtil::LoadObstacleInHyperPlanes(
    OpenSpaceInfo *const open_space_info) {
  *(open_space_info->mutable_obstacles_A()) =
      Eigen::MatrixXd::Zero(open_space_info->obstacles_edges_num().sum(), 2);
  *(open_space_info->mutable_obstacles_b()) =
      Eigen::MatrixXd::Zero(open_space_info->obstacles_edges_num().sum(), 1);
  // vertices using H-representation
  if (!GetHyperPlanes(open_space_info->obstacles_num(),
