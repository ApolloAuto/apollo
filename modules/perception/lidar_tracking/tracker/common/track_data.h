...
class TrackData {
 public:
  ...
  /**
   * @brief Get histroy object
   *
   * @param idx when idx > 0, get object from oldest
   *            when idx <= 0,  get object from latest
   *            when abs_idx is large than history size,
   *            return farest object close to idx
   * @return std::pair<double, TrackedObjectPtr>
   */
