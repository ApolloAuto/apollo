...
class PredictorManagerFuzz {
 public:
  void SetUp() {
    std::string file =
        "modules/prediction/testdata/single_perception_vehicle_onlane_fuzz.pb.txt";
...
void PredictorManagerFuzz::target() {
...
DEFINE_PROTO_FUZZER(const apollo::perception::PerceptionObstacles& message) {
  apollo::common::util::SetProtoToASCIIFile(message,
    "modules/prediction/testdata/single_perception_vehicle_onlane_fuzz.pb.txt");
  apollo::prediction::predictor_manager_fuzzer.SetUp();
  apollo::prediction::predictor_manager_fuzzer.target();
}
