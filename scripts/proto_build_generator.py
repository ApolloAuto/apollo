...
##==========================================================================##
# Check if the workdir is in repo apollo
def _in_apollo(build_file_path):
    return r"apollo" in os.path.abspath(build_file_path).split("/")
...
def main(build_file_path):
    repo_prefix = ""
    if not _in_apollo(build_file_path):
        repo_prefix += "@apollo"
    ...
    if grpc_found:
        fout.write(TEMPLATE_HEADER.format(repo_prefix, HEADER_PY_GRPC, HEADER_CC_GRPC))
    else:
        fout.write(TEMPLATE_HEADER.format(repo_prefix, "", ""))
