...
class DecoderBP : public DecoderBase<vpoint> {
 public:
  DecoderBP(const RS_Param &param);
...
double DecoderBP<vpoint>::getLidarTime(const uint8_t *pkt) {
  ...
  return std::mktime(&stm) +
         (double)RS_SWAP_SHORT(mpkt_ptr->header.timestamp.ms) / 1000.0 +
         (double)RS_SWAP_SHORT(mpkt_ptr->header.timestamp.us) / 1000000.0;
}

template <typename vpoint>
int DecoderBP<vpoint>::decodeMsopPkt(const uint8_t *pkt,
                                     std::vector<vpoint> &vec, int &height) {
  height = 32;
  RSBP_MsopPkt *mpkt_ptr = (RSBP_MsopPkt *)pkt;
  double pkt_time = 0;
  ...
  for (int blk_idx = 0; blk_idx < RSBP_BLOCKS_PER_PKT; blk_idx++) {
    ...
    for (int channel_idx = 0; channel_idx < RSBP_CHANNELS_PER_BLOCK;
      ...
      angle_vert =
          (((int)(this->vert_angle_list_[channel_idx] * 100) % 36000) + 36000) %
          36000;
...
int32_t DecoderBP<vpoint>::decodeDifopPkt(const uint8_t *pkt) {
  RSBP_DifopPkt *rs32_ptr = (RSBP_DifopPkt *)pkt;
  ...
  if (!(this->cali_data_flag_ & 0x2)) {
    ...
    p_pitch_cali = ((RSBP_DifopPkt *)pkt)->pitch_cali;
    ...
    if (angle_flag) {
      ...
      const uint8_t *p_yaw_cali = ((RSBP_DifopPkt *)pkt)->yaw_cali;
      for (int i = 0; i < 32; i++) {
