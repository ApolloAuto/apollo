...
bool Routing::FillParkingID(RoutingResponse* routing_response) {
  ...
  if (!has_parking_info) {
    ADEBUG << "routing has no parking info";
    return false;
  }
  ...
  if (has_parking_id) {
    ADEBUG << "routing already has parking ID";
    return false;
  }

  const bool has_parking_point =
      has_parking_info && routing_request.parking_info().has_parking_point();
  // not need to extract parking ID when parking_point is not available
  if (!has_parking_point) {
    ADEBUG << "routing has no parking point info";
    return false;
  }
