...
void Mediacontrolscmd120::Parse(const std::uint8_t* bytes, int32_t length,
  ...
  chassis->mutable_lexus()->mutable_media_controls_cmd_120()->set_media_controls_cmd(media_controls_cmd(bytes, length));
  chassis->mutable_lexus()->mutable_media_controls_cmd_120()->set_ignore_overrides(ignore_overrides(bytes, length));
  chassis->mutable_lexus()->mutable_media_controls_cmd_120()->set_clear_override(clear_override(bytes, length));
  chassis->mutable_lexus()->mutable_media_controls_cmd_120()->set_clear_faults(clear_faults(bytes, length));
  chassis->mutable_lexus()->mutable_media_controls_cmd_120()->set_enable(enable(bytes, length));
}

// config detail: {'name': 'media_controls_cmd', 'enum': {0: 'MEDIA_CONTROLS_CMD_MEDIA_CONTROL_NONE', 1: 'MEDIA_CONTROLS_CMD_MEDIA_CONTROL_VOICE_COMMAND', 2: 'MEDIA_CONTROLS_CMD_MEDIA_CONTROL_MUTE', 3: 'MEDIA_CONTROLS_CMD_MEDIA_CONTROL_PREV_TRACK_ANSWER', 4: 'MEDIA_CONTROLS_CMD_MEDIA_CONTROL_NEXT_TRACK_HANG_UP', 5: 'MEDIA_CONTROLS_CMD_MEDIA_CONTROL_VOL_UP', 6: 'MEDIA_CONTROLS_CMD_MEDIA_CONTROL_VOL_DOWN'}, 'precision': 1.0, 'len': 8, 'is_signed_var': False, 'offset': 0.0, 'physical_range': '[0|255]', 'bit': 15, 'type': 'enum', 'order': 'motorola', 'physical_unit': ''}
Media_controls_cmd_120::Media_controls_cmdType Mediacontrolscmd120::media_controls_cmd(const std::uint8_t* bytes, int32_t length) const {
  Byte t0(bytes + 1);
  int32_t x = t0.get_byte(0, 8);

  Media_controls_cmd_120::Media_controls_cmdType ret =  static_cast<Media_controls_cmd_120::Media_controls_cmdType>(x);
  return ret;
}
