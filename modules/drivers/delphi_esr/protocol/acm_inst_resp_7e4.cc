...
void Acminstresp7e4::Parse(const std::uint8_t* bytes, int32_t length,
  ...
  delphi_esr->mutable_acm_inst_resp_7e4()->set_rtn_cmd_counter(rtn_cmd_counter(bytes, length));
  delphi_esr->mutable_acm_inst_resp_7e4()->set_command_return_code(command_return_code(bytes, length));
