...
template <typename T_Point>
inline DecoderRSBP<T_Point>::DecoderRSBP(
  ...
  if (this->param_.min_distance < 0.1f ||
...
template <typename T_Point>
inline RSDecoderResult DecoderRSBP<T_Point>::decodeMsopPkt(
  ...
  *height = this->lidar_const_param_.LASER_NUM;
  RSBPMsopPkt* mpkt_ptr =
      const_cast<RSBPMsopPkt*>(reinterpret_cast<const RSBPMsopPkt*>(pkt));
...
template <typename T_Point>
inline RSDecoderResult DecoderRSBP<T_Point>::decodeDifopPkt(
    const uint8_t* pkt) {
  RSBPDifopPkt* dpkt_ptr =
      const_cast<RSBPDifopPkt*>(reinterpret_cast<const RSBPDifopPkt*>(pkt));
