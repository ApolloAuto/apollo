...
#include "modules/perception/multi_sensor_fusion/proto/dst_type_fusion_config.pb.h"
...
bool DstTypeFusion::Init(const TypeFusionInitOptions &options) {
  ...
  for (auto camera_param : params.camera_params()) {
    ...
    options_.camera_max_valid_dist[camera_id] = camera_param.valid_dist();
    options_.sensor_reliability[camera_id] = camera_param.reliability();
    options_.sensor_reliability_for_unknown[camera_id] =
        camera_param.reliability_for_unknown();
    AINFO << "dst type fusion params: " << camera_id
          << " max valid dist: " << options_.camera_max_valid_dist[camera_id]
          << " reliability: " << options_.sensor_reliability[camera_id]
  ...
  for (auto lidar_param : params.lidar_params()) {
    ...
    options_.sensor_reliability[lidar_id] = lidar_param.reliability();
    options_.sensor_reliability_for_unknown[lidar_id] =
        lidar_param.reliability_for_unknown();
    AINFO << "dst type fusion params: " << lidar_id
          << " reliability: " << options_.sensor_reliability[lidar_id]
          << " reliability for unknown: "
          << options_.sensor_reliability_for_unknown[lidar_id];
  }
...
void DstTypeFusion::UpdateWithoutMeasurement(const std::string &sensor_id,
  ...
  if (sensor_manager->IsCamera(sensor_id)) {
    ...
    bool status = sensor_data_manager->GetPose(sensor_id, measurement_timestamp,
                                               &sensor2world_pose);
    auto max_dist_it = options_.camera_max_valid_dist.find(sensor_id);
    if (max_dist_it == options_.camera_max_valid_dist.end()) {
      AWARN << boost::format(
                   "There is no pre-defined max valid camera"
                   " dist for sensor type: %s") %
                   sensor_id;
    }
    if (status && max_dist_it != options_.camera_max_valid_dist.end()) {
...
double DstTypeFusion::GetReliability(const std::string &sensor_id) const {
  auto find_res = options_.sensor_reliability.find(sensor_id);
  if (find_res == options_.sensor_reliability.end()) {
...
double DstTypeFusion::GetReliabilityForUnKnown(
  ...
  auto find_res = options_.sensor_reliability_for_unknown.find(sensor_id);
  if (find_res == options_.sensor_reliability_for_unknown.end()) {
    ADEBUG << "the sensor type: " << sensor_id
           << " is not supported by class fusion";
    return 0.0;
  }
  ...
  double prob =
      (algorithm::SensorManager::Instance()->IsCamera(sensor_id) && is_night)
          ? 0.1
          : 1.0;
