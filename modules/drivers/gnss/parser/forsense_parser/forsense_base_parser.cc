...
ForsenseBaseParser::ForsenseBaseParser(const config::Config& config) {}

void ForsenseBaseParser::GetMessages(MessageInfoVec* messages) {
  if (data_ == nullptr) {
    return;
  }
  if (!PrepareMessage()) {
...
void ForsenseBaseParser::PrepareMessageStatus(const uint8_t& system_state,
...
void ForsenseBaseParser::FillIns() {
