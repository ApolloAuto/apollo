...
// 从 path_decision 中提取静态障碍物，转换为 SLPolygon，并按 s 排序
void PathBoundsDeciderUtil::GetSLPolygons(
  ...
  // 过滤条件： 非虚拟障碍物 不是 "Ignore" 决策  是静态的（速度 < 阈值） 障碍物的 s_end >= ego 车后端 s（即在前方）
...
// 处理障碍物对边界的限制
// 遍历路径上的每个 s 点，检查所有与之相交的障碍物，根据“绕行方向”（左/右绕）更新边界。
bool PathBoundsDeciderUtil::UpdatePathBoundaryBySLPolygon(
  ...
  // (1) 初始化中心线策略
  // 变道 or 左/右 regular 路径 → 使用当前边界中心作为参考
  // (2) 设置最大绕行检查距离（nudge_check_distance）
  // 变道时用 FLAGS_max_nudge_check_distance_in_lc
  // 跟车时用 FLAGS_max_nudge_check_distance_in_lk
  ...
  *narrowest_width =
  ...
  // (3) 对每个 s 点（i 从 1 开始）
  // 获取当前边界 [l_lower, l_upper]
  // 查找所有 sl_polygon 中与当前 s 相交的障碍物（s ∈ [min_s, max_s]）
  for (size_t i = 1; i < path_boundary->size(); ++i) {
    ...
    for (size_t j = 0; j < sl_polygon->size(); j++) {
      ...
      // (4) 对每个障碍物：
      // 调用 UpdatePassableInfo() 判断左右是否可通过
      sl_polygon->at(j).UpdatePassableInfo(left_bound.l, right_bound.l,
                                           adc_obs_edge_buffer);
      // 计算障碍物在当前 s 处的左右边界 l_lower_obs, l_upper_obs
      ...
      // 添加安全缓冲 adc_obs_edge_buffer = 半车宽 + FLAGS_obstacle_lat_buffer
      ...
      if (sl_polygon->at(j).NudgeInfo() == SLPolygon::UNDEFINED) {
        ...
        if (sl_polygon->at(j).is_passable()[RIGHT_INDEX]) {
          if (sl_polygon->at(j).is_passable()[LEFT_INDEX]) {
            // 若障碍物靠近中心（<0.4m）且在起步 5m 内 → 根据 ego 初始 l 速度方向决定绕行
            ...
            // 否则 → 根据最近的最大绕行偏移 last_max_nudge_l 决定（避免来回晃）
      ...
      if (sl_polygon->at(j).NudgeInfo() == SLPolygon::RIGHT_NUDGE) {
        // right nudge
        // 新左边界 = obs_right_nudge_bound.l_upper.l（即障碍物右边界 + 缓冲）
        ...
        // left nudge
        // 新右边界 = obs_left_nudge_bound.l_lower.l（障碍物左边界 - 缓冲）
...
// 遍历所有 SLPolygon，对其绕行侧的边界点调用 AddCornerPoint。
// RIGHT_NUDGE → 添加右边界的点（作为左约束）
// LEFT_NUDGE → 添加左边界的点（作为右约束）
...
void PathBoundsDeciderUtil::AddAdcVertexBounds(
  ...
  // 对每个路径点 s，计算后轴位置 = s - front_edge_to_center
  for (size_t i = 0; i < path_boundary->size(); i++) {
    ...
    // 将原始边界（未缩进）作为顶点约束加入 adc_vertex_bound
...
// 根据后轴中心 + 车轮距（wheel_base），推算前轴中心坐标。
...
// 将边界从“车道中心”坐标系转换为“参考线”坐标系（通过 offset_to_lane_center）。
...
// QP 路径优化器要求路径曲率连续。若障碍物边界太“尖锐”，可能导致不可行解。因此需根据车辆最小转弯半径放宽边界
/// @brief 
/// @param path_bound_point 当前点
/// @param is_left 
/// @param init_l 
/// @param heading 
/// @param delta_s 
/// @param init_frenet_kappa 
/// @param min_radius 
/// @return 
bool PathBoundsDeciderUtil::RelaxBoundaryPoint(
  ...
  // 2. 左右边界处理分支  
  // is_left == true：处理左边界（l_upper）
  if (is_left) {
    // 3. 特殊情形：反向曲率 + 反向航向
    // 4. 计算可达横向偏移（protective_restrict）相对于 init_l 的最大可达 l 偏移量（带符号）
    ...
    // 5. 边界放松逻辑
    // 原左边界是 l_upper.l，现在可达位置是 init_l + protective_restrict。
    // 取两者中更大的值（因为左边界是上限，越大表示越靠左，约束越松）。
    relax_constraint =
    ...
    // old_buffer：原障碍物缓冲（如 0.3m）
    // new_buffer：ego 自身的松弛缓冲（如 0.1m）
    // + old_buffer - new_buffer 表示：把障碍物边界向外挪（放松）了 (old - new) 的距离
    // 但不能比可达区域更松，所以取 min(可达值, 放松后边界)
    relax_constraint =
    ...
    // 将障碍物边界向外放松到该可达区域
    // 同时调整缓冲（从 obstacle buffer → ego slack buffer）
    relax_constraint =
...
/*
对主路径边界（前若干米）应用 RelaxBoundaryPoint
仅处理 OBSTACLE 类型的边界点
*/
bool PathBoundsDeciderUtil::RelaxEgoPathBoundary(
  ...
  // l' = dl/ds 是 Frenet 坐标系下横向偏移对纵向距离的导数
...
// 对 extra_path_bound（角点约束）同样进行松弛
...
// 如果角点约束导致堵塞，更新 blocked_id 并裁剪路径
