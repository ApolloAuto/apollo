...
bool DenselineLanePostprocessor::Init(
  ...
  AINFO << " offset_x=" << input_offset_x_
    ...
    << " crop_w=" << input_crop_width_
    << " crop_h=" << input_crop_height_;
  ...
  cc_valid_pixels_ratio_ =
    lane_postprocessor_param_.cc_valid_pixels_ratio();
...
bool DenselineLanePostprocessor::Process2D(
  ...
  ClassifyLanelinePosTypeInImage(image_group_point_set_,
    &line_pos_type_vec, &line_flag);
...
void DenselineLanePostprocessor::ConvertImagePoint2Camera(
  CameraFrame* frame) {
  ...
  for (int y = 0; y < height - omit_bottom_line_num_; y++) {
    ...
    for (int x = 0; x < width; ++x) {
      ...
      if (max_channel_idx == 0 ||
        max_score < laneline_map_score_thresh_) {
        continue;
      }
...
void DenselineLanePostprocessor::InferPointSetFromOneCC(
  ...
  for (int j = ymin; j <= ymax; j++) {
    ...
    if (left_index != -1) {
      ...
      bool left_flag = MaxScorePoint(score_left_pointer,
        x_left_pointer, x_left_count_pointer, j, &left_point);
      if (left_flag) {
    ...
    if (right_index != -1) {
      ...
      bool right_flag = MaxScorePoint(score_right_pointer,
        x_right_pointer, x_right_count_pointer, j, &right_point);
      if (right_flag) {
...
bool DenselineLanePostprocessor::MaxScorePoint(const float *score_pointer,
  const float *x_pointer, const int *x_count_pointer,
  int y_pos, LanePointInfo *point_info) {
  int large_index[2];
...
bool DenselineLanePostprocessor::LocateLanelinePointSet(
  ...
  CHECK(channels >= net_model_channel_num_);  // NOLINT
  ...
  ADEBUG << "input_size: [" << input_image_width_
      << "," << input_image_height_ << "] "
  ...
  CalLaneMap(output_data, lane_map_width_, lane_map_height_,
    &lane_map_);
  ...
  lane_ccs_.clear();
  bool flag = FindCC(lane_map_, lane_map_width_, lane_map_height_,
    roi, &lane_ccs_);
  if (!flag) {
    return false;
  }
  ...
  std::vector<LaneType>
    ccs_pos_type(select_lane_ccs_.size(), LaneType::UNKNOWN_LANE);
  ...
  Convert2OriginalCoord(lane_map_group_point_set,
    &image_group_point_set_);
  return true;
}
...
void DenselineLanePostprocessor::ClassifyLanelinePosTypeInImage(
  ...
  LocateNeighborLaneLine(latitude_intersection,
    ego_left_index, true, &adj_left_index);
  if (adj_left_index != -1) {
  ...
  LocateNeighborLaneLine(latitude_intersection,
    ego_right_index, false, &adj_right_index);
...
void DenselineLanePostprocessor::PolyFitCameraLaneline(
  CameraFrame* frame) {
  std::vector<base::LaneLine>& lane_objects = frame->lane_objects;
  int laneline_num = static_cast<int>(lane_objects.size());
  for (int line_index = 0; line_index < laneline_num; line_index++) {
...
void DenselineLanePostprocessor::GetLaneCCs(
  std::vector<unsigned char>* lane_map,
  int* lane_map_width, int* lane_map_height,
