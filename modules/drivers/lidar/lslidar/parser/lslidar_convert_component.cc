...
bool LslidarConvertComponent::Init() {
  ...
  //AERROR << "LslidarConvertComponent::init";
  conv_.reset(new Convert());
  conv_->init(lslidar_config);
    //AERROR << "LslidarConvertComponent::init   channel  name: " << lslidar_config.convert_channel_name();
 ...
 // AERROR << "LslidarConvertComponent::init success";
...
bool LslidarConvertComponent::Proc(
   ...
   // AERROR << "LslidarConvertComponent::Proc";
//  signal(SIGINT, my_handler);
