...
/**
 * @file next_76c.h
 * @brief the class of 76c (for mobileye)
 */

#ifndef MODULES_DRIVERS_MOBILEYE_PROTOCOL_NEXT_76C_H
#define MODULES_DRIVERS_MOBILEYE_PROTOCOL_NEXT_76C_H
...
class Next76c : public ::apollo::drivers::canbus::ProtocolData<Mobileye> {
 public:
  static const int ID;
  ...
  // config detail: {'name': 'lane_mark_width', 'offset': 0.0, 'precision':
  // 0.01, 'len': 8, 'f_type': 'value', 'is_signed_var': False,
  // 'physical_range': '[0|2.5]', 'bit': 56, 'type': 'double', 'order': 'intel',
  // 'physical_unit': '"meter"'}
  double lane_mark_width(const uint8_t* bytes, int32_t length) const;
};
...
#endif  // MODULES_DRIVERS_MOBILEYE_PROTOCOL_NEXT_76C_H
