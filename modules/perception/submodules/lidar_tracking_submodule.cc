 ...
 *****************************************************************************/
#include "modules/perception/submodules/lidar_tracking_submodule.h"
...
bool LidarTrackingSubmodule::Init() {
  LidarTrackingSubmoduleConfig comp_config;
  if (!GetProtoConfig(&comp_config)) {
    return false;
  }
  AINFO << "Lidar Tracking Submodule Configs: " << comp_config.DebugString();
  ...
  if (!InitAlgorithmPlugin()) {
    AERROR << "Failed to init lidar tracking submodule algorithm plugin.";
    return false;
  }
  return true;
}

bool LidarTrackingSubmodule::Proc(
    const std::shared_ptr<LidarFrameMessage>& message) {
  AINFO << std::setprecision(16)
        << "Enter lidar tracking submodule, message timestamp: "
        << message->timestamp_
...
bool LidarTrackingSubmodule::InitAlgorithmPlugin() {
...
bool LidarTrackingSubmodule::InternalProc(
  ...
  PERF_BLOCK_END_WITH_INDICATOR(sensor_name, "tracking_1::track_obstacle");
  if (ret.error_code != lidar::LidarErrorCode::Succeed) {
    out_message->error_code_ =
        apollo::common::ErrorCode::PERCEPTION_ERROR_PROCESS;
    AERROR << "Lidar tracking process error, " << ret.log;
    return true;
  }
  // TODO(shigintmin)
  ...
  PERF_BLOCK_END_WITH_INDICATOR(sensor_name, "tracking_2::fill_out_message");
