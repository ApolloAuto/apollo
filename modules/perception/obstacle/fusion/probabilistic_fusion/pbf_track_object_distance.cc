...
float PbfTrackObjectDistance::compute(const PbfTrackPtr& fused_track,
    ...
    PbfSensorObjectPtr fused_object = fused_track->get_fused_object();
    ...
    const PbfSensorObjectPtr& lidar_object = fused_track->get_latest_lidar_object();
    const PbfSensorObjectPtr& radar_object = fused_track->get_latest_radar_object();
    if (is_lidar(sensor_type)) {
        if (lidar_object != nullptr) {
            distance = compute_velodyne64_velodyne64(fused_object, sensor_object, *ref_point);
        } else if (radar_object != nullptr) {
            distance = compute_velodyne64_radar(sensor_object, fused_object, *ref_point);
        } else {
            AWARN << "All of the objects are nullptr";
        }
    } else if (is_radar(sensor_type)) {
        if (lidar_object != nullptr) {
            distance = compute_velodyne64_radar(fused_object, sensor_object, *ref_point);
        } else if (radar_object != nullptr) {
...
float PbfTrackObjectDistance::compute_velodyne64_velodyne64(const PbfSensorObjectPtr& fused_object,
                                                            const PbfSensorObjectPtr& sensor_object,
                                                            const Eigen::Vector3d& ref_pos,
                                                            int range) {
    float distance = compute_distance_3d(fused_object, sensor_object, ref_pos, range);
    ADEBUG << "compute_velodyne64_velodyne64 distance: " << distance;
    return distance;
}

float PbfTrackObjectDistance::compute_velodyne64_radar(const PbfSensorObjectPtr& fused_object,
                                                       const PbfSensorObjectPtr& sensor_object,
                                                       const Eigen::Vector3d& ref_pos,
                                                       int range) {
    float distance = compute_distance_3d(fused_object, sensor_object, ref_pos, range);
...
float PbfTrackObjectDistance::compute_radar_radar(const PbfSensorObjectPtr& fused_object,
                                                  const PbfSensorObjectPtr& sensor_object,
                                                  const Eigen::Vector3d& ref_pos,
                                                  int range) {
    float distance = compute_distance_3d(fused_object, sensor_object, ref_pos, range);
...
float PbfTrackObjectDistance::compute_distance_3d(const PbfSensorObjectPtr& fused_object,
                                                  const PbfSensorObjectPtr& sensor_object,
                                                  const Eigen::Vector3d& ref_pos,
                                                  int range) {
    ...
    bool fused_state = compute_polygon_center(fused_poly, ref_pos, range, fused_poly_center);
    if (!fused_state) {
    ...
    bool sensor_state = compute_polygon_center(sensor_poly, ref_pos, range, sensor_poly_center);
    ...
    float distance = compute_euclidean_distance(fused_poly_center, sensor_poly_center);
    return distance;
}

float PbfTrackObjectDistance::compute_euclidean_distance(const Eigen::Vector3d& des,
...
bool PbfTrackObjectDistance::compute_polygon_center(const PolygonDType& polygon,
...
bool PbfTrackObjectDistance::compute_polygon_center(const PolygonDType& polygon,
    ...
    bool state = compute_polygon_center(polygon_part, center);
    return state;
}
