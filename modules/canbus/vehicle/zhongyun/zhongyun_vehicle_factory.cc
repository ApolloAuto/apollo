...
bool ZhongyunVehicleFactory::Init(const CanbusConf *canbus_conf) {
  // Init can client
  ...
  chassis_detail_writer_ =
      node_->CreateWriter<::apollo::canbus::Zhongyun>(FLAGS_chassis_detail_topic);

  return true;
}

bool ZhongyunVehicleFactory::Start() {
...
void ZhongyunVehicleFactory::Stop() {
  can_sender_.Stop();
  can_receiver_.Stop();
  can_client_->Stop();
...
void ZhongyunVehicleFactory::UpdateCommand(
...
Chassis ZhongyunVehicleFactory::publish_chassis() {
  Chassis chassis = vehicle_controller_->chassis();
  ADEBUG << chassis.ShortDebugString();
  return chassis;
}

void ZhongyunVehicleFactory::PublishChassisDetail() {
...
ZhongyunVehicleFactory::CreateMessageManager() {
  return std::unique_ptr<MessageManager<::apollo::canbus::Zhongyun>>(
