...
def callback_routing(data):
    ...
    global last_routing
    routing_=data
    if not last_routing==None:
        routing_.waypoint[0].CopyFrom(last_routing.waypoint[1])
    last_routing=routing_
    frouting = open("/apollo/data/routing_data/routing_"+str(num_)+".txt", 'w')
