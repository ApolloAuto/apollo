...
void Steeringmotorrpt375::Parse(const std::uint8_t* bytes, int32_t length,
  ...
  chassis->mutable_gem()->mutable_steering_motor_rpt_3_75()->set_torque_output(torque_output(bytes, length));
  chassis->mutable_gem()->mutable_steering_motor_rpt_3_75()->set_torque_input(torque_input(bytes, length));
}
