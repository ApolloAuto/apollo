 ...
 * @return The angle between the directions of the start point and the end point.  
 */
template <typename PointType>
double GetPathAngle(const std::vector<PointType> &points,
                    const int start,
                    const int end) {
 ...
 * @param points Points on the path.
 * @param angle_threshold Points are sampled when the accumulated direction change exceeds the threshold. 
...
bool DownSampleByAngle(const std::vector<PointType> &points,
    ...
    AINFO << "Point Vector is down sampled from " << points.size() << " to " << sampled_indices->size();
