...
bool MlfMotionFilter::Init(const MlfFilterInitOptions& options) {
  std::string config_file = "mlf_motion_filter.conf";
  ...
  MlfMotionRefinerInitOptions mlf_motion_refiner_options;
  mlf_motion_refiner_options.config_path = options.config_path;
  ACHECK(motion_refiner_->Init(mlf_motion_refiner_options));
  return true;
}
