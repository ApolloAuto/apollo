...
TEST_F(SimControlTest, Test) {
  ...
  routing.mutable_routing_request()->mutable_start()->mutable_pose()
    ->set_x(1.0);
  routing.mutable_routing_request()->mutable_start()->mutable_pose()
    ->set_y(1.0);
  ...
  sim_control.SetStartPoint(routing);

  AdapterManager::AddPlanningCallback(&SimControl::OnPlanning, &sim_control);
...

  {
    ...
    sim_control.TimerCallback(ros::TimerEvent());

    Chassis* chassis = AdapterManager::GetChassis()->GetLatestPublished();
    LocalizationEstimate* localization = AdapterManager::GetLocalization()
      ->GetLatestPublished();
