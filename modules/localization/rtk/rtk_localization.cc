...
void RTKLocalization::GpsCallback(
  ...
  double time_delay = last_received_timestamp_sec_ ?
      common::time::ToSecond(Clock::Now()) - last_received_timestamp_sec_ : last_received_timestamp_sec_;
...
void RTKLocalization::RunWatchDog(double gps_timestamp) {
  ...
  double gps_service_delay = common::time::ToSecond(Clock::Now()) - service_started_time;
  int64_t gps_delay_cycle_cnt =
      static_cast<int64_t>(gps_delay_sec * localization_publish_freq_);

  bool msg_delay = false;
  if ((gps_delay_cycle_cnt > report_threshold_err_num_) && (static_cast<int>(gps_service_delay) > service_delay_threshold)) {
    msg_delay = true;
    std::stringstream ss;
  ...
  if ((imu_delay_cycle_cnt > report_threshold_err_num_) && (static_cast<int>(gps_service_delay) > service_delay_threshold)) {
    msg_delay = true;
    std::stringstream ss;
