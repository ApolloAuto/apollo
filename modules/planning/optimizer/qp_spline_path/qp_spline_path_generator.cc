...
bool QpSplinePathGenerator::Init(const std::string& config_file) {
  if (!common::util::GetProtoFromFile(config_file, &qp_spline_path_config_)) {
...
bool QpSplinePathGenerator::generate(const ReferenceLine& reference_line,
  ...
  double curr_knot_s = start_s + delta_s;
  for (uint32_t i = 0; i <= qp_spline_path_config_.number_of_knots(); ++i) {
    fx_knots_.push_back(curr_knot_s);
    curr_knot_s += delta_s;
  }
...
bool QpSplinePathGenerator::init_smoothing_spline() {
  spline_generator_.reset(
      new Spline1dGenerator(fx_knots_, qp_spline_path_config_.spline_order()));
  return true;
}
...
bool QpSplinePathGenerator::setup_kernel() {
  ...
  if (qp_spline_path_config_.number_of_knots() > 1) {
    std::vector<double> l_vec(fx_knots_.size(), 0.0);
    spline_kernel->add_reference_line_kernel_matrix(
        fx_knots_, l_vec, qp_spline_path_config_.reference_line_weight());
  }
  return true;
}
