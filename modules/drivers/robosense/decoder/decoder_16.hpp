...
template <typename T_Point>
inline DecoderRS16<T_Point>::DecoderRS16(
  ...
  if (this->param_.min_distance < 0.2f ||
...
template <typename T_Point>
inline RSDecoderResult DecoderRS16<T_Point>::decodeMsopPkt(
  ...
  *height = this->lidar_const_param_.LASER_NUM;
  RS16MsopPkt* mpkt_ptr =
      const_cast<RS16MsopPkt*>(reinterpret_cast<const RS16MsopPkt*>(pkt));
  ...
  for (size_t blk_idx = 0; blk_idx < this->lidar_const_param_.BLOCKS_PER_PKT;
    ...
    if (blk_idx == 0) {
      azi_diff = static_cast<float>(
          (RS_ONE_ROUND + RS_SWAP_SHORT(mpkt_ptr->blocks[blk_idx + 1].azimuth) -
           cur_azi) %
          RS_ONE_ROUND);
    } else {
      azi_diff = static_cast<float>(
    ...
    for (size_t channel_idx = 0;
      ...
      if (this->echo_mode_ == ECHO_DUAL) {
        azi_channel_ori =
            cur_azi + azi_diff * this->lidar_const_param_.DSR_TOFFSET *
      ...
      int azi_channel_final =
          this->azimuthCalibration(azi_channel_ori, channel_idx % 16);
      float distance =
          RS_SWAP_SHORT(
      ...
      int angle_vert =
          ((this->vert_angle_list_[channel_idx % 16]) + RS_ONE_ROUND) %
          RS_ONE_ROUND;
      T_Point point;
      if ((distance <= this->param_.max_distance &&
          ...
          ((this->angle_flag_ && azi_channel_final >= this->start_angle_ &&
           ...
           (!this->angle_flag_ && ((azi_channel_final >= this->start_angle_) ||
                                   (azi_channel_final <= this->end_angle_))))) {
        double x = distance * this->cos_lookup_table_[angle_vert] *
                       this->cos_lookup_table_[azi_channel_final] +
                   this->lidar_const_param_.RX *
                       this->cos_lookup_table_[angle_horiz_ori];

        double y = -distance * this->cos_lookup_table_[angle_vert] *
      ...
      if (this->echo_mode_ != ECHO_DUAL && channel_idx > 15) {
        point.set_timestamp(block_timestamp +
                            (this->time_duration_between_blocks_ / 2) *
                                kSecondToNanoFactor);
...
template <typename T_Point>
inline RSDecoderResult DecoderRS16<T_Point>::decodeDifopPkt(
    const uint8_t* pkt) {
  RS16DifopPkt* dpkt_ptr =
      const_cast<RS16DifopPkt*>(reinterpret_cast<const RS16DifopPkt*>(pkt));
  ...
  if (!this->difop_flag_) {
    ...
    for (size_t i = 0; i < this->lidar_const_param_.CHANNELS_PER_BLOCK / 2;
         i++) {
      ...
      this->vert_angle_list_[i] =
          (lsb * 256 * 256 + mid * 256 + msb) * neg * 0.01f;  // / 180 * M_PI;
