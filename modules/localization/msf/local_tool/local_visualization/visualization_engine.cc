...
// =================VisualizationEngine=================   
bool MapImageKey::operator<(
        const MapImageKey &key) const {
...
VisualizationEngine::VisualizationEngine() 
    ...
    _map_image_cache(20), 
    _big_window(3072, 3072, CV_8UC3), _map_config(){
    _is_init = false;
    _follow_car = true;
    _auto_play = false;
    _resolution_id = 0;
...
bool VisualizationEngine::Init(const std::string &map_folder, 
    ...
    _cloud_img = cv::Mat(cv::Size(_map_config._map_node_size_x, 
            _map_config._map_node_size_y), CV_8UC3);
    _cloud_img_mask = cv::Mat(cv::Size(_map_config._map_node_size_x, 
            _map_config._map_node_size_y), CV_8UC1);

    Preprocess(map_folder);

    std::string params_file = _image_visual_resolution_path 
                                        + "/param.txt";
...
void VisualizationEngine::Visualize(const Eigen::Affine3d &cur_pose, 
    ...
    _cloud_img_lt_coord[0] 
            = cen[0] - _map_config._map_resolutions[_resolution_id] 
                * (img_width / 2.0f);
    _cloud_img_lt_coord[1] 
            = cen[1] - _map_config._map_resolutions[_resolution_id] 
                * (img_height / 2.0f);

    _cloud_img.setTo(cv::Scalar(0, 0, 0));
    _cloud_img_mask.setTo(cv::Scalar(0));
    CloudToMat(_car_pose, _velodyne_extrinsic, 
            cloud, _cloud_img, _cloud_img_mask); 
    Draw();
}
...
void VisualizationEngine::Preprocess(const std::string &map_folder) {
    ...
    _image_visual_resolution_path = 
                                image_visual_path + buf;
    std::cout << "image_visual_resolution_path: " 
                << _image_visual_resolution_path << std::endl;
    std::string image_resolution_path = 
                                image_path + buf;
    std::cout << "image_resolution_path: " 
            << image_resolution_path << std::endl;
    
    boost::filesystem::path image_visual_path_boost(
                                    image_visual_path);
    if (!boost::filesystem::exists(image_visual_path_boost)) {
        boost::filesystem::create_directory(
                                image_visual_path_boost);
    } 

    boost::filesystem::path image_resolution_path_boost(
    ...
    if (boost::filesystem::exists(_image_visual_resolution_path)) {
        ...
        boost::filesystem::create_directory(
                                image_visual_resolution_path_boost);
    }
    ...
    for (; iter != end_iter; ++iter) {
        if (!boost::filesystem::is_directory(*iter)) {
            ...
            tmp = tmp.substr(image_resolution_path.length(), 
                                                    tmp.length());
...
void VisualizationEngine::Draw() {
    ...
    for (int i = -1; i <= 1; ++i) {
        for (int j = -1; j <= 1; ++j) {
            ...
            if (LoadImageToCache(key)) {
                ...
                _subMat[i + 1][j + 1] = cv::Mat(1024, 1024, CV_8UC3, cv::Scalar(0, 0, 0));
            }
        }
    }
    ...
    for (int i = 0; i < 3; ++i) {
        for (int j = 0; j < 3; ++j) {
            _subMat[i][j].copyTo(_big_window(cv::Rect(j * 1024, i * 1024, 1024, 1024)));
        }
    }
    
    cv::Point map_grid_index = CoordToMapGridIndex(_view_center, 
                                    _resolution_id, _cur_stride);
    cv::Point node_grid_index = 
                    MapGridIndexToNodeGridIndex(map_grid_index);
...
void VisualizationEngine::DrawCloud(const cv::Point &bias) {
    if (_cur_level == 0) {
        ...
        if (lt.x >= 0 && lt.y >= 0 && rb.x <= 1024 * 3 && rb.y <= 1024 * 3) {
            _cloud_img.copyTo(_big_window(cv::Rect(lt.x, lt.y, 1024, 1024)), _cloud_img_mask);
...
void VisualizationEngine::UpdateLevel() {
    if (_cur_scale > _max_stride * 1.5)
        SetScale(_max_stride * 1.5);
    if (_cur_scale < 0.5)
        SetScale(0.5);
...
void VisualizationEngine::GenerateMutiResolutionImages(
    ...
    outf << image_visual_path_dst.substr(dst_folder.length(), 
            image_visual_path_dst.length() - 
            dst_folder.length() - 3) << std::endl;
    outf.close();
    return;
}
...
bool VisualizationEngine::InitOtherParams(const std::string &params_file) {
    ...
    inf >> x_min >> y_min >> x_max 
        >> y_max >> level >> path;
    inf.close();

    InitOtherParams(x_min, y_min, x_max, y_max,
                    level, path); 
...
void VisualizationEngine::InitOtherParams(const int x_min, const int y_min,
    ...
    double init_center_x = (double(x_max + x_min) /2 * 
            ...
            _map_config._map_node_size_x) + _map_config._map_range.GetMinX();
    double init_center_y = (double(y_max + y_min) /2 * 
                _map_config._map_resolutions[_resolution_id] * 
                _map_config._map_node_size_y) + _map_config._map_range.GetMinY();
    ...
    std::cout << "image_visual_leaf_path: " 
                << _image_visual_leaf_path << std::endl;
}
...
void VisualizationEngine::CoordToImageKey(const Eigen::Vector2d& coord,
    ...
    MapNodeIndex index = MapNodeIndex::get_map_node_index(_map_config, coord, 
                                        _resolution_id, _zone_id);
...
bool VisualizationEngine::LoadImageToCache(const MapImageKey& key) {
    ...
    if (!_map_image_cache.Get(key, img)) {
        ...
        snprintf(path, 1024, "%s/%02d/%08d/%08d_%d.png",
                _image_visual_leaf_path.c_str(), key.zone_id,
                key.node_north_id, key.node_east_id, key.level);
