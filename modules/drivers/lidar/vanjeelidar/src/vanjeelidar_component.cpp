...
bool VanjeelidarComponent::Init() {
    ...
    driver_ptr_ = std::make_shared<
            ::vanjee::lidar::LidarDriver<PointCloudMsg>>();
    ...
    if (conf_.config_base().source_type()
        ...
        driver_ptr_->regPacketCallback(std::bind(
                &VanjeelidarComponent::VanjeePacketCallback,
                this,
                std::placeholders::_1));
    }
    ...
    driver_ptr_->regPointCloudCallback(
            ...
            std::bind(
                    &VanjeelidarComponent::VanjeeCloudPutCallback,
                    this,
                    std::placeholders::_1));
    driver_ptr_->regExceptionCallback(
            [](const ::vanjee::lidar::Error& code) {
...
void VanjeelidarComponent::ReadScanCallback(
    ...
    std::shared_ptr<::vanjee::lidar::Packet> scan_packet
            = std::make_shared<::vanjee::lidar::Packet>();
    ...
    std::shared_ptr<Buffer> pkt = cb_get_pkt_(scan_message->data().pkt_length);
    memcpy(pkt->data(), scan_message->data().begin(), scan_message->data().end());
    pkt->setData(0, scan_message->data().pkt_length);
    pushPacket(pkt);
...
std::shared_ptr<PointCloudMsg> VanjeelidarComponent::VanjeeCloudAllocateCallback() {
