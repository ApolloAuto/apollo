/******************************************************************************
 * Adapted from Apollo.(spiral_problem_interface.cc)
 * Author: Jinyun Zhou
 *
...
bool cosThetaProbleminterface::get_bounds_info(int n, double* x_l, double* x_u,
                                               int m, double* g_l,
                                               double* g_u) {
  assert(std::size_t(n) == num_of_variables_);
  assert(std::size_t(m) == num_of_constraints_);
  // variables
  // a. for x, y
...
bool cosThetaProbleminterface::eval_grad_f(int n, const double* x, bool new_x,
  ...
  for (std::size_t i = 0; i < num_of_points_ - 2; ++i) {
    ...
    grad_f[index] +=
        -weight_cos_included_angle_ *
        ((x[index + 2] - x[index + 4]) /
             (std::pow((x[index] - x[index + 2]) * (x[index] - x[index + 2]) +
                           (x[index + 1] - x[index + 3]) *
                               (x[index + 1] - x[index + 3]),
                       0.5) *
              std::pow((x[index + 2] - x[index + 4]) *
                               (x[index + 2] - x[index + 4]) +
                           (x[index + 3] - x[index + 5]) *
                               (x[index + 3] - x[index + 5]),
                       0.5)) -
...
bool cosThetaProbleminterface::eval_h(int n, const double* x, bool new_x,
  ...
  if (values == NULL) {
    ...
    assert(index == static_cast<std::size_t>(nele_hess));
  } else {
    ...
    for (std::size_t i = 0; i < num_of_points_ - 2; ++i) {
      ...
      values[idx_map_[std::make_pair(topleft, topleft)]] +=
          ...
          ((3 * (2 * x[topleft] - 2 * x[topleft + 2]) *
            (2 * x[topleft] - 2 * x[topleft + 2]) *
            ((x[topleft] - x[topleft + 2]) * (x[topleft + 2] - x[topleft + 4]) +
             (x[topleft + 1] - x[topleft + 3]) *
                 (x[topleft + 3] - x[topleft + 5]))) /
               (4 *
                std::pow((x[topleft] - x[topleft + 2]) *
                                 (x[topleft] - x[topleft + 2]) +
                             (x[topleft + 1] - x[topleft + 3]) *
                                 (x[topleft + 1] - x[topleft + 3]),
                         2.5) *
                std::pow((x[topleft + 2] - x[topleft + 4]) *
                                 (x[topleft + 2] - x[topleft + 4]) +
                             (x[topleft + 3] - x[topleft + 5]) *
                                 (x[topleft + 3] - x[topleft + 5]),
                         0.5)) -
           ((x[topleft] - x[topleft + 2]) * (x[topleft + 2] - x[topleft + 4]) +
      ...
      values[idx_map_[std::make_pair(topleft + 2, topleft)]] +=
          obj_factor * (-weight_cos_included_angle_) *
          (1 / (std::pow((x[topleft] - x[topleft + 2]) *
                                 (x[topleft] - x[topleft + 2]) +
                             (x[topleft + 1] - x[topleft + 3]) *
                                 (x[topleft + 1] - x[topleft + 3]),
                         0.5) *
               ...
               (std::pow((x[topleft] - x[topleft + 2]) *
                ...
                std::pow((x[topleft + 2] - x[topleft + 4]) *
                             ...
                             (x[topleft + 3] - x[topleft + 5]) *
                                 (x[topleft + 3] - x[topleft + 5]),
                         0.5)) -
           2 / (std::pow((x[topleft] - x[topleft + 2]) *
                                 (x[topleft] - x[topleft + 2]) +
                             (x[topleft + 1] - x[topleft + 3]) *
                                 (x[topleft + 1] - x[topleft + 3]),
                         0.5) *
