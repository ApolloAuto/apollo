...
void Select::SelectTrafficLights(
  ...
  for (size_t row = 0; row < hdmap_bboxes->size(); ++row) {
    auto center_hd = hdmap_bboxes->at(row)->region.detection_roi.Center();
    if (hdmap_bboxes->at(row)->region.outside_image) {
    ...
    for (size_t col = 0; col < refined_bboxes.size(); ++col) {
      ...
      const auto &crop_roi = hdmap_bboxes->at(row)->region.crop_roi;
  ...
  for (size_t i = 0; i < hdmap_bboxes->size(); ++i) {
    hdmap_bboxes->at(i)->region.is_selected = false;
    hdmap_bboxes->at(i)->region.is_detected = false;
  }

  for (size_t i = 0; i < assignments.size(); ++i) {
    if (assignments[i].first >= hdmap_bboxes->size() ||
        assignments[i].second >= refined_bboxes.size() ||
        hdmap_bboxes->at(assignments[i].first)->region.is_selected ||
      ...
      auto &hdmap_bbox_region = hdmap_bboxes->at(assignments[i].first)->region;
  ...
  for (size_t i = 0; i < hdmap_bboxes->size(); ++i) {
    AINFO << "hdmap_bboxes-" << i << ":"
          << " projection_roi: "
          << hdmap_bboxes->at(i)->region.projection_roi.ToStr()
          << " detection_roi: "
          << hdmap_bboxes->at(i)->region.detection_roi.ToStr();
  }
}
