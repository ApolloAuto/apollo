...
TEST_F(Vcupowerstatus214Test, reset) {
  ...
  EXPECT_EQ(cd.vcu_powerstatus_214().vcu_powermode(), 1);
  EXPECT_EQ(cd.vcu_powerstatus_214().vcu_powermodevalid(), 2);
  EXPECT_EQ(cd.vcu_powerstatus_214().replacebatterystateindication(), true);
}
