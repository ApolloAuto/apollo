...
class WorkerThread: public std::enable_shared_from_this<WorkerThread<ThreadPool> > {
 ...
 public:
	...
	static void create_and_attach(std::shared_ptr<ThreadPool> const & pool) {
	  ...
	  if (worker) {
			worker->thread_.reset(new std::thread(std::bind(&WorkerThread::run, worker)));
	  }
	}
...
// TODO is only called once
  ...
  private:	
    ...
    void terminate_all_workers(bool const wait) volatile
    {
      ...
      if(wait)
      {
        ...
        for(typename std::vector<std::shared_ptr<WorkerType> >::iterator it = self->terminated_workers_.begin();
          it != self->terminated_workers_.end();
          ++it)
        {
          (*it)->join();
        }
    ...
    */
    bool resize(size_t const worker_count) volatile
    {
      ...
      { // decrease worker count
        locked_this->task_or_terminate_workers_event_.notify_all();   // TODO: Optimize number of notified workers
      }
  ...
  private: // The following members are accessed only by _one_ thread at the same time:
    ...
    std::vector<std::shared_ptr<WorkerType> > terminated_workers_; // List of workers which are terminated but not fully destructed.
    
  private: // The following members are implemented thread-safe:
    mutable std::recursive_mutex  monitor_;
    mutable std::condition_variable_any worker_idle_or_terminated_event_;	// A worker is idle or was terminated.
    mutable std::condition_variable_any task_or_terminate_workers_event_;  // Task is available OR total worker count should be reduced.
