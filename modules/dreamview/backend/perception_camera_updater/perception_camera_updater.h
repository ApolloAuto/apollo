...
class PerceptionCameraUpdater {
 ...
 private:
  ...
  void InitReaders();
  void OnCompressedImage(const std::shared_ptr<apollo::drivers::CompressedImage> &image);
  ...
  void OnObstacles(const std::shared_ptr<apollo::perception::PerceptionObstacles> &obstacles);
  /**
  ...
  std::deque<std::shared_ptr<apollo::localization::LocalizationEstimate>>
      localization_queue_;
  std::shared_ptr<cyber::Reader<drivers::Image>>
