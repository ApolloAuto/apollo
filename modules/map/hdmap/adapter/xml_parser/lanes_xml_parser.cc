...
Status LanesXmlParser::ParseSingleSideRoadSampleAssociates(
    const tinyxml2::XMLElement& xml_node, PbLane* lane, bool bleft) {
  CHECK_NOTNULL(lane);

  auto sub_node = xml_node.FirstChildElement(
                                  bleft ? "leftRoadSampleAssociations"
...
Status LanesXmlParser::ParseLeftRoadSampleAssociates(
  ...
  return ParseSingleSideRoadSampleAssociates(xml_node, lane, true);
}
...
Status LanesXmlParser::ParseRightRoadSampleAssociates(
  ...
  return ParseSingleSideRoadSampleAssociates(xml_node, lane, false);
}
