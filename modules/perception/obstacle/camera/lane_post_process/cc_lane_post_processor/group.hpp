...
// @file: group.hpp
// @brief: group class definition for lane post-processing

#ifndef ADU_PERCEPTION_OBSTACLE_CAMERA_CC_LANE_POST_PROCESSER_GROUP_HPP
#define ADU_PERCEPTION_OBSTACLE_CAMERA_CC_LANE_POST_PROCESSER_GROUP_HPP
...
struct Group {
    ...
    static bool comp(const Group& a, const Group& b) {
        return (a.space_type == SpaceType::IMAGE) ?
               a.end_pos(1) > b.end_pos(1) : a.end_pos(1) < b.end_pos(1);
    }
...
inline int Group::compute_orientation(const std::vector<Marker>& markers,
    ...
    switch (shape_type) {
        case MarkerShapeType::POINT : {
            ...
            XLOG_IF(ERROR, start_ind > end_ind)
                << "start_ind (" << start_ind << ") is larger than end_ind (" << end_ind << ")";
            XLOG(DEBUG) << "start markers: start_ind=" << start_ind << ", end_ind=" << end_ind;
        ...
        case MarkerShapeType::LINE_SEGMENT : {
            int start_ind = std::min(group_param.orientation_estimation_skip_marker_num,
        ...
        default : {
            XLOG(FATAL) << "unknown marker shape type.";
        }
    }
    ...
    switch (shape_type) {
        case MarkerShapeType::POINT : {
            ...
            XLOG_IF(ERROR, start_ind < end_ind)
                << "start_ind (" << start_ind << ") is smaller than end_ind (" << end_ind << ")";
            XLOG(DEBUG) << "end markers: start_ind=" << start_ind << ", end_ind=" << end_ind;
...
inline int Group::compute_orientations(const std::vector<Marker>& markers,
    ...
    switch (shape_type) {
        case MarkerShapeType::POINT : {
            ...
            for (start_ind = 0; start_ind <= end_ind; ++start_ind) {
                this->start_pos_list.push_back(markers.at(start_marker_idx.at(start_ind)).pos);
                this->start_orie_list.push_back(markers.at(start_marker_idx.at(end_ind)).pos -
                                                markers.at(start_marker_idx.at(start_ind)).pos);
            }
            break;
        }
    ...
    for (auto it_start_orie = this->start_orie_list.begin();
        ...
        this->start_angle_list.push_back(std::atan2((*it_start_orie)(1), (*it_start_orie)(0)));
    ...
    switch (shape_type) {
        case MarkerShapeType::POINT : {
            ...
            for (end_ind = 0; end_ind <= start_ind; ++end_ind) {
                this->end_pos_list.push_back(markers.at(end_marker_idx.at(end_ind)).pos);
                this->end_orie_list.push_back(markers.at(end_marker_idx.at(end_ind)).pos -
                                              markers.at(end_marker_idx.at(start_ind)).pos);
            }
            break;
        }
    ...
    for (auto it_end_orie = this->end_orie_list.begin();
        ...
        this->end_angle_list.push_back(std::atan2((*it_end_orie)(1), (*it_end_orie)(0)));
...
inline ScalarType Group::compute_distance(const Group& tar_group,
    ...
    if (norm_v < kEpsilon) {
        XLOG(DEBUG) << "norm of orientation vector for reference group is too small: " << norm_v;
    }
    ...
    if (projection_dist < param.min_distance || projection_dist > param.max_distance) {
        return -1;
    }
    ...
    ScalarType departure_dist = std::abs(this->end_orie.y() * tar_group.start_pos.x() -
    ...
    XLOG_IF(ERROR, deviation_angle_dist < 0 ||
        ...
        << "deviation_angle_dist is out range of [0, 2*pi]: " << deviation_angle_dist;
    ...
    XLOG_IF(ERROR, deviation_angle_dist < 0 ||
        ...
        << "deviation_angle_dist is out range of [0, pi]: " << deviation_angle_dist;
    ...
    XLOG(DEBUG) << "(4a) tar_group_start_len = " << std::to_string(tar_group_start_len)
    ...
    if (tar_group_start_len > param.min_orientation_estimation_size) {
        ...
        XLOG_IF(ERROR, orie_dist < 0 ||
                       orie_dist > 2 * static_cast<ScalarType>(M_PI))
        ...
        XLOG_IF(ERROR, orie_dist < 0 ||
                       orie_dist > static_cast<ScalarType>(M_PI))
            << "orie_dist is out range of [0, pi]: " << orie_dist;
        XLOG(DEBUG) << "(4b) orie_dist = " << std::to_string(orie_dist / M_PI * 180.0)
...
inline ScalarType Group::compute_distance(const Group& tar_group,
                                          ...
                                          int cur_group_end_marker_descend_id,
                                          int tar_group_start_marker_ascend_id) const {
    XLOG(DEBUG) << "cur_group_end_marker_descend_id=" << cur_group_end_marker_descend_id;
    XLOG_IF(ERROR, cur_group_end_marker_descend_id < 0)
        << "cur_group_end_marker_descend_id = " << cur_group_end_marker_descend_id;
    XLOG_IF(ERROR, cur_group_end_marker_descend_id >=
                   static_cast<int>(this->end_pos_list.size()))
        << "cur_group_end_marker_descend_id = " << cur_group_end_marker_descend_id;

    Vector2D cur_group_end_pos = this->end_pos_list.at(cur_group_end_marker_descend_id);
    Vector2D cur_group_end_orie = this->end_orie_list.at(cur_group_end_marker_descend_id);

    XLOG(DEBUG) << "tar_group_start_marker_ascend_id=" << tar_group_start_marker_ascend_id;
    XLOG_IF(ERROR, tar_group_start_marker_ascend_id < 0)
        << "tar_group_start_marker_ascend_id = " << tar_group_start_marker_ascend_id;
    XLOG_IF(ERROR, tar_group_start_marker_ascend_id >=
                   static_cast<int>(tar_group.start_pos_list.size()))
        << "tar_group_start_marker_ascend_id = " << tar_group_start_marker_ascend_id;

    Vector2D tar_group_start_pos = tar_group.start_pos_list.at(tar_group_start_marker_ascend_id);
    Vector2D tar_group_start_orie = tar_group.start_orie_list.at(tar_group_start_marker_ascend_id);
    ...
    if (norm_v < kEpsilon) {
        XLOG(INFO) << "norm of orientation vector for reference group is too small: " << norm_v;
    }
    ...
    // the normal distance from a point (x2, y2) to the line:
    // d = |A * x2 + B * y2 + C| / sqrt(A^2 + B^2)
    ScalarType departure_dist = std::abs(cur_group_end_orie.y() * tar_group_start_pos.x() -
    ...
    ScalarType gamma = std::atan2(displacement(1), displacement(0));
    XLOG_IF(ERROR, gamma < -static_cast<ScalarType>(M_PI) ||
        gamma > static_cast<ScalarType>(M_PI)) << "gamma is out range of [-pi, pi]: " << gamma;
    if (gamma < 0) {
        gamma += 2 * static_cast<ScalarType>(M_PI);
    }
    XLOG(DEBUG) << "gamma = " << std::to_string(gamma / M_PI * 180.0);
    ...
    if (tar_group_start_len > param.min_orientation_estimation_size) {
        // orientation angle of start markers on target group
        ScalarType alpha = tar_group.start_angle_list.at(tar_group_start_marker_ascend_id);
        XLOG(DEBUG) << "alpha = " << std::to_string(alpha / M_PI * 180.0);

        orie_dist = std::abs(alpha - beta);
        XLOG_IF(ERROR, orie_dist < 0 || orie_dist > 2 * static_cast<ScalarType>(M_PI))
            << "orie_dist is out range of [0, 2*pi]: " << orie_dist;
        if (orie_dist > static_cast<ScalarType>(M_PI)) {
            orie_dist = 2 * static_cast<ScalarType>(M_PI) - orie_dist;
        }
...
#endif  // ADU_PERCEPTION_OBSTACLE_CAMERA_CC_LANE_POST_PROCESSER_GROUP_HPP
