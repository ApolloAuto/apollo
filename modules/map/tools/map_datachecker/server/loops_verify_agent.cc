...
void LoopsVerifyAgent::StartVerify(LOOPS_VERIFY_REQUEST_TYPE *request,
                                   LOOPS_VERIFY_RESPONSE_TYPE *response) {
...
void LoopsVerifyAgent::CheckVerify(LOOPS_VERIFY_REQUEST_TYPE *request,
                                   LOOPS_VERIFY_RESPONSE_TYPE *response) {
  ...
  if (std::abs(1.0 - progress) < 1e-8) {
    ...
    if (data_type == DataType::MAP_CHECKOUT) {
      if (is_reached) {
        ...
        loop_result->set_loop_num(
            lap - _sp_conf->laps_number >= 0
                ? static_cast<double>(lap - _sp_conf->laps_number)
                : 0.0);
      }
    }
  }
...
void LoopsVerifyAgent::StopVerify(LOOPS_VERIFY_REQUEST_TYPE *request,
                                  LOOPS_VERIFY_RESPONSE_TYPE *response) {
...
int LoopsVerifyAgent::get_poses_to_check(
  ...
  std::shared_ptr<std::vector<FramePose>> sp_vec_poses =
      _sp_pose_collection_agent->get_poses();
...
LoopsVerifyAgentState LoopsVerifyAgent::get_state() { return _state; }
