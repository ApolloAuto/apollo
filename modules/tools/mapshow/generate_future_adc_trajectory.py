...
def generate_future_pose_list(rosbag_in, period):
    ...
    for i in range(len(total_adc_trajectory)-1):
        ...
        ds = math.sqrt( \
          (curr_pose.position.x - next_pose.position.x) * (curr_pose.position.x - next_pose.position.x) + \
          (curr_pose.position.y - next_pose.position.y) * (curr_pose.position.y - next_pose.position.y))
        if ds < 0.001:
...
def augment_future_traj(new_localization, ptr_start, ptr_end, pose_list):
    ...
    for i in range(ptr_start, ptr_end-1):
        ...
        ds = math.sqrt((pose2.position.y - pose1.position.y) * (pose2.position.y - pose1.position.y) + \
            (pose2.position.x - pose1.position.x) * (pose2.position.x - pose1.position.x))
        trajectory_point.path_point.kappa = (pose2.heading - pose1.heading) / ds;
    ...
    last_trajectory_point.path_point.kappa = new_localization.trajectory_point[last_idx].path_point.kappa
    last_trajectory_point.path_point.s = sum_s
    trajectory_point.relative_time = relative_time

if __name__ == '__main__':
    parser = argparse.ArgumentParser(description = "generate perfect future trajectory\
            based on localization estimate")
    ...
    if os.path.isfile(path):
        ...
        path_out = os.path.dirname(path) + '/' + bag_name + '_with_future_trajectory.bag'
