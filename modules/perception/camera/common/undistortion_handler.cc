...
bool UndistortionHandler::Init(const std::string &sensor_name, int device) {
    ...
    InitUndistortRectifyMap(distort_model->get_intrinsic_params(),
                            ...
                            I,
                            distort_model->get_intrinsic_params(),
                            width_, height_, &d_mapx_, &d_mapy_);
    ...
    switch (src_img.channels()) {
      case 1:
        status = nppiRemap_8u_C1R(src_img.gpu_data(), image_size,
                                  ...
                                  d_mapx_.gpu_data(), d_map_step,
                                  d_mapy_.gpu_data(), d_map_step,
                                  dst_img->mutable_gpu_data(),
                                  dst_img->width_step(), image_size,
                                  remap_mode);
        break;
      case 3:
