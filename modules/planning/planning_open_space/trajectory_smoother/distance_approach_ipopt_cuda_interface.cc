...
#if USE_GPU == 1
#include "modules/planning/planning_open_space/trajectory_smoother/planning_block.h"
#endif
...
DistanceApproachIPOPTCUDAInterface::DistanceApproachIPOPTCUDAInterface(
    ...
    ACHECK(horizon < std::numeric_limits<int>::max()) << "Invalid cast on horizon in open space planner";
    horizon_ = static_cast<int>(horizon);
    ACHECK(obstacles_num < std::numeric_limits<int>::max()) << "Invalid cast on obstacles_num in open space planner";
...
bool DistanceApproachIPOPTCUDAInterface::get_nlp_info(
    ...
    nnz_jac_g = 24 * horizon_ + 3 * horizon_ + 2 * horizon_ + tmp - 1 + (num_of_variables_ - (horizon_ + 1) + 2);
...
bool DistanceApproachIPOPTCUDAInterface::get_bounds_info(
    ...
    ADEBUG << "constraint_index after adding Euler forward dynamics constraints: " << constraint_index;
...
bool DistanceApproachIPOPTCUDAInterface::eval_f(int n, const double* x, bool new_x, double& obj_value) {
    eval_obj(n, x, &obj_value);
    return true;
}

bool DistanceApproachIPOPTCUDAInterface::eval_grad_f(int n, const double* x, bool new_x, double* grad_f) {
    gradient(tag_f, n, x, grad_f);
    return true;
}

bool DistanceApproachIPOPTCUDAInterface::eval_grad_f_hand(int n, const double* x, bool new_x, double* grad_f) {
    ...
    if (grad_f == nullptr) {
        ...
        grad_f[control_index] += 2.0 * last_time_steer_rate * (weight_stitching_steer_ / x[time_index] / ts_);
        grad_f[control_index + 1] += 2.0 * last_time_a_rate * (weight_stitching_a_ / x[time_index] / ts_);
        ...
        for (int i = 0; i < horizon_ - 1; ++i) {
            ...
            grad_f[time_index] += -2.0 * weight_rate_steer_ * steering_rate * steering_rate / x[time_index];
        ...
        for (int i = 0; i < horizon_ + 1; ++i) {
            grad_f[time_index] += weight_first_order_time_ + 2.0 * weight_second_order_time_ * x[time_index];
            time_index++;
        }
    }
...
bool DistanceApproachIPOPTCUDAInterface::eval_g(int n, const double* x, bool new_x, int m, double* g) {
...
#pragma omp parallel for schedule(dynamic, 1) num_threads(4)
        ...
        for (int i = 0; i < horizon_; ++i) {
            ...
            values[nz_index] = x[time_index] * ts_
                    * (x[state_index + 3] + x[time_index] * ts_ * 0.5 * x[control_index + 1])
                    * std::sin(x[state_index + 2]
                               + x[time_index] * ts_ * 0.5 * x[state_index + 3] * std::tan(x[control_index])
                                       / wheelbase_);  // a.
            ++nz_index;
            ...
            values[nz_index] = -1.0
                    * (x[time_index] * ts_
                               * std::cos(
                                       ...
                                       + x[time_index] * ts_ * 0.5 * x[state_index + 3] * std::tan(x[control_index])
                                               / wheelbase_)
                       + x[time_index] * ts_ * (x[state_index + 3] + x[time_index] * ts_ * 0.5 * x[control_index + 1])
                               * (-1) * x[time_index] * ts_ * 0.5 * std::tan(x[control_index]) / wheelbase_
                               * std::sin(
                                       x[state_index + 2]
                                       + x[time_index] * ts_ * 0.5 * x[state_index + 3] * std::tan(x[control_index])
                                               / wheelbase_));  // b
            ...
            values[nz_index] = -1.0 * x[time_index] * ts_ * std::tan(x[control_index]) / wheelbase_;  // k.
            ++nz_index;
            ...
            values[nz_index] = -1.0
                    * (x[time_index] * ts_ * x[time_index] * ts_ * 0.5 * std::tan(x[control_index])
                       / wheelbase_);  // m.
            ++nz_index;
            ...
            values[nz_index] = -1.0
                    * (ts_ * (x[state_index + 3] + x[time_index] * ts_ * 0.5 * x[control_index + 1])
                       ...
                       + x[time_index] * ts_ * ts_ * 0.5 * x[control_index + 1] * std::tan(x[control_index])
                               / wheelbase_);  // n.
            ++nz_index;
        ...
        // with respect to time
        values[nz_index] = -1.0 * (x[control_index] - last_time_u_(0, 0)) / x[time_index] / x[time_index] / ts_;
        ++nz_index;
        time_index++;
        control_index += 2;
        ...
        for (int i = 1; i < horizon_; ++i) {
            ...
            // with respect to time
            values[nz_index] = -1.0 * (x[control_index] - x[control_index - 2]) / x[time_index] / x[time_index] / ts_;
            ++nz_index;
        ...
        ADEBUG << "After fulfilled time constraints derivative, nz_index : " << nz_index << " nele_jac : " << nele_jac;
...
#pragma omp parallel for schedule(dynamic, 1) num_threads(4)
        for (int iter = 0; iter < (horizon_ + 1) * obstacles_num_; iter++) {
            ...
            // with respect to l
            for (int k = 0; k < current_edges_num; ++k) {
                values[nz_index_tmp] = (x[state_index_tmp] + std::cos(x[state_index_tmp + 2]) * offset_) * Aj(k, 0)
    ...
    if (values == nullptr) {
        ...
        for (int i = 0; i < horizon_ + 1; ++i) {
            ...
            for (int j = 0; j < obstacles_num_; ++j) {
                ...
                // 2. G' * mu + R' * lambda == 0, part 1
                // With respect to x
                values[nz_index] = -std::sin(x[state_index + 2]) * tmp1 + std::cos(x[state_index + 2]) * tmp2;  // u
                ++nz_index;
                ...
                // 3. G' * mu + R' * lambda == 0, part 2
                // With respect to x
                values[nz_index] = -std::cos(x[state_index + 2]) * tmp1 - std::sin(x[state_index + 2]) * tmp2;  // x
                ++nz_index;
                ...
                // with respect to l
                for (int k = 0; k < current_edges_num; ++k) {
                    values[nz_index] = (x[state_index] + std::cos(x[state_index + 2]) * offset_) * Aj(k, 0)
...
bool DistanceApproachIPOPTCUDAInterface::eval_h(
        int n,
        const double* x,
        bool new_x,
        double obj_factor,
...
bool DistanceApproachIPOPTCUDAInterface::eval_obj(int n, const T* x, T* obj_value) {
    ...
    *obj_value += weight_stitching_steer_ * last_time_steer_rate * last_time_steer_rate
    ...
    for (int i = 0; i < horizon_ - 1; ++i) {
        ...
        *obj_value += weight_rate_steer_ * steering_rate * steering_rate + weight_rate_a_ * a_rate * a_rate;
        control_index += 2;
        time_index++;
    }
...
bool DistanceApproachIPOPTCUDAInterface::eval_constraints(int n, const T* x, int m, T* g) {
    ...
    for (int i = 0; i < horizon_; ++i) {
        ...
        g[constraint_index] = x[state_index + 4]
                - (x[state_index]
                   + ts_ * x[time_index] * (x[state_index + 3] + ts_ * x[time_index] * 0.5 * x[control_index + 1])
                           * cos(x[state_index + 2]
                                 + ts_ * x[time_index] * 0.5 * x[state_index + 3] * tan(x[control_index])
                                         / wheelbase_));
        // x2
        g[constraint_index + 1] = x[state_index + 5]
                - (x[state_index + 1]
                   + ts_ * x[time_index] * (x[state_index + 3] + ts_ * x[time_index] * 0.5 * x[control_index + 1])
                           * sin(x[state_index + 2]
                                 + ts_ * x[time_index] * 0.5 * x[state_index + 3] * tan(x[control_index])
        ...
        // x3
        g[constraint_index + 2] = x[state_index + 6]
                - (x[state_index + 2]
                   + ts_ * x[time_index] * (x[state_index + 3] + ts_ * x[time_index] * 0.5 * x[control_index + 1])
    ...
    for (int i = 0; i < horizon_ + 1; ++i) {
        ...
        for (int j = 0; j < obstacles_num_; ++j) {
            ...
            g[constraint_index + 1]
                    = x[n_index] - x[n_index + 2] + cos(x[state_index + 2]) * tmp1 + sin(x[state_index + 2]) * tmp2;

            g[constraint_index + 2]
                    = x[n_index + 1] - x[n_index + 3] - sin(x[state_index + 2]) * tmp1 + cos(x[state_index + 2]) * tmp2;
            ...
            g[constraint_index + 3] = tmp3 + (x[state_index] + cos(x[state_index + 2]) * offset_) * tmp1
                    + (x[state_index + 1] + sin(x[state_index + 2]) * offset_) * tmp2 - tmp4;
...
bool DistanceApproachIPOPTCUDAInterface::check_g(int n, const double* x, int m, const double* g) {
    ...
    for (int idx = 0; idx < m; ++idx) {
        if (g[idx] > g_u_tmp[idx] + delta_v || g[idx] < g_l_tmp[idx] - delta_v) {
            ADEBUG << "constratins idx unfeasible: " << idx << ", g: " << g[idx] << ", lower: " << g_l_tmp[idx]
                   << ", upper: " << g_u_tmp[idx];
        }
    }
    return true;
