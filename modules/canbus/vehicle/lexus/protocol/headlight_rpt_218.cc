...
void Headlightrpt218::Parse(const std::uint8_t* bytes, int32_t length,
  ...
  chassis->mutable_lexus()->mutable_headlight_rpt_218()->set_vehicle_fault(vehicle_fault(bytes, length));
  chassis->mutable_lexus()->mutable_headlight_rpt_218()->set_pacmod_fault(pacmod_fault(bytes, length));
  chassis->mutable_lexus()->mutable_headlight_rpt_218()->set_override_active(override_active(bytes, length));
  chassis->mutable_lexus()->mutable_headlight_rpt_218()->set_output_reported_fault(output_reported_fault(bytes, length));
  chassis->mutable_lexus()->mutable_headlight_rpt_218()->set_input_output_fault(input_output_fault(bytes, length));
  chassis->mutable_lexus()->mutable_headlight_rpt_218()->set_enabled(enabled(bytes, length));
  chassis->mutable_lexus()->mutable_headlight_rpt_218()->set_command_output_fault(command_output_fault(bytes, length));
  chassis->mutable_lexus()->mutable_headlight_rpt_218()->set_output_value(output_value(bytes, length));
  chassis->mutable_lexus()->mutable_headlight_rpt_218()->set_manual_input(manual_input(bytes, length));
  chassis->mutable_lexus()->mutable_headlight_rpt_218()->set_commanded_value(commanded_value(bytes, length));
}
...
// config detail: {'name': 'output_value', 'enum': {0: 'OUTPUT_VALUE_HEADLIGHTS_OFF', 1: 'OUTPUT_VALUE_LOW_BEAMS', 2: 'OUTPUT_VALUE_HIGH_BEAMS'}, 'precision': 1.0, 'len': 8, 'is_signed_var': False, 'offset': 0.0, 'physical_range': '[0|2]', 'bit': 31, 'type': 'enum', 'order': 'motorola', 'physical_unit': ''}
Headlight_rpt_218::Output_valueType Headlightrpt218::output_value(const std::uint8_t* bytes, int32_t length) const {
  Byte t0(bytes + 3);
...
// config detail: {'name': 'manual_input', 'enum': {0: 'MANUAL_INPUT_HEADLIGHTS_OFF', 1: 'MANUAL_INPUT_LOW_BEAMS', 2: 'MANUAL_INPUT_HIGH_BEAMS'}, 'precision': 1.0, 'len': 8, 'is_signed_var': False, 'offset': 0.0, 'physical_range': '[0|2]', 'bit': 15, 'type': 'enum', 'order': 'motorola', 'physical_unit': ''}
Headlight_rpt_218::Manual_inputType Headlightrpt218::manual_input(const std::uint8_t* bytes, int32_t length) const {
  Byte t0(bytes + 1);
  int32_t x = t0.get_byte(0, 8);
...
// config detail: {'name': 'commanded_value', 'enum': {0: 'COMMANDED_VALUE_HEADLIGHTS_OFF', 1: 'COMMANDED_VALUE_LOW_BEAMS', 2: 'COMMANDED_VALUE_HIGH_BEAMS'}, 'precision': 1.0, 'len': 8, 'is_signed_var': False, 'offset': 0.0, 'physical_range': '[0|2]', 'bit': 23, 'type': 'enum', 'order': 'motorola', 'physical_unit': ''}
Headlight_rpt_218::Commanded_valueType Headlightrpt218::commanded_value(const std::uint8_t* bytes, int32_t length) const {
  Byte t0(bytes + 2);
  int32_t x = t0.get_byte(0, 8);

  Headlight_rpt_218::Commanded_valueType ret =  static_cast<Headlight_rpt_218::Commanded_valueType>(x);
  return ret;
}
}  // namespace lexus
}  // namespace canbus
