...
public:
    ...
    /**@brief Construct a map node index, given a global coordinate, eigen version. */
    static MapNodeIndex get_map_node_index(const BaseMapConfig& option, const Eigen::Vector3d& coordinate,
                                           unsigned int resolution_id, int zone_id);
    static MapNodeIndex get_map_node_index(const BaseMapConfig& option, const Eigen::Vector2d& coordinate,
                                           unsigned int resolution_id, int zone_id);

    /**@brief Get the index range (maximum possible index + 1) in the east direction. */
    static unsigned int get_map_index_range_east(const BaseMapConfig& option, 
        unsigned int resolution_id);
    /**@brief Get the index range (maximum possible index + 1) in the north direction. */
