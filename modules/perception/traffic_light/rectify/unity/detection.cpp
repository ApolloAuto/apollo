...
void Detection::perform(const cv::Mat &ros_image, std::vector<LightPtr> *lights) {
  cv::Mat crop_image = ros_image(_crop_box);
  ...
  float col_shrink = float(_min_crop_size) / float(crop_image.cols);
  float row_shrink = float(_min_crop_size) / float(crop_image.rows);
  ...
  switch (_detect_output_type) {
    case DetectOutputBoxType::BOX_ALL:    // day and night
      select_output_bboxes(crop_image, 0, inflate_col, inflate_row, lights);
      select_output_bboxes(crop_image, 1, inflate_col, inflate_row, lights);
      break;
    ...
    case DetectOutputBoxType::BOX_QUADRATE:    // night
      ...
      break;
    default:
      AINFO << "DenseBoxDetection::perform get unknown detetion output box type: "
            << _detect_output_type;
      return;
  }
...
void Detection::init(int &min_crop_size, const std::string &refine_net,
...
bool Detection::set_output_box_type(DetectOutputBoxType type) {
  ...
  _detect_output_type = type;

  return true;
}

bool Detection::select_output_bboxes(const cv::Mat &crop_image,
  ...
  for (int candidate_id = 0; candidate_id < result_bbox.size(); candidate_id++) {
    ...
    if (!box_is_valid(tmp->region.rectified_roi, crop_image.size())) {
    ...
    tmp->region.rectified_roi = refined_box(tmp->region.rectified_roi, crop_image.size());
...
void Detection::set_crop_box(const cv::Rect &box) {
  _crop_box = box;
}
