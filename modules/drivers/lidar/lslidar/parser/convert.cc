...
void Convert::ConvertPacketsToPointcloud(
	...
	AINFO << "Convert scan msg seq " << scan_msg->header().sequence_num();
  //AERROR << "Convert scan msg seq " << scan_msg->header().sequence_num();

	parser_->GeneratePointcloud(scan_msg, point_cloud);
	//AERROR << "line: " << __LINE__ << "point_cloud->width: " << point_cloud->width();
	//AERROR << "line: " << __LINE__ << "point_cloud->height: " << point_cloud->height();
