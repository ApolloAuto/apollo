...
bool MotionUnet::Init(const PointCloudParserInitOptions& options) {
    ...
    z_offset_ = model_param_.z_offset();
    ...
    motion_class_mapping_.push_back(static_cast<uint8_t>(PointMotionLabel::STATIONARY));
    motion_class_mapping_.push_back(static_cast<uint8_t>(PointMotionLabel::STATIONARY));
    motion_class_mapping_.push_back(static_cast<uint8_t>(PointMotionLabel::MOVING));
    motion_class_mapping_.push_back(static_cast<uint8_t>(PointMotionLabel::UNKNOWN));
    CHECK_EQ(motion_class_mapping_.size(), 4) << "class num and motion map size not match";

    return true;
}

bool MotionUnet::Parse(const PointCloudParserOptions& options, LidarFrame* frame) {
    // check input
    if (frame == nullptr) {
        AERROR << "Input null frame ptr.";
        return false;
    }
    if (frame->cloud == nullptr || frame->world_cloud == nullptr) {
    ...
    while (!prev_world_clouds_.empty()
           && timestamp - prev_world_clouds_.front().first > model_param_.fuse_time_interval()) {
        prev_world_clouds_.pop_front();
    }

    // check cached pointcloud
    if (prev_world_clouds_.size() == 0) {
        AINFO << "No cached point cloud available, skip.";
        prev_world_clouds_.push_back(std::make_pair(timestamp, frame->world_cloud));
        return false;
    }
    ...
    base::PointFCloudPtr prev_cloud(new base::PointFCloud());
    TransformCloud(prev_cloud, prev_world_clouds_.front().second);

    Timer timer;

    GeneratePfnFeature(prev_cloud);
    pfn_inference_->Infer();
    GenerateBackboneFeature(prev_bev_feature_);
    double prev_pfn_feature_time = timer.toc(true);

    GeneratePfnFeature(original_cloud_);
    pfn_inference_->Infer();
    GenerateBackboneFeature(bev_feature_);
    double cur_pfn_feature_time = timer.toc(true);

    seg_pred_blob_->Reshape({max_point_number_, 4});
    inference_->Infer();
    ...
    AINFO << "MotionNet time: "
          << "previous pfn feature time: " << prev_pfn_feature_time << "ms, "
          << "current pfn feature time: " << cur_pfn_feature_time << "ms, "
...
bool MotionUnet::InitModel() {
    ...
    if (input_names.size() == 4) {
        ...
        prev_bev_feature_ = inference_->get_blob(input_names.at(3));

        CHECK_NOTNULL(coors_.get());
        CHECK_NOTNULL(point_feats_.get());
        CHECK_NOTNULL(bev_feature_.get());
        CHECK_NOTNULL(prev_bev_feature_.get());
    } else {
...
void MotionUnet::GeneratePfnFeatureCPU(base::PointFCloudPtr cloud) {
    // clear unet nodes
    unet_nodes_.clear();
    std::vector<Unode>().swap(unet_nodes_);
    unet_nodes_.resize(unet_width_ * unet_length_);
    ...
    for (size_t i = 0; i < cloud->size(); i++) {
        auto pt = cloud->at(i);
        // check valid range
        if (pt.x < min_x_ || pt.x > max_x_ || pt.y < min_y_ || pt.y > max_y_ || pt.z + z_offset_ < min_height_
            || pt.z + z_offset_ > max_height_) {
            continue;
        }
    ...
    for (size_t i = 0; i < valid_point_index_.size(); i++) {
        auto pt = cloud->at(valid_point_index_.at(i));
        Unode node = unet_nodes_.at(valid_node_index_.at(i));
        // generate voxels feature
        voxel_cpu_data[i * 10 + 0] = pt.x;
        voxel_cpu_data[i * 10 + 1] = pt.y;
        voxel_cpu_data[i * 10 + 2] = pt.z + z_offset_;
        voxel_cpu_data[i * 10 + 3] = pt.intensity / 255.;
        ...
        voxel_cpu_data[i * 10 + 9] = pt.z + z_offset_ - node.geometric_z;
...
void MotionUnet::GenerateBackboneFeatureCPU(std::shared_ptr<base::Blob<float>> feature) {
    ...
    auto feature_data = feature->mutable_cpu_data();
    memset(feature_data, 0, sizeof(float) * 64 * unet_length_ * unet_width_);

    for (size_t i = 0; i < valid_node_index_.size(); i++) {
        int pillar_idx = valid_node_index_.at(i);
        Unode node = unet_nodes_.at(pillar_idx);
        // voxel_features_
        for (int j = 0; j < 64; j++) {
            int index = j * unet_length_ * unet_width_ + node.grid_y * unet_length_ + node.grid_x;
            float value = feature_data[index];
            float max_val = point_feats_cpu[i * 64 + j];
            max_val = std::max(max_val, value);
            feature_data[index] = max_val;
        }
    }
}

void MotionUnet::LabelToPointCloud(LidarFrame* frame) {
    // check input
    ...
    for (size_t i = 0; i < valid_point_index_.size(); i++) {
        ...
        for (int n = 0; n < 4; n++) {
            float prob = sematic_result[i * 4 + n];
            if (max_prob < prob) {
                max_prob = prob;
                max_index = n;
            }
        ...
        SetMotionLabel(
                static_cast<PointMotionLabel>(motion_class_mapping_[max_index]),
...
void MotionUnet::TransformCloud(
    base::PointFCloudPtr &cloud_ptr,
    const base::PointDCloudPtr &world_cloud_ptr) {
    cloud_ptr->clear();
    cloud_ptr->reserve(world_cloud_ptr->size());
    for (size_t i = 0; i < world_cloud_ptr->size(); ++i) {
        auto &point = world_cloud_ptr->at(i);
...
PERCEPTION_REGISTER_POINTCLOUDPARSER(MotionUnet);
