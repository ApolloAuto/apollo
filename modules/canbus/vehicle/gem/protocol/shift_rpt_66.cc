...
void Shiftrpt66::Parse(const std::uint8_t* bytes, int32_t length,
  ...
  chassis->mutable_gem()->mutable_shift_rpt_66()->set_commanded_value(commanded_value(bytes, length));
...
Shift_rpt_66::Manual_inputType Shiftrpt66::manual_input(const std::uint8_t* bytes, int32_t length) const {
  Byte t0(bytes + 0);
...
Shift_rpt_66::Commanded_valueType Shiftrpt66::commanded_value(const std::uint8_t* bytes, int32_t length) const {
  Byte t0(bytes + 1);
  int32_t x = t0.get_byte(0, 8);
...
Shift_rpt_66::Output_valueType Shiftrpt66::output_value(const std::uint8_t* bytes, int32_t length) const {
  Byte t0(bytes + 2);
  int32_t x = t0.get_byte(0, 8);
