 ...
 * @file
 */
#include "modules/planning/planning_open_space/trajectory_smoother/distance_approach_ipopt_relax_end_interface.h"
...
bool DistanceApproachIPOPTRelaxEndInterface::eval_f(int n, const double* x, bool new_x, double& obj_value) {
    eval_obj(n, x, &obj_value);
    return true;
}

bool DistanceApproachIPOPTRelaxEndInterface::eval_grad_f(int n, const double* x, bool new_x, double* grad_f) {
    gradient(tag_f, n, x, grad_f);
    return true;
}

bool DistanceApproachIPOPTRelaxEndInterface::eval_g(int n, const double* x, bool new_x, int m, double* g) {
...
bool DistanceApproachIPOPTRelaxEndInterface::check_g(int n, const double* x, int m, const double* g) {
    int kN = n;
    int kM = m;
    double x_u_tmp[kN];
    double x_l_tmp[kN];
    ...
    for (int idx = 0; idx < m; ++idx) {
        if (g[idx] > g_u_tmp[idx] + delta_v || g[idx] < g_l_tmp[idx] - delta_v) {
            AINFO << "constratins idx unfeasible: " << idx << ", g: " << g[idx] << ", lower: " << g_l_tmp[idx]
                  << ", upper: " << g_u_tmp[idx];
        }
    }
    return true;
...
void DistanceApproachIPOPTRelaxEndInterface::generate_tapes(int n, int m, int* nnz_jac_g, int* nnz_h_lag) {
