...
def _find_hipblas_config(rocm_install_path):

  def hipblas_version_numbers(path):
    version_file = os.path.join(path, "hipblas/include/hipblas-version.h")
    if not os.path.exists(version_file):
      raise ConfigError(
  ...
  major, minor, patch = hipblas_version_numbers(rocm_install_path)
...
def find_rocm_config():
  ...
  result.update(_find_hipblas_config(rocm_install_path))
