...
class ChannelVerify {
 ...
 private:
  bool is_record_file(const std::string& path) const;
  std::shared_ptr<CyberRecordInfo> get_record_info(
      const std::string& record_path) const;
  int incremental_check(const std::vector<std::string>& records_path);
  std::vector<std::string> get_records_path(
      const std::string& record_dir_or_record_full_path) const;
  bool is_record_checked(const std::string& record_path);
  OneRecordChannelCheckResult check_record_channels(
