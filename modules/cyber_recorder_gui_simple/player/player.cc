...
#include "modules/cyber_recorder_gui_simple/player/player.h"
...
Player::Player(const PlayParam& play_param, const NodePtr& node, const bool preload_fill_buffer_mode) :
        ...
        last_played_msg_real_time_s_(0.0),
        progress_time_s_(0.0),
        total_progress_time_s_(0.0) {
...
bool Player::ThreadFunc_Play_Nohup() {
    ...
    total_progress_time_s_.store(total_progress_time_s);
    AINFO << "total_progress_time_s_: " << total_progress_time_s
           << ", play_param.end_time_ns: " << play_param.end_time_ns
           << ", play_param.begin_time_ns: " << play_param.begin_time_ns;
    while (!is_stopped_.load() && apollo::cyber::OK()) {
        ...
        if (consumer_->last_played_msg_real_time_ns() > 0) {
            progress_time_s += static_cast<double>(
                                        consumer_->last_played_msg_real_time_ns() - consumer_->base_msg_play_time_ns()
                                        + consumer_->base_msg_real_time_ns() - producer_->play_param().begin_time_ns)
                    / 1e9;
        }
        progress_time_s_.store(progress_time_s);
...
bool Player::Start() {
    ...
    if (!is_stopped_.load()) {
        AERROR << "player has been started.";
        return false;
    }
    if (nohup_play_th_ != nullptr && nohup_play_th_->joinable()) {
        nohup_play_th_->join();
    ...
    // // std::cout << std::fixed;
    // total_progress_time_s_ = static_cast<double>(play_param.end_time_ns - play_param.begin_time_ns) / 1e9
    //         + static_cast<double>(play_param.start_time_s);
    ...
    //     } else {
    //         consumer_->Continue();
    //         // std::cout << "\r[RUNNING] Current Record Time : ";
    //     }

    //     last_played_msg_real_time_s_ = static_cast<double>(consumer_->last_played_msg_real_time_ns()) / 1e9;

    //     progress_time_s_ = static_cast<double>(producer_->play_param().start_time_s);
    //     if (consumer_->last_played_msg_real_time_ns() > 0) {
    //         progress_time_s_ += static_cast<double>(
    ...
    //     // std::cout << std::setprecision(3) << last_played_msg_real_time_s_ << "    Progress: " << progress_time_s_
...
bool Player::Stop() {
    const bool was_stopped = is_stopped_.exchange(true);
    producer_->Stop();
    consumer_->Stop();
...
bool Player::ResetProcessTime(double process_time) {
    Reset();
