...
void LoopsVerifyAgent::CheckVerify(LoopsVerifyRequest *request,
  ...
  if (sp_laps_checker_->get_return_state() != 0) {
    response->set_progress(0.0);
    response->set_code(sp_laps_checker_->get_return_state());
    return;
  }

  double progress = sp_laps_checker_->get_progress();
  response->set_progress(progress);
  response->set_code(ErrorCode::SUCCESS);
  if (std::abs(1.0 - progress) < 1e-8) {
    double confidence = sp_laps_checker_->get_confidence();
    size_t lap = sp_laps_checker_->get_lap();
    AINFO << "aquired lap: " << lap << ", conf: " << confidence;
    apollo::hdmap::LoopResult *loop_result = response->mutable_loop_result();
...
void LoopsVerifyAgent::StopVerify(LoopsVerifyRequest *request,
  ...
  response->set_progress(sp_laps_checker_->get_progress());
  if (std::abs(1.0 - sp_laps_checker_->get_progress()) < 1e-8) {
    double conf = sp_laps_checker_->get_confidence();
    size_t lap = sp_laps_checker_->get_lap();
...
LoopsVerifyAgent::get_verify_range(LoopsVerifyRequest *request) {
  ...
  for (int i = 0; i < range_size; ++i) {
    const ::apollo::hdmap::VerifyRange &range = request->range(i);
    sp_range->push_back(std::make_pair(range.start_time(), range.end_time()));
  }
  return sp_range;
}

size_t LoopsVerifyAgent::get_loops_to_check(LoopsVerifyRequest *request) {
...
int LoopsVerifyAgent::get_poses_to_check(
  ...
  std::shared_ptr<std::vector<FramePose>> sp_vec_poses =
      sp_pose_collection_agent_->get_poses();
...
LoopsVerifyAgentState LoopsVerifyAgent::get_state() { return state_; }
