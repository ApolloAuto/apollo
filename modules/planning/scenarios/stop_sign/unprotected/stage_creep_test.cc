...
TEST_F(StopSignUnprotectedStageCreepTest, Init) {
  StopSignUnprotectedStageCreep stop_sign_unprotected_stage_creep(
      stop_sign_unprotected_config_.stage_config(2));
  EXPECT_EQ(stop_sign_unprotected_stage_creep.Name(),
      ScenarioConfig::StageType_Name(
      stop_sign_unprotected_config_.stage_config(2).stage_type()));
}
