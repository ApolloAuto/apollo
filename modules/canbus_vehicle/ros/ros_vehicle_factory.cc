...
bool RosVehicleFactory::Init(const CanbusConf *canbus_conf) {
  // Init can client
  ...
  chassis_detail_writer_ =
      node_->CreateWriter<::apollo::canbus::Ros>(FLAGS_chassis_detail_topic);

  chassis_detail_sender_writer_ = node_->CreateWriter<::apollo::canbus::Ros>(
      FLAGS_chassis_detail_sender_topic);

  return true;
}

bool RosVehicleFactory::Start() {
...
void RosVehicleFactory::UpdateCommand(
...
Chassis RosVehicleFactory::publish_chassis() {
  Chassis chassis = vehicle_controller_->chassis();
  ADEBUG << chassis.ShortDebugString();
  return chassis;
}

void RosVehicleFactory::PublishChassisDetail() {
  Ros chassis_detail = vehicle_controller_->GetNewRecvChassisDetail();
  ADEBUG << "latest chassis_detail is " << chassis_detail.ShortDebugString();
  chassis_detail_writer_->Write(chassis_detail);
}

void RosVehicleFactory::PublishChassisDetailSender() {
  Ros sender_chassis_detail =
...
void RosVehicleFactory::UpdateHeartbeat() { can_sender_.Update_Heartbeat(); }

bool RosVehicleFactory::CheckChassisCommunicationFault() {
...
void RosVehicleFactory::AddSendProtocol() {
  vehicle_controller_->AddSendMessage();
}

void RosVehicleFactory::ClearSendProtocol() { can_sender_.ClearMessage(); }

bool RosVehicleFactory::IsSendProtocolClear() {
...
Chassis::DrivingMode RosVehicleFactory::Driving_Mode() {
...
RosVehicleFactory::CreateVehicleController() {
  return std::unique_ptr<VehicleController<::apollo::canbus::Ros>>(
      new ros::RosController());
}

std::unique_ptr<MessageManager<::apollo::canbus::Ros>>
RosVehicleFactory::CreateMessageManager() {
  return std::unique_ptr<MessageManager<::apollo::canbus::Ros>>(
