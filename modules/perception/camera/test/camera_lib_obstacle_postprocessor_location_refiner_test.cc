...
static bool load_camera_k_mat_and_ground_pitch_height(const std::string &fname,
                                                      ...
                                                      float *camera_pitch,
                                                      float *camera_height) {
  if (image_width == nullptr
      || image_height == nullptr
      || camera_pitch == nullptr
...
static bool load_label_from_file(const std::string &filename,
  ...
  ObjectInfo label_in;
  while (fin >> label_in.type
             >> label_in.truncated
             >> label_in.occluded
             >> label_in.alpha
             >> label_in.bbox[0] >> label_in.bbox[1]
             >> label_in.bbox[2] >> label_in.bbox[3]
             >> label_in.size_hwl[0] >> label_in.size_hwl[1]
             >> label_in.size_hwl[2]
             >> label_in.location[0] >> label_in.location[1]
             >> label_in.location[2]
             >> label_in.rotation_y
             >> label_in.score) {
...
TEST(LocationRefinedObstaclePostProcessorTestOnlineCalibration,
  ...
  load_camera_k_mat_and_ground_pitch_height(fname_k_mat,
                                            k_mat,
                                            &width,
                                            &height,
                                            &camera_pitch,
  ...
  BaseObstaclePostprocessor *postprocessor
      = BaseObstaclePostprocessorRegisterer::
      GetInstanceByName("LocationRefinerObstaclePostprocessor");
...
//  frame.detected_objects.clear();
  ...
  for (auto &obj : detections) {
    if (std::find(candidate_type_list.begin(),
  ...
  intrinsic << k_mat[0] , k_mat[1] , k_mat[2],
               k_mat[3],  k_mat[4],  k_mat[5],
               k_mat[6],  k_mat[7],  k_mat[8];
