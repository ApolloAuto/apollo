...
//		packet_end_time = gps_base_usec_ * 1000 - (packets_size - num - 1)*1282*POINTS_PER_PACKET;
//  else
...
//  last_packet_time = packet_end_time;
  ...
  for (int i = 0; i < 128; i++) {
	if (abs(prism_angle128[0]) < 1e-6 && abs(prism_angle128[1]) < 1e-6 && abs(prism_angle128[2]) < 1e-6 &&
		abs(prism_angle128[3]) < 1e-6) {
  ...
  for (int point_idx1 = 0; point_idx1 < POINTS_PER_PACKET; point_idx1++) 
  {
	 firings[point_idx1].vertical_line=raw->points[point_idx1].vertical_line;
	  ...
	  firings[point_idx1].azimuth=static_cast<double>(point_amuzith.uint*0.01*DEG_TO_RAD);
	  ...
	  firings[point_idx1].distance=static_cast<double>(point_distance.uint) * DISTANCE_RESOLUTION2;     
	...
	for (int point_idx = 0; point_idx < POINTS_PER_PACKET; point_idx++) 
	{
		...
		z_sin_altitude = sin_scan_laser_altitude_ch128[firings[point_idx].vertical_line / 4] +
		...
		uint64_t point_time = packet_end_time - 1562.5*((POINTS_PER_PACKET - point_idx)/7 - 1);
		...
		if (config_.calibration()) {
			...
			ComputeCoords2(firings[point_idx].vertical_line, CH128, firings[point_idx].distance, corrections,
