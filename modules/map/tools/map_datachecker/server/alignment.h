...
class Alignment {
 public:
  ...
  virtual double get_progress() const { return progress_; }

  virtual void set_start_time(double start_time) { start_time_ = start_time; }

  virtual void set_end_time(double end_time) { end_time_ = end_time; }

  virtual void update_bad_pose_info(const FramePose& pose) {
    update_pose_info(pose, sp_bad_pose_info_);
  }

  virtual void clear_bad_pose_info() { clear_pose_info(sp_bad_pose_info_); }

  virtual void update_good_pose_info(const FramePose& pose) {
    update_pose_info(pose, sp_good_pose_info_);
  }

  virtual void clear_good_pose_info() { clear_pose_info(sp_good_pose_info_); }
  ...
  ErrorCode get_return_state() const { return return_state_; }
 ...
 protected:
  ...
  int time_to_index(const std::vector<FramePose>& poses, double time) {
