...
bool LoadTypes(const std::string &path,
  ...
  while (ifs >> type) {
    if (base::kName2SubTypeMap.find(type) ==
        base::kName2SubTypeMap.end()) {
...
void FillObjectPolygonFromBBox3D(base::Object* object_ptr) {
  ...
  double y2 = -y1;
  double len =
      sqrt(object_ptr->direction[0] * object_ptr->direction[0] +
  ...
  object_ptr->polygon[0].x = x1 * cos_theta + y1 * sin_theta +
      object_ptr->center[0];
  object_ptr->polygon[0].y = y1 * cos_theta - x1 * sin_theta +
      object_ptr->center[1];
  ...
  object_ptr->polygon[2].x = x2 * cos_theta + y2 * sin_theta +
      object_ptr->center[0];
  object_ptr->polygon[2].y = y2 * cos_theta - x2 * sin_theta +
      object_ptr->center[1];
