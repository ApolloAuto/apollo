...
#include "modules/perception/traffic_light/base/utils.h" DEFINE_double(light_height_adjust, 0,
                            ...
                            bool BoundaryProjection::Project(
                              ...
                              if (bound_size < 4) {
                                AERROR << "Light boundary should be rectangle, "
                                          "which has "
                                          "four points! Got :"
                                       << bound_size;
                                return false;
                              ...
                              for (int i = 0; i < bound_size; ++i) {
                                if (!ProjectPointDistort(
                                        camera_coeffient, pose,
                                        tl_info.boundary().point(i), &x[i],
                                        &y[i])) {
                                  return false;
                                }
                              }
                              ...
                              cv::Rect roi(minx, miny, maxx - minx,
                                           maxy - miny);
                              AINFO << "projection get ROI:" << roi;
                              if (minx < 0 || miny < 0 ||
                                  maxx >= static_cast<int>(
                                              camera_coeffient.image_width) ||
                                  maxy >= static_cast<int>(
                                              camera_coeffient.image_height)) {
                                AWARN
                                    << "Projection get ROI outside the image. ";
                                return false;
                              }
                              light->region.projection_roi = RefinedBox(
                                  roi, cv::Size(camera_coeffient.image_width,
                                                camera_coeffient.image_height));
                              AINFO << "refined ROI:"
                            ...
                            bool BoundaryProjection::ProjectPoint(
                              ...
                              TL_loc_LTM << point.x(), point.y(),
                                  point.z() + FLAGS_light_height_adjust, 1.0;
                              TL_loc_LTM = coeffient.camera_extrinsic *
                                           pose.inverse() * TL_loc_LTM;

                              // The light may behind the car, we can't project
                              // them on the
                              // images.
                              if (TL_loc_LTM(2) < 0) {
                                AWARN << "Compute a light behind the car. "
                                         "light to car Pose:\n"
                                      << TL_loc_LTM;
                                return false;
                              }
                              TL_loc_cam =
                                  coeffient.camera_intrinsic * TL_loc_LTM;
                            ...
                            bool BoundaryProjection::ProjectPointDistort(
                                const CameraCoeffient &coeffient,
                                const Eigen::Matrix4d &pose,
                                const common::PointENU &point, int *center_x,
                                int *center_y) const {
                              Eigen::Matrix<double, 4, 1> TL_loc_LTM;
                              Eigen::Matrix<double, 3, 1> TL_loc_cam;
                              ...
                              pt2d = PixelDenormalize(
                                  pt2d, coeffient.camera_intrinsic,
                            ...
                            BoundaryProjection::PixelDenormalize(
                                const Eigen::Matrix<double, 2, 1> &pt2d,
                                const Eigen::Matrix<double, 3, 4>
                                    &camera_intrinsic,
                                const Eigen::Matrix<double, 5, 1>
                                    &distort_params) const {
                              // add distortion
                              double r_sq =
                                  pt2d[0] * pt2d[0] + pt2d[1] * pt2d[1];
                              Eigen::Matrix<double, 2, 1> pt2d_radial =
                                  pt2d *
                                  (1 + distort_params[0] * r_sq +
                                   distort_params[1] * r_sq * r_sq +
                                   distort_params[4] * r_sq * r_sq * r_sq);
                              Eigen::Matrix<double, 2, 1> dpt2d;
                              dpt2d[0] =
                                  2 * distort_params[2] * pt2d[0] * pt2d[1] +
                                  distort_params[3] *
                                      (r_sq + 2 * pt2d[0] * pt2d[0]);
                              dpt2d[1] =
                                  distort_params[2] *
                                      (r_sq + 2 * pt2d[1] * pt2d[1]) +
                              ...
                              pt[0] =
                                  pt2d_distort[0] * focal_length_x + center_x;
                              pt[1] =
                                  pt2d_distort[1] * focal_length_y + center_y;
