...
void Lslidar16Parser::GeneratePointcloud(
  ...
  //                double scan_altitude_real_value[16] =
  //                {-0.261799,0.0174533,-0.226893,0.0513127,-0.191986,0.084823,
...
void Lslidar16Parser::Unpack(const LslidarPacket &pkt,
  ...
  if (!config_.time_synchronization()) {
    //    packet_end_time =
    //    *(reinterpret_cast<uint64_t>(const_cast<uint64_t>(pkt.stamp())));
    ...
    for (int firing = 0, k = 0; firing < LSC16_FIRINGS_PER_BLOCK; ++firing) {
      for (int dsr = 0; dsr < LSC16_SCANS_PER_FIRING;
        ...
        if (2 == config_.return_mode()) {
          ...
          //			point_time = packet_end_time * 1000 - 100000 *
          //(5
          //- block/2) - 3125 * (31 - firing * 16 - dsr);
        } else {
          // modify
          if (previous_packet_time > 1e-6) {
            point_time = current_packet_time -
        ...
        //          if(2 == config_.return_mode()){
        //              point_time = packet_end_time * 1000 - 49152 * (5 -
        //              block/2) - 3125 * (31 - dsr);
        //          }else{
        //              point_time = packet_end_time * 1000 - 49152 * (11 -
        //              block) - 3125 * (31 - dsr);
        //          }
        ...
        if (distance2 > config_.max_range() ||
          ...
          if (intensity > 172)
            intensity =
                172;  // todo
                      // 如果不设置172，cyber_visualizer中显示会有问题，点云会显示不完整
          // 如果想要获取真实的强度值，则注释这一行
