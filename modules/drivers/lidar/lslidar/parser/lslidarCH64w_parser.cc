...
//    AERROR << "size:" << out_msg->point_size();
...
//		packet_end_time = gps_base_usec_ * 1000 - (packets_size - num - 1)*521*POINTS_PER_PACKET;   //1726
//  else
//		packet_end_time = (gps_base_usec_ - (gps_base_usec_ %1000000) + raw->time_stamp % 1000000)*1000;
//
  ...
  for (size_t point_idx = 0; point_idx < POINTS_PER_PACKET; point_idx++) 
  {
	...
	for (size_t point_idx = 0; point_idx < POINTS_PER_PACKET; point_idx++) 
	{
		...
		int line_num = firings[point_idx].vertical_line;
		 if (line_num / 4 % 2 == 0) {
            cos_xita = cos((firings[point_idx].azimuth * RAD_TO_DEG / 2.0 + 22.5) * M_PI / 180);
        }else{
           cos_xita = cos((-firings[point_idx].azimuth * RAD_TO_DEG / 2.0 + 112.5) * M_PI / 180);
        }
...
//		point_time = packet_end_time - 521*(POINTS_PER_PACKET - point_idx - 1);  //1726
//		if(time_last < point_time && time_last > 0){
//		  point_time = time_last + 521;         //1726
//		}
//		time_last = point_time;
		...
		if (config_.calibration()) {
			...
			ComputeCoords2(firings[point_idx].vertical_line, CH64w, firings[point_idx].distance, corrections,
