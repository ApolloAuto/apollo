...
class Decoder32 : public DecoderBase<vpoint> {
 public:
  Decoder32(const RS_Param &param);
...
template <typename vpoint>
int Decoder32<vpoint>::decodeMsopPkt(const uint8_t *pkt,
...
int32_t Decoder32<vpoint>::decodeDifopPkt(const uint8_t *pkt) {
  ...
  if (!(this->cali_data_flag_ & 0x2)) {
    ...
    p_pitch_cali = ((RS32_DifopPkt *)pkt)->pitch_cali;
    ...
    if (angle_flag) {
      ...
      const uint8_t *p_yaw_cali = ((RS32_DifopPkt *)pkt)->yaw_cali;
      for (int i = 0; i < 32; i++) {
