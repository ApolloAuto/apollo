...
//    // Change feature size due to disabling HoG in OpenCV
//    _y_f = create_gaussian_peak(cvFloor(_window_size / _cell_size),
//                                cvFloor(_window_size / _cell_size));

    _y_f = create_gaussian_peak(_window_size, _window_size);
    _cos_window = calculate_hann(_y_f.size());

    return true;
}

bool KCFComponents::get_feature(const cv::Mat &img, const cv::Rect &box,
...
//    // Disable for opencv version transition
//    std::vector<cv::Mat> feat = _hog_extractor.get_feature(box_img);
...
bool KCFComponents::detect(const Tracked &tracked_obj, const std::vector<cv::Mat> &z_f,
                           float &score) {

    cv::Mat k_f = gaussian_correlation(z_f, tracked_obj._x_f);

    cv::Mat response;
    cv::idft(complex_multiplication(tracked_obj._alpha_f, k_f),
             response, cv::DFT_SCALE | cv::DFT_REAL_OUTPUT);
...
cv::Mat KCFComponents::gaussian_correlation(std::vector<cv::Mat> xf, std::vector<cv::Mat> yf) {
    ...
    cv::exp((-1 / (_kernel_sigma * _kernel_sigma)) * max(0.0, (xx + yy - 2 * xy) / numel_xf), k);
...
cv::Mat KCFComponents::complex_multiplication(const cv::Mat &x1, const cv::Mat &x2) {
...
cv::Mat KCFComponents::create_gaussian_peak(int sizey, int sizex) {
    ...
    // Assume the tracking padding is still 2.5, which means the center of box is the focus
