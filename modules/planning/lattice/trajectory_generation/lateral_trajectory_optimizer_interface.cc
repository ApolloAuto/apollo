...
bool LateralTrajectoryOptimizerInterface::get_nlp_info(
  ...
  // variables
  n = static_cast<int>(num_of_variables_);

  // constraints
  m = static_cast<int>(num_of_constraints_);

  nnz_jac_g = 11 * (static_cast<int>(num_of_points_) - 1) + 3;

  // none zero hessian and lagrangian
  nnz_h_lag = static_cast<int>(num_of_variables_);
...
bool LateralTrajectoryOptimizerInterface::get_starting_point(
  ...
  CHECK_EQ(num_of_variables_, static_cast<size_t>(n));
  CHECK(init_x == true);
  CHECK(init_z == false);
...
bool LateralTrajectoryOptimizerInterface::eval_f(int n, const double* x,
  ...
  size_t offset_prime = num_of_points_;
  size_t offset_pprime = 2 * num_of_points_;
  for (size_t i = 0; i < num_of_points_; ++i) {
...
bool LateralTrajectoryOptimizerInterface::eval_g(int n, const double* x,
  ...
  size_t offset = 3 * (num_of_points_ - 1);
...
bool LateralTrajectoryOptimizerInterface::eval_jac_g(int n, const double* x,
                                                     ...
                                                     int* jCol,
                                                     double* values) {
  CHECK_EQ(size_t(n), num_of_variables_);
  CHECK_EQ(size_t(m), num_of_constraints_);

  int offset_prime = static_cast<int>(num_of_points_);
  int offset_pprime = 2 * static_cast<int>(num_of_points_);
  ...
  if (values == nullptr) {
    ...
    for (int variable_index = 0;
         variable_index + 1 < static_cast<int>(num_of_points_);
         ++variable_index) {
      // d_i''
    ...
    CHECK_EQ(nz_index, static_cast<size_t>(nele_jac));
    CHECK_EQ(constraint_index, static_cast<size_t>(m));
  } else {
    ...
    // d_i+1'' - d_i''
    for (size_t variable_index = 0; variable_index + 1 < num_of_points_;
         ++variable_index) {
      values[nz_index] = -1.0;
      ++nz_index;
...
bool LateralTrajectoryOptimizerInterface::eval_h(int n, const double* x,
                                                 ...
                                                 int* iRow, int* jCol,
                                                 double* values) {
  CHECK_EQ(num_of_variables_, static_cast<size_t>(nele_hess));
  if (values == nullptr) {
    for (size_t i = 0; i < num_of_variables_; ++i) {
      iRow[i] = jCol[i] = static_cast<int>(i);
    }
  } else {
    for (size_t i = 0; i < num_of_points_; ++i) {
    ...
    for (size_t i = num_of_points_; i < 2 * num_of_points_; ++i) {
      values[i] = 2.0 * w_d_prime_;
    }
    for (size_t i = 2 * num_of_points_; i < num_of_variables_; ++i) {
...
void LateralTrajectoryOptimizerInterface::finalize_solution(
  ...
  size_t offset_prime = num_of_points_;
  size_t offset_pprime = offset_prime + num_of_points_;
