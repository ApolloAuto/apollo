...
void PiecewiseJerkSpeedProblem::CalculateKernel(std::vector<c_float>* P_data,
  ...
  auto delta_t_square = delta_t_ * delta_t_;
  // x(i)''^2 * (w_ddx + 2 * w_dddx / delta_s^2)
  columns[2 * n].emplace_back(2 * n,
                              (weight_ddx_ + weight_dddx_ / delta_t_square) /
                                  (scale_factor_[2] * scale_factor_[2]));
  ++value_index;

  for (int i = 0; i < n - 1; ++i)
  {
    columns[2 * n + i + 1].emplace_back(2 * n + i, (-weight_dddx_ / delta_t_square) / 
                                                      (scale_factor_[2] * scale_factor_[2]));
    ++value_index;
  }
  ...
  for (int i = 1; i < n - 1; ++i) {
    columns[2 * n + i].emplace_back(
        2 * n + i, (weight_ddx_ + 2.0 * weight_dddx_ / delta_t_square) /
                       (scale_factor_[2] * scale_factor_[2]));
    ++value_index;
  }
  ...
  columns[3 * n - 1].emplace_back(
      3 * n - 1,
      (weight_ddx_ + weight_dddx_ / delta_t_square + weight_end_state_[2]) /
          (scale_factor_[2] * scale_factor_[2]));
  ++value_index;
