...
void Horncmd11c::Parse(const std::uint8_t* bytes, int32_t length,
  ...
  chassis->mutable_lexus()->mutable_horn_cmd_11c()->set_ignore_overrides(ignore_overrides(bytes, length));
  chassis->mutable_lexus()->mutable_horn_cmd_11c()->set_enable(enable(bytes, length));
  chassis->mutable_lexus()->mutable_horn_cmd_11c()->set_clear_override(clear_override(bytes, length));
  chassis->mutable_lexus()->mutable_horn_cmd_11c()->set_clear_faults(clear_faults(bytes, length));
  chassis->mutable_lexus()->mutable_horn_cmd_11c()->set_horn_cmd(horn_cmd(bytes, length));
}
...
bool Horncmd11c::ignore_overrides(const std::uint8_t* bytes, int32_t length) const {
  Byte t0(bytes + 0);
...
bool Horncmd11c::clear_override(const std::uint8_t* bytes, int32_t length) const {
  Byte t0(bytes + 0);
...
// config detail: {'name': 'horn_cmd', 'enum': {0: 'HORN_CMD_OFF', 1: 'HORN_CMD_ON'}, 'precision': 1.0, 'len': 1, 'is_signed_var': False, 'offset': 0.0, 'physical_range': '[0|1]', 'bit': 8, 'type': 'enum', 'order': 'motorola', 'physical_unit': ''}
Horn_cmd_11c::Horn_cmdType Horncmd11c::horn_cmd(const std::uint8_t* bytes, int32_t length) const {
  Byte t0(bytes + 1);
  int32_t x = t0.get_byte(0, 1);
