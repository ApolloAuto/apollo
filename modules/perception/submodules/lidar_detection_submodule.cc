 ...
 *****************************************************************************/
#include "modules/perception/submodules/lidar_detection_submodule.h"
...
std::atomic<uint32_t> LidarDetectionSubmodule::seq_num_{0};

bool LidarDetectionSubmodule::Init() {
  LidarDetectionSubmoduleConfig comp_config;
  if (!GetProtoConfig(&comp_config)) {
    return false;
  }
  ADEBUG << "Lidar Detection Submodule Configs: " << comp_config.DebugString();
  ...
  if (!InitAlgorithmPlugin()) {
    AERROR << "Failed to init lidar detection submodule algorithm plugin.";
    return false;
  }
  return true;
}

bool LidarDetectionSubmodule::Proc(
    const std::shared_ptr<drivers::PointCloud>& message) {
  AINFO << std::setprecision(16)
        << "Enter lidar detection submodule, message timestamp: "
...
bool LidarDetectionSubmodule::InitAlgorithmPlugin() {
...
bool LidarDetectionSubmodule::InternalProc(
