...
#else
class SLICEPlugin : public nvinfer1::IPluginV2Ext {
 public:
  SLICEPlugin(const SliceParameter &param, const nvinfer1::Dims &in_dims) {
    CHECK_GT(param.slice_point_size(), 0);
    for (int i = 0; i < param.slice_point_size(); i++) {
  ...
  nvinfer1::IPluginV2Ext* clone() const noexcept override {
    ...
    for (size_t i = 0; i < slice_point_.size(); i++) {
      p->slice_point_.push_back(slice_point_[i]);
    }
    for (size_t i = 0; i < out_slice_dims_.size(); i++) {
      p->out_slice_dims_.push_back(out_slice_dims_[i]);
    }
    return p;
  }
  ...
  void configurePlugin(
    ...
    bool const *outputIsBroadcast, nvinfer1::PluginFormat floatFormat,
