...
Optimizer::Optimizer(
  ...
  dual_variable_warm_start_.reset(
      new DualVariableWarmStartProblem(
          config.dual_variable_warm_start_config()));

  // Initialize distance approach trajectory smootherclass pointer
  distance_approach_.reset(
      new DistanceApproachProblem(config.distance_approach_config()));

  // Initialize iterative anchoring smoother config class pointer
  iterative_anchoring_smoother_.reset(
      new IterativeAnchoringSmoother(
          config.iterative_anchoring_smoother_config()));
}

Status Optimizer::Plan(const OpenSpaceInfo open_space_info) {
  // const auto start_timestamp = std::chrono::system_clock::now();

  const std::vector<std::vector<Vec2d>>& obstacles_vertices_vec =
      open_space_info.obstacles_vertices_vec();
  const std::vector<double>& XYbounds = open_space_info.ROI_xy_boundary();
  const Eigen::MatrixXi& obstacles_edges_num =
      open_space_info.obstacles_edges_num();
  const Eigen::MatrixXd& obstacles_A = open_space_info.obstacles_A();
  const Eigen::MatrixXd& obstacles_b = open_space_info.obstacles_b();

  DiscretizedTrajectory warm_start_trajectory =
      open_space_info.path_planning_trajectory_result();
  const double origin_angle = open_space_info.origin_heading();
  const Vec2d origin_point = open_space_info.origin_point();
  for (size_t i = 0; i < warm_start_trajectory.size(); i++) {
    double x = warm_start_trajectory[i].path_point().x();
    double y = warm_start_trajectory[i].path_point().y();
    double theta = warm_start_trajectory[i].path_point().theta();
    OpenSpaceTrajectoryOptimizerUtil::PathPointNormalizing(origin_angle,
                                                           origin_point,
                                                           &x,
                                                           &y,
                                                           &theta);
    warm_start_trajectory[i].mutable_path_point()->set_x(x);
    warm_start_trajectory[i].mutable_path_point()->set_y(y);
    warm_start_trajectory[i].mutable_path_point()->set_theta(theta);
  }
  ...
  if (FLAGS_enable_parallel_trajectory_smoothing) {
    std::vector<DiscretizedTrajectory> partition_trajectories;

    PartitionTrajectory(warm_start_trajectory, &partition_trajectories);
    ...
    for (size_t i = 0; i < size; ++i) {
      LoadTrajectoryInEigen(partition_trajectories[i], xWS_vec[i],
                                   uWS_vec[i]);
    ...
    LoadTrajectoryInEigen(warm_start_trajectory, xWS, uWS);
  ...
  if (FLAGS_enable_record_debug) {
    ...
    RecordDebugInfo(origin_point, origin_angle,
                    open_space_info.open_space_end_pose(),
                    xWS, uWS, l_warm_up, n_warm_up, dual_l_result_ds,
  ...
  // double temp = diff.count() * 1000.0;

  return Status::OK();
}

void Optimizer::LoadTrajectoryInEigen(
    DiscretizedTrajectory& result, Eigen::MatrixXd& xWS, Eigen::MatrixXd& uWS) {
  size_t horizon = result.size() - 1;
  xWS.resize(4, horizon + 1);
  uWS.resize(2, horizon);

  for (size_t i = 0; i < horizon; i++) {
    xWS(0, i) = result[i].path_point().x();
    xWS(1, i) = result[i].path_point().y();
    xWS(2, i) = result[i].path_point().theta();
    xWS(3, i) = result[i].v();
    uWS(0, i) = result[i].steer();
    uWS(1, i) = result[i].a();
  }
  xWS(0, horizon) = result.back().path_point().x();
  xWS(1, horizon) = result.back().path_point().y();
  xWS(2, horizon) = result.back().path_point().theta();
  xWS(3, horizon) = result.back().v();
}

void Optimizer::CombineTrajectories(
...
bool Optimizer::GenerateDistanceApproachTraj(
  ...
  double ts = FLAGS_open_space_delta_t;
...
bool Optimizer::GenerateDecoupledTraj(
...
void Optimizer::UseWarmStartAsResult(
    const Eigen::MatrixXd& xWS, const Eigen::MatrixXd& uWS,
  ...
  *time_result_ds = Eigen::MatrixXd::Constant(
      1, time_result_horizon, FLAGS_open_space_delta_t);
}

void Optimizer::PartitionTrajectory(
  ...
  for (size_t i = 0; i < horizon - 1; ++i) {
    current_trajectory->push_back(raw_trajectory.at(i));
    ...
    if (cur_gear != gear) {
      gear = cur_gear;
      partitioned_trajectories->emplace_back();
      current_trajectory = &(partitioned_trajectories->back());
      current_trajectory->push_back(raw_trajectory.at(i));
    }
  }
  current_trajectory->push_back(raw_trajectory.back());
}

void Optimizer::LoadTrajectory(
...
void Optimizer::RecordDebugInfo(
  ...
  // open_space_debug_.mutable_roi_shift_point()->mutable_path_point()->set_x();
...
void Optimizer::UpdateDebugInfo(
