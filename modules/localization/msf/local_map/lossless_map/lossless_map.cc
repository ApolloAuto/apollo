...
void LosslessMap::set_value(const Eigen::Vector3d& coordinate, int zone_id,
    ...
    for (size_t i = 0; i < _map_config._map_resolutions.size(); ++i) {
        MapNodeIndex index = MapNodeIndex::get_map_node_index(_map_config, coordinate,
                                                              i, zone_id);
        LosslessMapNode* node = static_cast<LosslessMapNode*>(get_map_node_safe(index));
        node->set_value(coordinate, intensity);
    }
}

void LosslessMap::set_value_layer(const Eigen::Vector3d& coordinate, int zone_id,
                         unsigned char intensity) {
    for (size_t i = 0; i < _map_config._map_resolutions.size(); ++i) {
        MapNodeIndex index = MapNodeIndex::get_map_node_index(_map_config, coordinate,
                                                              i, zone_id);
...
void LosslessMap::get_alt_var_safe(const Eigen::Vector3d& coordinate, int zone_id,
                      unsigned int resolution_id, std::vector<float>& alt_vars) {
    assert(resolution_id < _map_config._map_resolutions.size());
    MapNodeIndex index = MapNodeIndex::get_map_node_index(_map_config, coordinate,
                                                          resolution_id, zone_id);
...
unsigned char LosslessMap::get_value(const Eigen::Vector3d& coordinate, int zone_id,
...
unsigned char LosslessMap::get_value_safe(const Eigen::Vector3d& coordinate, int zone_id,
                            unsigned int resolution_id) {
    assert(resolution_id < _map_config._map_resolutions.size());
    MapNodeIndex index = MapNodeIndex::get_map_node_index(_map_config, coordinate,
                                                          resolution_id, zone_id);
...
float LosslessMap::get_alt_var_safe(const Eigen::Vector3d& coordinate, int zone_id,
                      unsigned int resolution_id) {
    assert(resolution_id < _map_config._map_resolutions.size());
    MapNodeIndex index = MapNodeIndex::get_map_node_index(_map_config, coordinate,
                                                          resolution_id, zone_id);
...
unsigned int LosslessMap::get_count(const Eigen::Vector3d& coordinate, int zone_id,
                           unsigned int resolution_id) {
    assert(resolution_id < _map_config._map_resolutions.size());
    MapNodeIndex index = MapNodeIndex::get_map_node_index(_map_config, coordinate,
                                                          resolution_id, zone_id);
...
unsigned int LosslessMap::get_count_safe(const Eigen::Vector3d& coordinate, int zone_id,
                           unsigned int resolution_id) {
    assert(resolution_id < _map_config._map_resolutions.size());
    MapNodeIndex index = MapNodeIndex::get_map_node_index(_map_config, coordinate,
                                                          resolution_id, zone_id);
...
bool LosslessMap::load_map_area(const Eigen::Vector3d& seed_pt3d, 
    ...
    BaseMap::load_map_area(seed_pt3d, resolution_id, zone_id, filter_size_x, filter_size_y);
    return true;
}
