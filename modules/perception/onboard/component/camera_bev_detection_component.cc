 ...
 *****************************************************************************/
#include "modules/perception/onboard/component/camera_bev_detection_component.h"
...
CameraBevDetectionComponent::~CameraBevDetectionComponent() {}
...
bool CameraBevDetectionComponent::Proc(
  ...
  if (camfl_msg == nullptr) {
    AERROR << "camera_front_left msg is not ready!";
    return false;
  }
  ...
  if (camfr_msg == nullptr) {
    AERROR << "camera_front_right msg is not ready!";
    return false;
  }
  ...
  if (cambl_msg == nullptr) {
    AERROR << "camera_back_left msg is not ready!";
    return false;
  }
  ...
  if (cambr_msg == nullptr) {
    AERROR << "camera_back_right msg is not ready!";
    return false;
  }
...
void CameraBevDetectionComponent::OnReceiveImage(
  ...
  AINFO << "Enter CameraBevDetectionComponent::Proc(), "
...
int CameraBevDetectionComponent::InitConfig() {
  ...
  if (camera_names_.size() != 6) {
    AERROR << "Now CameraBevDetectionComponent only support 6 cameras";
    return cyber::FAIL;
  }
  ...
  std::string format_str = R"(
      CameraBevDetectionComponent InitConfig success
      camera_names:    %s, %s
...
int CameraBevDetectionComponent::InitSensorInfo() {
  if (camera_names_.size() != 6) {
...
int CameraBevDetectionComponent::InitAlgorithmPlugin() {
...
int CameraBevDetectionComponent::InitCameraFrames() {
  if (camera_names_.size() != 6) {
    AERROR << "invalid camera_names_.size(): " << camera_names_.size();
    return cyber::FAIL;
  }
...
int CameraBevDetectionComponent::InitProjectMatrix() {
...
int CameraBevDetectionComponent::InitCameraListeners() {
  for (size_t i = 0; i < camera_names_.size(); ++i) {
    ...
    std::function<void(const ImageMsgType &)> camera_callback =
        std::bind(&CameraBevDetectionComponent::OnReceiveImage, this,
...
int CameraBevDetectionComponent::InitBevCameraListeners() {
  cyber::ReaderConfig camf_reader_config;
  camf_reader_config.channel_name = input_camera_channel_names_[0];
...
int CameraBevDetectionComponent::InitMotionService() {
  ...
  std::function<void(const MotionServiceMsgType &)> motion_service_callback =
      std::bind(&CameraBevDetectionComponent::OnMotionService, this,
                std::placeholders::_1);
...
void CameraBevDetectionComponent::OnMotionService(
...
/*
void CameraBevDetectionComponent::SetCameraHeightAndPitch() {
...
int CameraBevDetectionComponent::InternalProc(
...
int CameraBevDetectionComponent::MakeProtobufMsg(
...
int CameraBevDetectionComponent::ConvertObjectToPb(
...
int CameraBevDetectionComponent::ConvertObjectToCameraObstacle(
    const base::ObjectPtr &object_ptr,
...
int CameraBevDetectionComponent::ConvertLaneToCameraLaneline(
...
int CameraBevDetectionComponent::MakeCameraDebugMsg(
