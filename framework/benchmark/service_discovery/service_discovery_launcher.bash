...
function run() {
  ...
  echo "start master"
  ./service_discovery_master 1>$LOG_DIR/$MASTER_LOG_NAME 2>&1 &
  sleep 1
  ...
  while [ $curr_slave_num -lt $slave_num ]; do
    ...
    ./service_discovery_slave $role_num $role_num $role_num $role_num $role_num &
  done
}

function end_and_count() {
  echo "stop master and slave"
  ps -ef | grep service_discovery_master | grep -v grep | awk '{print $2}' | xargs kill -9
  ps -ef | grep service_discovery_slave | grep -v grep | awk '{print $2}' | xargs kill -9
