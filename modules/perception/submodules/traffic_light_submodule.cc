 ...
 *****************************************************************************/
#include "modules/perception/submodules/traffic_light_submodule.h"
...
bool TrafficLightSubmodule::Init() {
  ...
  if (InitConfig() != cyber::SUCC) {
    AERROR << "TrafficLightSubmodule InitConfig failed.";
    return false;
  }

  if (InitAlgorithmPlugin() != cyber::SUCC) {
    AERROR << "TrafficLightSubmodule InitAlgorithmPlugin failed.";
    return false;
  }

  if (InitCameraFrame() != cyber::SUCC) {
    AERROR << "TrafficLightSubmodule InitCameraFrame failed.";
    return false;
  }

  if (InitCameraListeners() != cyber::SUCC) {
    AERROR << "TrafficLightSubmodule InitCameraListeners failed.";
    return false;
  }

  if (InitV2XListener() != cyber::SUCC) {
    AERROR << "TrafficLightSubmodule InitV2XListener failed.";
    return false;
  }
...
int TrafficLightSubmodule::InitConfig() {
  apollo::perception::TrafficLightParam traffic_light_param;
  if (!GetProtoConfig(&traffic_light_param)) {
    AINFO << "load traffic light submodule proto param failed";
    return cyber::FAIL;
  }
...
int TrafficLightSubmodule::InitAlgorithmPlugin() {
  // init preprocessor
  preprocessor_.reset(new camera::TLPreprocessor);
  if (!preprocessor_) {
    AERROR << "TrafficLightSubmodule new preprocessor failed";
    return cyber::FAIL;
  }

  preprocessor_init_options_.camera_names = camera_names_;
  if (!preprocessor_->Init(preprocessor_init_options_)) {
    AERROR << "TrafficLightSubmodule init preprocessor failed";
    return cyber::FAIL;
  }
...
int TrafficLightSubmodule::InitCameraListeners() {
  ...
  for (size_t i = 0; i < camera_names_.size(); ++i) {
    ...
    std::function<void(const ImageMsgType&)> sub_camera_callback =
        std::bind(&TrafficLightSubmodule::OnReceiveImage, this,
...
int TrafficLightSubmodule::InitV2XListener() {
  ...
  std::function<void(const V2XTrafficLightsMsgType&)> sub_v2x_tl_callback =
      std::bind(&TrafficLightSubmodule::OnReceiveV2XMsg, this,
                std::placeholders::_1);
...
int TrafficLightSubmodule::InitCameraFrame() {
...
void TrafficLightSubmodule::OnReceiveImage(
...
bool TrafficLightSubmodule::QueryPoseAndSignals(
...
bool TrafficLightSubmodule::VerifyLightsProjection(
...
bool TrafficLightSubmodule::UpdateCameraSelection(
...
bool TrafficLightSubmodule::CheckCameraImageStatus(
...
bool TrafficLightSubmodule::GetCarPose(const double timestamp,
                                       camera::CarPose* pose) {
  PERF_FUNCTION();
...
bool TrafficLightSubmodule::GetPoseFromTF(const double timestamp,
...
bool TrafficLightSubmodule::TransformOutputMessage(
  ...
  if (!TransformDebugMessage(frame, out_msg)) {
    AERROR << "TrafficLightSubmodulefailed to transform debug msg.";
    return false;
  }
...
void TrafficLightSubmodule::TransRect2Box(
...
double TrafficLightSubmodule::stopline_distance(
...
bool TrafficLightSubmodule::TransformDebugMessage(
...
void TrafficLightSubmodule::Visualize(
    const camera::CameraFrame& frame,
...
void TrafficLightSubmodule::SyncV2XTrafficLights(camera::CameraFrame* frame) {
...
void TrafficLightSubmodule::SendSimulationMsg() {
