...
bool KCFAffinityTracker::get_affinity_matrix(const cv::Mat &img,
...
bool KCFAffinityTracker::update_tracked(const cv::Mat &img, const std::vector<Detected> &detected,
    ...
    for (auto &tracked_obj: tracked) {
        ...
        if (det_id >= 0 && _detected_features.find(det_id)!=_detected_features.end()) {
