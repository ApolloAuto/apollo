...
#include "modules/canbus_vehicle/demo/demo_vehicle_factory.h"
...
bool DemoVehicleFactory::Init(const CanbusConf *canbus_conf) {
  // Init can client
  ...
  chassis_detail_writer_ =
      node_->CreateWriter<::apollo::canbus::Demo>(FLAGS_chassis_detail_topic);

  chassis_detail_sender_writer_ = node_->CreateWriter<::apollo::canbus::Demo>(
      FLAGS_chassis_detail_sender_topic);

  return true;
}

bool DemoVehicleFactory::Start() {
...
void DemoVehicleFactory::UpdateCommand(
...
Chassis DemoVehicleFactory::publish_chassis() {
  Chassis chassis = vehicle_controller_->chassis();
  ADEBUG << chassis.ShortDebugString();
  return chassis;
}

void DemoVehicleFactory::PublishChassisDetail() {
  Demo chassis_detail = vehicle_controller_->GetNewRecvChassisDetail();
  ADEBUG << "latest chassis_detail is " << chassis_detail.ShortDebugString();
  chassis_detail_writer_->Write(chassis_detail);
}

void DemoVehicleFactory::PublishChassisDetailSender() {
  Demo sender_chassis_detail = vehicle_controller_->GetNewSenderChassisDetail();
...
void DemoVehicleFactory::UpdateHeartbeat() { can_sender_.Update_Heartbeat(); }

bool DemoVehicleFactory::CheckChassisCommunicationFault() {
...
void DemoVehicleFactory::AddSendProtocol() {
  vehicle_controller_->AddSendMessage();
}

void DemoVehicleFactory::ClearSendProtocol() { can_sender_.ClearMessage(); }

bool DemoVehicleFactory::IsSendProtocolClear() {
...
Chassis::DrivingMode DemoVehicleFactory::Driving_Mode() {
  return vehicle_controller_->driving_mode();
}

std::unique_ptr<VehicleController<::apollo::canbus::Demo>>
DemoVehicleFactory::CreateVehicleController() {
  return std::unique_ptr<VehicleController<::apollo::canbus::Demo>>(
      new demo::DemoController());
}

std::unique_ptr<MessageManager<::apollo::canbus::Demo>>
DemoVehicleFactory::CreateMessageManager() {
  return std::unique_ptr<MessageManager<::apollo::canbus::Demo>>(
      new demo::DemoMessageManager());
}
