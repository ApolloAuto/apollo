...
bool SidePassScenario::Init(const PlanningConfig& config) {
  if (is_init_) {
    return true;
  }
  RegisterTasks();
  for (const auto task : config.side_pass_scenario_config().task()) {
    tasks_.emplace_back(
...
void SidePassScenario::RecordObstacleDebugInfo(
...
void SidePassScenario::RecordDebugInfo(ReferenceLineInfo* reference_line_info,
...
Status SidePassScenario::PlanOnReferenceLine(
...
void SidePassScenario::GenerateFallbackPathProfile(
...
SLPoint SidePassScenario::GetStopSL(const ObjectStop& stop_decision,
...
ScenarioConfig::ScenarioType SidePassScenario::Transfer(
