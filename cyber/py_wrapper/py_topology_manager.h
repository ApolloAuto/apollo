 ...
 public:
  ...
  PyObject *GetNodeList() {
    ...
    if (nodes.empty()) {
        ...
        for (auto& node : nodes) {
            PyList_Append(pyobj_list, Py_BuildValue("s", ConnectChar("    ",
  ...
  const char *ConnectChar(const char* convert_temp1,
    ...
    std::string convert_temp = std::string(convert_temp1) +
                                                   std::string(convert_temp2);
  ...
  PyObject* ShowNodeInfo(const std::string& node_name) {
      ...
      if (node_manager_->HasNode(node_name)) {
        ...
        for (auto& node : nodes) {
          if (node.node_name() == node_name) {
            ...
            PyList_Append(pyobj_list, Py_BuildValue("s",
                    ConnectChar("processid: ", convert_temp.str().c_str())));
            convert_temp.clear();
            PyList_Append(pyobj_list, Py_BuildValue("s",
    ...
    if (notFound) {
        PyList_Append(pyobj_list, Py_BuildValue("s",
                    ConnectChar("Node cannot be found: ", node_name.c_str())));
    }
    return pyobj_list;
  }
 ...
 public:
  ...
  PyObject* ShowChannelInfo(const std::string& channel_name) {
     ...
     std::unordered_map<std::string, std::vector
                           <apollo::cyber::proto::RoleAttributes>> roles_info;
     if (channel_manager_->HasWriter(channel_name)||
       ...
       PyList_Append(pyobj_list, Py_BuildValue("s",
               ConnectChar("channels size: ", convert_temp.str().c_str())));
       ...
       PyList_Append(pyobj_list, Py_BuildValue("s",
               ConnectChar("channel name: ", convert_temp.str().c_str())));
       for (auto role_info : it->second) {
         convert_temp.str("");
         convert_temp << role_info.id();
         PyList_Append(pyobj_list, Py_BuildValue("s",
            ConnectChar("  roleid:      ", convert_temp.str().c_str())));
         PyList_Append(pyobj_list, Py_BuildValue("s",
            ConnectChar("    hostname:   ", role_info.host_name().c_str())));
         convert_temp.str("");
         convert_temp << role_info.process_id();
         PyList_Append(pyobj_list, Py_BuildValue("s",
           ConnectChar("    processid:  ", convert_temp.str().c_str())));
         PyList_Append(pyobj_list, Py_BuildValue("s",
           ConnectChar("    nodename:   ", role_info.node_name().c_str())));
         PyList_Append(pyobj_list, Py_BuildValue("s",
           ConnectChar("    msgtype:    ",
     ...
     if (notFound) {
       PyList_Append(pyobj_list, Py_BuildValue("s",
           ConnectChar("Channel cannot be found: ", channel_name.c_str())));
     }
     return pyobj_list;
  }
