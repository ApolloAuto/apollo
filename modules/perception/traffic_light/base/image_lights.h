...
struct ImageLights {
  ...
  std::shared_ptr<LightPtrs>
      lights_outside_image;  // record the lights outside the lights.
  ...
  double preprocess_send_timestamp =
      0.0;  // timestamp when PreprocessSubnode pub event
  bool is_pose_valid = false;
  double diff_image_pose_ts =
      0.0;  // image' timestamp minus the most recently pose's timestamp
