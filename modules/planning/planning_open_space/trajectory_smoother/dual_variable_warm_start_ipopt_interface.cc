...
#include "modules/planning/planning_open_space/trajectory_smoother/dual_variable_warm_start_ipopt_interface.h"
...
DualVariableWarmStartIPOPTInterface::DualVariableWarmStartIPOPTInterface(
    ...
    l_warm_up_ = Eigen::MatrixXd::Zero(obstacles_edges_sum_, horizon_ + 1);
    n_warm_up_ = Eigen::MatrixXd::Zero(4 * obstacles_num_, horizon_ + 1);
    weight_d_ = dual_variable_warm_start_config.weight_d();
}
...
bool DualVariableWarmStartIPOPTInterface::eval_f(int n, const double* x, bool new_x, double& obj_value) {
    eval_obj(n, x, &obj_value);
    return true;
}

bool DualVariableWarmStartIPOPTInterface::eval_grad_f(int n, const double* x, bool new_x, double* grad_f) {
...
bool DualVariableWarmStartIPOPTInterface::eval_g(int n, const double* x, bool new_x, int m, double* g) {
...
bool DualVariableWarmStartIPOPTInterface::eval_h(
        int n,
        const double* x,
        bool new_x,
        double obj_factor,
...
bool DualVariableWarmStartIPOPTInterface::eval_constraints(int n, const T* x, int m, T* g) {
    ...
    for (int i = 0; i < horizon_ + 1; ++i) {
        ...
        for (int j = 0; j < obstacles_num_; ++j) {
            ...
            // G' * mu + R' * A' * lambda == 0
            g[constraint_index + 1] = x[n_index] - x[n_index + 2] + cos(xWS_(2, i)) * tmp1 + sin(xWS_(2, i)) * tmp2;

            g[constraint_index + 2] = x[n_index + 1] - x[n_index + 3] - sin(xWS_(2, i)) * tmp1 + cos(xWS_(2, i)) * tmp2;
            ...
            g[constraint_index + 3] = x[d_index] + tmp3 - (xWS_(0, i) + cos(xWS_(2, i)) * offset_) * tmp1
