...
bool ObjPostProcessor::AdjustCenterWithGround(const float *bbox,
  ...
  while (!stop) {
    algorithm::IProjectThroughIntrinsic(k_mat_, center, x);
    x[0] *= algorithm::IRec(x[2]);
    x[1] *= algorithm::IRec(x[2]);
    bool in_front = algorithm::IBackprojectPlaneIntersectionCanonical(
    ...
    float dist = algorithm::ISqrt(algorithm::ISqr(center[0] - center_test[0]) +
                                  algorithm::ISqr(center[2] - center_test[2]));
...
bool ObjPostProcessor::PostRefineCenterWithGroundBoundary(
  ...
  float dxdz_acc[2] = {0};
  float ratio_x_over_z = center[0] * algorithm::IRec(center[2]);
...
int ObjPostProcessor::GetDepthXPair(const float *bbox, const float *hwl,
  ...
  for (int i = 0; i < 4; ++i) {
    algorithm::IProjectThroughExtrinsic(rot, center, pt, pt_c);
    algorithm::IProjectThroughIntrinsic(k_mat_, pt_c, pt_proj);
    depth_pts[i] = pt_c[2];
    x_pts[i] = algorithm::IRound(pt_proj[0] * algorithm::IRec(pt_proj[2]));
    int y_proj = algorithm::IRound(pt_proj[1] * algorithm::IRec(pt_proj[2]));
    if (y_proj < y_min) {
      y_min = y_proj;
    }
    if (save_pts_c) {
