...
#include "modules/perception/obstacle/fusion/imf_fusion/async_fusion.h"
...
bool AsyncFusion::Init() {
    track_manager_ = AsyncTrackManager::instance();
...
bool AsyncFusion::Fuse(
...

    {
        fusion_mutex_.lock();
        FuseFrame(frame);
        // 4.collect results, we don't need to collect fused_objects unless we have to
...
std::string AsyncFusion::name() const {
    return "AsyncFusion";
}
...
void AsyncFusion::CreateNewTracks(
    ...
    for (size_t i = 0; i < unassigned_ids.size(); i++) {
        ...
        AsyncTrackPtr track(new AsyncTrack(sensor_objects[id]));
        track_manager_->AddTrack(track);
    }
}

void AsyncFusion::UpdateAssignedTracks(
        std::vector<AsyncTrackPtr> *tracks,
        const std::vector<PbfSensorObjectPtr> &sensor_objects,
...
void AsyncFusion::UpdateUnassignedTracks(
        std::vector<AsyncTrackPtr> *tracks, const std::vector<int> &unassigned_tracks,
...
void AsyncFusion::CollectFusedObjects(
    ...
    int fg_obj_num = 0;
    std::vector<AsyncTrackPtr> &tracks = track_manager_->GetTracks();
