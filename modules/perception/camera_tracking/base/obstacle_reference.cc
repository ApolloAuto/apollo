...
void ObstacleReference::Init(const ReferenceParam &ref_param, float width,
                             float height) {
...
void ObstacleReference::UpdateReference(const CameraTrackingFrame *frame,
  ...
  for (auto &&reference : refs) {
    ...
    vd_samples[count_vd++] = algorithm::IRec(z_ref);
  }
...
void ObstacleReference::CorrectSize(CameraTrackingFrame *frame) {
    ...
    if (Contain(object_template_manager_->TypeRefinedByRef(), obj->sub_type)) {
      ...
      if (ground_estimator.GetGroundModel(l)) {
        double z_ground_reversed =
            (-l[0] * supplement.box.ymax - l[2]) * algorithm::IRec(l[1]);
        double z_ground = algorithm::IRec(z_ground_reversed);
      ...
      if (frame->calibration_service != nullptr) {
        ...
        // wxt todo: solve this problem
        // frame->calibration_service->QueryGroundPlaneInCameraTrackingFrame(&plane_param);
      }
