...
bool DistanceApproachProblem::Solve(
  ...
  app->Options()->SetIntegerValue("print_level", 
      planner_open_space_config_.distance_approach_config().ipopt_print_level());
  app->Options()->SetIntegerValue("mumps_mem_percent", 
      planner_open_space_config_.distance_approach_config().mumps_mem_percent());
  app->Options()->SetNumericValue("mumps_pivtol", 
      planner_open_space_config_.distance_approach_config().mumps_pivtol());
  app->Options()->SetIntegerValue("max_iter", 
      planner_open_space_config_.distance_approach_config().ipopt_max_iter());
  app->Options()->SetNumericValue("tol", 
      planner_open_space_config_.distance_approach_config().ipopt_tol());
  app->Options()->SetNumericValue("acceptable_constr_viol_tol", 
      planner_open_space_config_.distance_approach_config().ipopt_acceptable_constr_viol_tol());
  app->Options()->SetNumericValue("min_hessian_perturbation", 
      planner_open_space_config_.distance_approach_config().ipopt_min_hessian_perturbation());
  app->Options()->SetNumericValue("jacobian_regularization_value", 
      planner_open_space_config_.distance_approach_config().ipopt_jacobian_regularization_value());
  app->Options()->SetStringValue("print_timing_statistics", 
      planner_open_space_config_.distance_approach_config().ipopt_print_timing_statistics());
  app->Options()->SetStringValue("alpha_for_y", 
      planner_open_space_config_.distance_approach_config().ipopt_alpha_for_y());
  app->Options()->SetStringValue("recalc_y", 
      planner_open_space_config_.distance_approach_config().ipopt_recalc_y());
