...
// @brief: detector for Continental ARS408-21.
...
void ContiArsDetector::RawObs2Frame(const ContiRadar& corrected_obstacles,
  ...
  rotation_novatel << 0, -angular_speed(2), angular_speed(1),
                      angular_speed(2), 0, -angular_speed(0),
                      -angular_speed(1), angular_speed(0), 0;
  ...
  for (auto radar_obs : corrected_obstacles.contiobs()) {
    ...
    radar_object->center_uncertainty = (radar2world_rotate * dist_rms
                                     * dist_rms.transpose()
                                     * radar2world_rotate_t).cast<float>();

    radar_object->velocity_uncertainty = (radar2world_rotate * vel_rms
                                     * vel_rms.transpose()
                                     * radar2world_rotate_t).cast<float>();
    ...
    float local_angle = static_cast<float>(
        std::atan2(local_loc(1), local_loc(0)));
