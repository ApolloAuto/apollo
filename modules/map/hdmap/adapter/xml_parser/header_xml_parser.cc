...
Status HeaderXmlParser::parse(const tinyxml2::XMLElement& xml_node,
                        PbHeader* header) {
    const auto header_node = xml_node.FirstChildElement("header");
    if (!header_node) {
    ...
    const tinyxml2::XMLElement *geo_reference_node =
                                header_node->FirstChildElement("geoReference");
    if (geo_reference_node) {
        const auto geo_text = geo_reference_node->FirstChild()->ToText();
        if (geo_text) {
            std::cout << geo_text->Value() << std::endl;
        }
    }
