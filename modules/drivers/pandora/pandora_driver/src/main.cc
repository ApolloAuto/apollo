...
public:
  PandoraHesaiClient(ros::NodeHandle node, ros::NodeHandle nh)
  {
    ...
    std::string lidarTopic = std::string("/apollo/sensor/pandora/hesai40/PointCloud2");
    ...
    bool ret = parseParameter(nh, pandoraIP, lidarRecvPort, gpsRecvPort, startAngle, pandoraCameraPort,
  ...
  bool parseParameter(ros::NodeHandle nh, std::string& pandoraIP, int& lidarRecvPort,
          int& gpsRecvPort, int& startAngle, int& pandoraCameraPort, std::string& lidarTopic,
          std::string cameraTopics[], bool& enableCamera, int& timezone, std::string& frameId) {
  ...
  void cameraCallback(boost::shared_ptr<cv::Mat> matp, double timestamp, int pic_id, bool distortion)
  {
