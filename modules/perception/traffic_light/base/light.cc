...
double stopline_distance(
  ...
  if (stopline.segment_size() == 0) {
    AWARN << "compute car to stopline's distance failed(stopline has no "
             "segment line). "
          << "car_pose:" << car_pose
  ...
  if (stopline.segment(0).line_segment().point_size() == 0) {
    AWARN
        << "compute car to stopline's distance failed(stopline has no point). "
        << "car_pose:" << car_pose
        << " stopline:" << stopline.ShortDebugString();
    return -1;
  }
...
double trafficlight_distance(
  ...
  if (subsignal.size() == 0) {
    AWARN
        << "compute car to traffic_light's distance failed(no traffic_light). "
        << "car_pose:" << car_pose;
    return -1;
  }
  double car_x = car_pose(0, 3);
  ...
  return sqrt(std::pow(car_x - traffic_light_x, 2) +
              std::pow(car_y - traffic_light_y, 2));
}
