 ...
 * @file
 */
#include "modules/planning/planning_open_space/trajectory_smoother/distance_approach_ipopt_fixed_ts_interface.h"
namespace apollo {
namespace planning {
...
bool DistanceApproachIPOPTFixedTsInterface::eval_f(int n, const double* x, bool new_x, double& obj_value) {
    eval_obj(n, x, &obj_value);
    return true;
}

bool DistanceApproachIPOPTFixedTsInterface::eval_grad_f(int n, const double* x, bool new_x, double* grad_f) {
    gradient(tag_f, n, x, grad_f);
    return true;
}

bool DistanceApproachIPOPTFixedTsInterface::eval_g(int n, const double* x, bool new_x, int m, double* g) {
    eval_constraints(n, x, m, g);
    if (enable_constraint_check_) {
        check_g(n, x, m, g);
    }
...
void DistanceApproachIPOPTFixedTsInterface::eval_constraints(int n, const T* x, int m, T* g) {
    ...
    for (int i = 0; i < horizon_; ++i) {
        ...
        */
        ...
        g[constraint_index + 1]
                = x[state_index + 5] - (x[state_index + 1] + ts_ * x[state_index + 3] * sin(x[state_index + 2]));
        ...
        */
        g[constraint_index + 3] = x[state_index + 7] - (x[state_index + 3] + ts_ * x[control_index + 1]);
...
bool DistanceApproachIPOPTFixedTsInterface::check_g(int n, const double* x, int m, const double* g) {
    int kN = n;
    int kM = m;
    double x_u_tmp[kN];
    double x_l_tmp[kN];
...
void DistanceApproachIPOPTFixedTsInterface::generate_tapes(int n, int m, int* nnz_jac_g, int* nnz_h_lag) {
