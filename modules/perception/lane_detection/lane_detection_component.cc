 ...
 *****************************************************************************/
#include "modules/perception/lane_detection/lane_detection_component.h"
...
static int GetGpuId(const camera::CameraPerceptionInitOptions &options) {
  ...
  std::string config_file =
      GetAbsolutePath(options.config_path, options.config_file);
...
int LaneDetectionComponent::InitConfig() {
  ...
  // Init camera_lane_pipeline
  auto detection_param = lane_detection_param.detection_param();
  camera_perception_init_options_.config_path = detection_param.config_path();
  camera_perception_init_options_.config_file = detection_param.config_file();
  ...
  std::string format_str = R"(
      ...
      camera_names:    %s, %s
      camera_lane_perception_config_path:    %s
      camera_lane_perception_config_file:    %s
      frame_capacity:    %d
      image_channel_num:    %d
  ...
  std::string config_info_str =
      str(boost::format(format_str.c_str()) % camera_names_[0] %
          camera_names_[1] % camera_perception_init_options_.config_path %
          camera_perception_init_options_.config_file % frame_capacity_ %
...
int LaneDetectionComponent::InitSensorInfo() {
  ...
  auto *sensor_manager = algorithm::SensorManager::Instance();
...
int LaneDetectionComponent::InternalProc(
  ...
  camera::CameraPerceptionOptions camera_perception_options;
  if (!camera_lane_pipeline_->Perception(camera_perception_options,
                                         &camera_frame)) {
