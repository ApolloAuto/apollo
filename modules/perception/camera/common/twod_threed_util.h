...
struct LineSegment2D {
  ...
  LineSegment2D(const T &x_start, const T &y_start,
                const T &x_end, const T &y_end) {
    pt_start[0] = x_start;
    pt_start[1] = y_start;
    pt_end[0] = x_end;
    pt_end[1] = y_end;
...
T GetJaccardIndex(const T *bbox_ref, const T &x_min, const T &y_min,
  ...
  T overlap_x
      = std::min(bbox_ref[2], x_max) - std::max(bbox_ref[0], x_min);
  T overlap_y
      = std::min(bbox_ref[3], y_max) - std::max(bbox_ref[1], y_min);
...
bool Occlude(const T *bbox1, const T &h1, const T *bbox2, const T &h2) {
  T overlap_x
      = std::min(bbox1[2], bbox2[2]) - std::max(bbox1[0], bbox2[0]);
  T overlap_y
      = std::min(bbox1[3], bbox2[3]) - std::max(bbox1[1], bbox2[1]);
  if (overlap_y <= (T) 0 || overlap_x <= (T) 0) {
    return false;
  }
...
template<typename T>
void GetBboxFromPts(const T *pts, const int &n,
                    T *bbox) {
  bbox[0] = bbox[2] = pts[0];
  bbox[1] = bbox[3] = pts[1];
  int i2 = 2;
...
void GetDxDzForCenterFromGroundLineSeg(const LineSegment2D<T> &ls,
  ...
  bool all_positive
      = zs[0] > 0.0f && zs[1] > 0.f && zs[2] > 0.f && zs[3] > 0.f;
  if (all_positive) {
