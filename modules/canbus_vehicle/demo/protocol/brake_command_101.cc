...
#include "modules/canbus_vehicle/demo/protocol/brake_command_101.h"
...
void Brakecommand101::Parse(const std::uint8_t* bytes, int32_t length,
                            Demo* chassis) const {
  chassis->mutable_brake_command_101()->set_heartbeat_101(
      heartbeat_101(bytes, length));
  chassis->mutable_brake_command_101()->set_aeb_en_ctrl(
      aeb_en_ctrl(bytes, length));
  chassis->mutable_brake_command_101()->set_brake_dec(brake_dec(bytes, length));
...
void Brakecommand101::UpdateData_Heartbeat(uint8_t* data) {
  ...
  ++heartbeat_101_;
  heartbeat_101_ = (heartbeat_101_) % 16;
...
Brakecommand101* Brakecommand101::set_heartbeat_101(int heartbeat_101) {
  heartbeat_101_ = heartbeat_101;
  return this;
}

// config detail: {'bit': 7, 'is_signed_var': False, 'len': 4, 'name':
// 'Heartbeat_101', 'offset': 0.0, 'order': 'motorola', 'physical_range':
// '[0|15]', 'physical_unit': '', 'precision': 1.0, 'type': 'int'}
void Brakecommand101::set_p_heartbeat_101(uint8_t* data, int heartbeat_101) {
  heartbeat_101 = ProtocolData::BoundedValue(0, 15, heartbeat_101);
  int x = heartbeat_101;
...
// 'len': 16, 'name': 'Brake_Pedal_Target', 'offset': 0.0, 'order': 'motorola',
...
// 'BRAKE_EN_CTRL_DISABLE', 1: 'BRAKE_EN_CTRL_ENABLE'}, 'is_signed_var': False,
// 'len': 1, 'name': 'Brake_EN_CTRL', 'offset': 0.0, 'order': 'motorola',
// 'physical_range':
...
int Brakecommand101::heartbeat_101(const std::uint8_t* bytes,
