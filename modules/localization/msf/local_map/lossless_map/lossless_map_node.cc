...
namespace msf {
LosslessMapNode::LosslessMapNode() : BaseMapNode(
                new LosslessMapMatrix(), new ZlibStrategy()) {
}
...
void LosslessMapNode::set_value(const Eigen::Vector3d& coordinate,
    ...
    assert(is_success);
    LosslessMapCell& map_cell = static_cast<LosslessMapMatrix*>(_map_matrix)->get_map_cell(y, x);
...
void LosslessMapNode::set_value_layer(const Eigen::Vector3d& coordinate,
    ...
    assert(is_success);
    LosslessMapCell& map_cell = static_cast<LosslessMapMatrix*>(_map_matrix)->get_map_cell(y, x);
    map_cell.set_value_layer(coordinate[2], intensity, 
...
unsigned char LosslessMapNode::get_value(const Eigen::Vector3d& coordinate) const {
    Eigen::Vector2d coord2d(coordinate[0], coordinate[1]);
    unsigned int x = 0; 
    unsigned int y = 0;
    bool is_success = get_coordinate(coord2d, x, y);
...
unsigned int LosslessMapNode::get_count(const Eigen::Vector3d& coordinate) const {
    Eigen::Vector2d coord2d(coordinate[0], coordinate[1]);
    unsigned int x = 0; 
    unsigned int y = 0;
    bool is_success = get_coordinate(coord2d, x, y);
