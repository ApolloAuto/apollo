...
///////////////////////////////////////////Adapter API///////////////////////////////////////////
void BNAInit(const std::string &cfgPath);
...
bool BNAGetVoiceText(std::string& resText, bool& bPreempt);

bool BNAGetLaneInfo(BNAGuidanceLaneInfo &laneInfo);

bool BNAGetHighWayInfo(BNAHighWayInfo &highWayInfo);

bool BNASetMapAutoLevelStatus(BNAScreenType type, bool  status);

bool BNASetMapMemoryScale(BNAScreenType type, int  memoryScale);
...
bool BNAEnterViewAll(int left, int top, int right, int bottom, int height, int width);

bool BNAExitViewAll();

 bool BNASwitchToAvoidRoute(const bool bSwitch = true, const unsigned int& routeIdx = 0);
...
BNARequestStatus BNASearchCancelByRequestId(const int requestId);
 ...
 * @return	TrueOrFalse
 */
bool BNAMapControlAddBkgData(const BNA_MAP_CONTROL_MERCATOR_POINTINFO*  p_point_info_list, int id_count, BNA_MAP_CONTROL_LATLON sug_point, BNAScreenType type) ;
