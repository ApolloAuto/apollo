...
const double MrfTrackData::kMaxHistoryTime = 2.0;

void MrfTrackData::Reset() {
  TrackData::Reset();
  duration_ = 0.0;
...
void MrfTrackData::Reset(TrackedObjectPtr obj, int track_id) {
  Reset();
  track_id_ = track_id;
  PushTrackedObjectToCache(obj);
}

void MrfTrackData::PushTrackedObjectToTrack(TrackedObjectPtr obj) {
...
void MrfTrackData::PushTrackedObjectToCache(TrackedObjectPtr obj) {
  double timestamp = obj->object_ptr->latest_tracked_time;
...
bool MrfTrackData::ToObject(const Eigen::Vector3d& local_to_global_offset,
  ...
  // f). update cloud world in radar4d supplement
  for (auto& pt : object->radar4d_supplement.cloud_world) {
    pt.x += offset_x;
    pt.y += offset_y;
    pt.z += offset_z;
  }
...
void MrfTrackData::PredictState(double timestamp) const {
  if (history_objects_.empty()) {
    return;
  }
  auto latest_iter = history_objects_.rbegin();
...
void MrfTrackData::GetAndCleanCachedObjectsInTimeInterval(
...
void MrfTrackData::RemoveStaleHistory(double timestamp) {
