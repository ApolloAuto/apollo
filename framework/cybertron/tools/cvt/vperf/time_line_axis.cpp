...
void TimeLineAxis::setBaseTime(int baseTime, int step) {
  QGraphicsTextItem *textItem = _textItems.at(0);
  if (textItem->toPlainText().toInt() == baseTime) return;

  for (int i = 0; i < _textItems.count(); ++i) {
    textItem = _textItems.at(i);
    textItem->setPlainText(QObject::tr("%1").arg(baseTime));
    baseTime += step;
  }
}
