...
static int GetGpuId(
    const camera::CameraPerceptionInitOptions &options) {
...
bool SetCameraHeight(const std::string &sensor_name,
  ...
  try {
    ...
    AINFO << base_h;
    YAML::Node camera_ex = YAML::LoadFile(
        params_dir + "/" + sensor_name + "_extrinsics.yaml");
...
bool GetProjectMatrix(const std::vector<std::string> &camera_names,
  ...
  *project_matrix = intrinsic_map.at(camera_names[0]).cast<double>() *
  ...
  if (pitch_diff != nullptr) {
    Eigen::Vector3d euler =
        (extrinsic_map.at(camera_names[0]).block<3, 3>(0, 0).inverse() *
            extrinsic_map.at(camera_names[1]).block<3, 3>(0, 0)).eulerAngles(0,
                                                                             1,
                                                                             2);
    *pitch_diff = euler(0);
...
void FusionCameraDetectionComponent::OnReceiveImage(
  ...
  if (last_timestamp_ - msg_timestamp > ts_diff_) {
    AINFO << "Received an old message. Last ts is "
             << std::setprecision(19)
             << last_timestamp_
             << " current ts is " << msg_timestamp
  ...
  // for e2e lantency statistics
  {
    ...
    const double start_latency =
        (cur_time - message->measurement_time()) * 1e3;
    AINFO << "FRAME_STATISTICS:Camera:Start:msg_time["
      << camera_name << "-" << GLOG_TIMESTAMP(message->measurement_time())
  ...
  apollo::common::ErrorCode error_code =
      apollo::common::PERCEPTION_ERROR_NONE;

  // prefused msg
  std::shared_ptr<SensorFrameMessage>
      prefused_message(
      new(std::nothrow)
          SensorFrameMessage);

  if (InternalProc(message, camera_name, &error_code,
                   prefused_message.get(), out_message.get())
                  != cybertron::SUCC) {
  ...
  // for e2e lantency statistics
  {
    ...
    AINFO << "FRAME_STATISTICS:Camera:End:msg_time["
      << camera_name << "-" << GLOG_TIMESTAMP(message->measurement_time())
...
int FusionCameraDetectionComponent::InitConfig() {
  ...
  std::string config_info_str = str(boost::format(format_str.c_str()) %
      camera_names_[0] % camera_names_[1] %
      camera_perception_init_options_.root_dir %
...
int FusionCameraDetectionComponent::InitSensorInfo() {
  if (camera_names_.size() != 2) {
    AERROR << "invalid camera_names_.size(): "
              << camera_names_.size();
    return cybertron::FAIL;
  }
  ...
  for (size_t i = 0; i < camera_names_.size(); ++i) {
    ...
    if (!(sensor_manager->GetSensorInfo(camera_names_[i],
                                        &sensor_info))) {
  ...
  std::string format_str = R"(
      ...
      image_height: %d
      image_channel_num: %d)";
  std::string sensor_info_str = str(boost::format(format_str.c_str()) %
      camera_names_[0] % camera_names_[0] %
      tf_camera_frame_id_map_[camera_names_[0]] %
      tf_camera_frame_id_map_[camera_names_[1]] %
...
int FusionCameraDetectionComponent::InitProjectMatrix() {
  if (!GetProjectMatrix(camera_names_,
                        extrinsic_map_, intrinsic_map_,
...
int FusionCameraDetectionComponent::InitCameraListeners() {
  for (size_t i = 0; i < camera_names_.size(); ++i) {
    ...
    const std::string &listener_name = camera_name +
        "_fusion_camera_listener";
    ...
    std::function<void(const ImageMsgType&)> camera_callback =
        std::bind(&FusionCameraDetectionComponent::OnReceiveImage,
...
int FusionCameraDetectionComponent::InternalProc(
  ...
  const double msg_timestamp = in_message->measurement_time() +
      timestamp_offset_;
  ...
  prefused_message->process_stage_ =
  ...
  if (camera2world_trans_wrapper_map_[camera_name]->GetSensor2worldTrans(
      msg_timestamp,
      &camera2world_trans) != cybertron::SUCC) {
  ...
  if (!camera_obstacle_pipeline_->Perception(
      camera_perception_options_, &camera_frame)) {
  ...
  if (output_final_obstacles_ &&
      MakeProtobufMsg(msg_timestamp, seq_num_,
                      camera_frame.tracked_objects, *error_code, out_message)
          != cybertron::SUCC) {
    AERROR << "MakeProtobufMsg failed" << " ts: "
  ...
  *error_code = apollo::common::ErrorCode::PERCEPTION_ERROR_NONE;
  ...
  if (FLAGS_obs_enable_visualization) {
    ...
    prefused_message->frame_->
        camera_frame_supplement.image_blob.reset(new base::Blob<uint8_t>);
  ...
  if (enable_visualization_) {
    ...
    std::shared_ptr<CameraPerceptionVizMessage>
        viz_msg(
        new(std::nothrow)
            CameraPerceptionVizMessage(
            camera_name,
            msg_timestamp,
            camera2world_trans.matrix(),
            image_blob,
...
int FusionCameraDetectionComponent::ConvertObjectToPb(
  ...
  apollo::common::Point3D *obj_anchor_point =
      pb_msg->mutable_anchor_point();
