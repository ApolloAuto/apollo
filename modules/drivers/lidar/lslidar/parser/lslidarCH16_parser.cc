...
//	    packet_end_time = gps_base_usec_ * 1000 - (packets_size - num - 1)*1665*POINTS_PER_PACKET;
//  else
...
//  last_packet_time = packet_end_time;
  ...
  for (size_t point_idx = 0; point_idx < POINTS_PER_PACKET; point_idx++) 
  {
	  ...
	  firings[point_idx].distance=static_cast<double>(point_distance.uint) / 256.0 * DISTANCE_RESOLUTION;     
	...
	for (size_t point_idx = 0; point_idx < POINTS_PER_PACKET; point_idx++) 
	{
		...
		abs(prism_angle[2]) < 1e-6 && abs(prism_angle[3]) < 1e-6 ){
			z_sin_altitude = sin_scan_laser_altitude[firings[point_idx].vertical_line / 4 + 2] +
							 2 * cos_azimuth_half * sin_scan_mirror_altitude[firings[point_idx].vertical_line % 4];
		}else{
			z_sin_altitude = sin_scan_laser_altitude[firings[point_idx].vertical_line / 4 + 2] +
							 2 * cos_azimuth_half * sin(prism_angle[firings[point_idx].vertical_line % 4] * M_PI / 180);
		}
		...
		if (config_.calibration()) {
			...
			ComputeCoords2(firings[point_idx].vertical_line, CH16, firings[point_idx].distance, corrections,
