...
class PyReader {
 public:
  PyReader(const std::string &channel, const std::string &type,
    ...
    reader_ = node_->CreateReader<apollo::cybertron::message::PyMessageWrap>(
        channel, f);
  }
 ...
 private:
  void cb(const std::shared_ptr<const apollo::cybertron::message::PyMessageWrap>
              &message) {
    {
  ...
  std::shared_ptr<
      apollo::cybertron::Reader<apollo::cybertron::message::PyMessageWrap>>
      reader_;
...
class PyService {
 public:
  PyService(const std::string &service_name, const std::string &data_type,
    ...
    service_ = node_->CreateService<apollo::cybertron::message::PyMessageWrap,
                                    apollo::cybertron::message::PyMessageWrap>(
        service_name, f);
  }
 ...
 private:
  void cb(
    ...
    m.reset(new apollo::cybertron::message::PyMessageWrap(msg, data_type_));
    response = m;
  }
  ...
  int (*func_)(const char *) = nullptr;
  std::shared_ptr<
      apollo::cybertron::Service<apollo::cybertron::message::PyMessageWrap,
                                 apollo::cybertron::message::PyMessageWrap>>
      service_;
  std::mutex msg_lock_;
...
class PyClient {
 public:
  PyClient(const std::string &name, const std::string &data_type,
    ...
    client_ =
        node_->CreateClient<apollo::cybertron::message::PyMessageWrap,
  ...
  std::string send_request(std::string request) {
    std::shared_ptr<apollo::cybertron::message::PyMessageWrap> m;
    m.reset(new apollo::cybertron::message::PyMessageWrap(request, data_type_));
 ...
 private:
  ...
  std::string data_type_;
  std::shared_ptr<
      apollo::cybertron::Client<apollo::cybertron::message::PyMessageWrap,
                                apollo::cybertron::message::PyMessageWrap>>
      client_;
};
...
class PyNode {
 public:
  ...
  void register_message(const std::string &desc) {
    apollo::cybertron::message::ProtobufFactory::Instance()
        ->RegisterPythonMessage(desc);
  }
 ...
 private:
  ...
  std::unique_ptr<apollo::cybertron::Node> node_ = nullptr;
};
