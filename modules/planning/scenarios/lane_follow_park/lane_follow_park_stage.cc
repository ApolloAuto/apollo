...
StageResult LaneFollowParkStage::Process(const TrajectoryPoint& planning_start_point, Frame* frame) {
    ...
    for (auto& reference_line_info : *frame->mutable_reference_line_info()) {
        ...
        bool escape = LaneEscapeUtil::IsNeedEscape(
                reference_line_info,
                scenario_config_.min_distance_block_obs_to_junction(),
                scenario_config_.enable_junction_borrow(),
                scenario_config_.passby_min_gap(),
                scenario_config_.passby_kappa_ratio(),
                scenario_config_.queue_check_count(),
                scenario_config_.stable_block_count());
    ...
    AINFO << "need escape: " << need_escape;
    if (need_escape == 1) {
        ParkDataCenter::Instance()->set_need_escape(true);
        next_stage_.clear();
        return StageResult(StageStatusType::FINISHED);
    }
