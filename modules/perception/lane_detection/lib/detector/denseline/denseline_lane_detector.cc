 ...
 *****************************************************************************/
#include "modules/perception/lane_detection/lib/detector/denseline/denseline_lane_detector.h"
...
bool DenselineLaneDetector::Init(const LaneDetectorInitOptions &options) {
  ...
  if (!cyber::common::GetProtoFromFile(config_file, &denseline_param_)) {
    AINFO << "load proto param failed, root dir: " << config_file;
    return false;
  }

  AINFO << "denseline param: " << denseline_param_.DebugString();
