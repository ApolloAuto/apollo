...
//		packet_end_time = gps_base_usec_ * 1000 - (packets_size - num - 1)*1726*POINTS_PER_PACKET;
//  else
  ...
  for (size_t point_idx = 0; point_idx < POINTS_PER_PACKET; point_idx++) 
  {
	...
	for (size_t point_idx = 0; point_idx < POINTS_PER_PACKET; point_idx++) 
	{
		...
		if (config_.calibration()) {
			...
			ComputeCoords2(firings[point_idx].vertical_line, CH64, firings[point_idx].distance, corrections,
