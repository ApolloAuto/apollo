...
TEST_F(Aebwheelimpulse355Test, reset) {
  ...
  EXPECT_EQ(cd.aeb_wheelimpulse_355().flimpulse(), 413);
  EXPECT_EQ(cd.aeb_wheelimpulse_355().flimpulsevalid(), true);
  EXPECT_EQ(cd.aeb_wheelimpulse_355().frimpulse(), 76);
  EXPECT_EQ(cd.aeb_wheelimpulse_355().frimpulsevalid(), false);
  EXPECT_EQ(cd.aeb_wheelimpulse_355().rlimpulse(), 868);
  EXPECT_EQ(cd.aeb_wheelimpulse_355().rlimpulsevalid(), false);
  EXPECT_EQ(cd.aeb_wheelimpulse_355().rrimpulse(), 650);
  EXPECT_EQ(cd.aeb_wheelimpulse_355().rrimpulsevalid(), true);
}
