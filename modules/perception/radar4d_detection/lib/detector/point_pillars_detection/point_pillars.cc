...
const int PointPillars::kGridSize = Params::kGridSize;
const int PointPillars::kNumAnchorPerGrid = Params::kNumAnchorPerGrid;
...
PointPillars::PointPillars(const bool reproduce_result_mode,
                           ...
                           const common::Framework& model_type,
...
void PointPillars::GenerateAnchors(float* anchors_px_, float* anchors_py_,
  ...
  for (size_t head = 0; head < kNumAnchorSets.size(); ++head) {
    float x_stride = (kMaxXRange - kMinXRange) / (kGridSize - 1);
    float y_stride = (kMaxYRange - kMinYRange) / (kGridSize - 1);
    ...
    float x_offset = kMinXRange;
    float y_offset = kMinYRange;
...
void PointPillars::InitTorch() {
  // Init inference and data shape
  inference_.reset(inference::CreateInferenceByName(
      model_type_, model_file_, "", output_blob_names_, input_blob_names_));
  ...
  shape_map.emplace(std::pair<std::string, std::vector<int>>(
      input_blob_names_[1], {kMaxNumPillars}));
  shape_map.emplace(std::pair<std::string, std::vector<int>>(
      input_blob_names_[2], {kMaxNumPillars, 4}));

  shape_map.emplace(std::pair<std::string, std::vector<int>>(
      output_blob_names_[0],
      {1, kGridSize, kGridSize, kNumAnchorPerGrid * kNumClass}));
  shape_map.emplace(std::pair<std::string, std::vector<int>>(
      output_blob_names_[1],
      {1, kGridSize, kGridSize, kNumAnchorPerGrid * 7}));
  shape_map.emplace(std::pair<std::string, std::vector<int>>(
      output_blob_names_[2],
      {1, kGridSize, kGridSize, kNumAnchorPerGrid * 2}));

  ACHECK(inference_->Init(shape_map)) << "Failed to init inference.";
}
...
void PointPillars::DoInference(const float* in_points_array,
  ...
  // put input data to inferece_ blob
  auto pillar_point_feature = inference_->get_blob(input_blob_names_.at(0));
  auto points_per_pillar = inference_->get_blob(input_blob_names_.at(1));
  auto pillar_coors = inference_->get_blob(input_blob_names_.at(2));
  cudaStream_t stream;
  GPU_CHECK(cudaStreamCreate(&stream));
  GPU_CHECK(cudaMemcpyAsync(pillar_point_feature->mutable_gpu_data(),
                            dev_pillar_point_feature_,
                            kMaxNumPillars * kMaxNumPointsPerPillar *
                                kNumPointFeature * sizeof(float),
                            cudaMemcpyDeviceToDevice, stream));
  GPU_CHECK(cudaMemcpyAsync(points_per_pillar->mutable_gpu_data(),
                            dev_num_points_per_pillar_,
                            kMaxNumPillars * sizeof(float),
                            cudaMemcpyDeviceToDevice, stream));
  GPU_CHECK(cudaMemcpyAsync(pillar_coors->mutable_gpu_data(),
                            dev_pillar_coors_,
                            kMaxNumPillars * 4 * sizeof(float),
                            cudaMemcpyDeviceToDevice, stream));
  ...
  auto cls_score = inference_->get_blob(output_blob_names_.at(0));
  auto bbox_pred = inference_->get_blob(output_blob_names_.at(1));
  auto dir_cls_preds = inference_->get_blob(output_blob_names_.at(2));

  GPU_CHECK(cudaMemset(dev_filter_count_, 0, sizeof(int)));
  postprocess_cuda_ptr_->DoPostprocessCuda(
      bbox_pred->gpu_data(),
      cls_score->gpu_data(),
      dir_cls_preds->gpu_data(),
