 ...
 *****************************************************************************/
#include "modules/perception/common/lib/calibrator/laneline/laneline_calibrator.h"
...
bool LaneLineCalibrator::Calibrate(const CalibratorOptions &options,
  ...
  algorithm::IMultAx3x4(p2w, cam_ori, cam_coord_cur_);
  ...
  for (size_t i = 0; i < lane_objects.size(); ++i) {
    if (lane_objects[i].pos_type == base::LaneLinePositionType::EGO_LEFT) {
      ...
      for (int j = 0; j < num_points; ++j) {
        ...
        x_curve = curve.a * algorithm::ICub(y_curve) +
                  curve.b * algorithm::ISqr(y_curve) + curve.c * y_curve +
                  curve.d;
