...
bool ObjMapper::SolveCenterFromNearestVerticalEdge(const float *bbox,
  ...
  if (algorithm::IRound(bbox[3]) >= height_ - 1) {
...
bool ObjMapper::Solve3dBboxGivenOneFullBboxDimensionOrientation(
  ...
  const float small_angle_diff =
      algorithm::IDegreeToRadians(params_.angle_resolution_degree);
  float center_2d[2] = {0};
  bool success =
  ...
  bool truncated = bbox[0] <= min_x || bbox[2] >= max_x || bbox[1] <= min_y ||
                   bbox[3] >= max_y;
  float dist_rough =
      sqrtf(algorithm::ISqr(center[0]) + algorithm::ISqr(center[2]));
...
bool ObjMapper::Solve3dBbox(const ObjMapperOptions &options, float center[3],
  ...
  *ry = options.ry;
  ...
  if (options.is_veh && check_dimension) {
    ...
    float shrink_ratio_vol = algorithm::ISqr(sqrtf(params_.iou_high));
  ...
  float yaw_score_mean =
      algorithm::IMean(ry_score_.data(), static_cast<int>(ry_score_.size()));
  float yaw_score_sdv = algorithm::ISdv(ry_score_.data(), yaw_score_mean,
                                        static_cast<int>(ry_score_.size()));
  var_yaw =
      algorithm::ISqrt(algorithm::IRec(yaw_score_sdv + params_.eps_mapper));
  ...
  for (float z_test = z_start; z_test <= z_end; z_test += dz) {
    ...
    float sf = z_test * algorithm::IRec(center_test[2]);
    algorithm::IScale3(center_test, sf);
    float score_test = GetProjectionScore(*ry, bbox, hwl, center_test);
    score_z[count_z_test++] = score_test;
  }
  float z_score_mean = algorithm::IMean(score_z, count_z_test);
  float z_score_sdv = algorithm::ISdv(score_z, z_score_mean, count_z_test);
  var_z = algorithm::ISqr(algorithm::IRec(z_score_sdv + params_.eps_mapper));
  ...
  float sf_z_to_x = fabsf(bbox_cx - k_mat_[2]) * algorithm::IRec(focal);
  float var_x = var_z * algorithm::ISqr(sf_z_to_x);
...
void ObjMapper::PostRefineOrientation(const float *bbox, const float *hwl,
  ...
  algorithm::IUnitize(ry_score_.data(), kNrBinsRy);
...
void ObjMapper::GetCenter(const float *bbox, const float &z_ref,
  ...
  while (!stop) {
    algorithm::IBackprojectCanonical(x, k_mat_, z_ref, center_test);
    ...
    for (int i = 0; i < 8; ++i) {
      ...
      algorithm::IProjectThroughKRt(k_mat_, rot, center_test, x_box, x_proj);
    ...
    float cost_cur = algorithm::ISqrt(algorithm::ISqr(x_cur[0] - x_target[0]) +
                                      algorithm::ISqr(x_cur[1] - x_target[1]));
