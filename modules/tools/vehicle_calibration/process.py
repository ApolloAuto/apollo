...
def process(data):
    ...
    transition = np.where(
        np.logical_or(
            np.diff(
                data['ctlbrake']) != 0, np.diff(
                data['ctlthrottle']) != 0))[0]
    ...
    for i in range(len(transition) - 1):
        # print "process transition index:", data['time'][transition[i]], ":",
        # data['time'][transition[i + 1]]
        ...
        # print "Speed min, max", speedmin, speedmax, is_increase, firstindex,
        # lastindex, speedsection[-1]
        accvalue = []
