...
void Brakerpt6c::Parse(const std::uint8_t* bytes, int32_t length,
  ...
  chassis->mutable_gem()->mutable_brake_rpt_6c()->set_commanded_value(commanded_value(bytes, length));
...
// config detail: {'name': 'brake_on_off', 'enum': {0: 'BRAKE_ON_OFF_OFF', 1: 'BRAKE_ON_OFF_ON'}, 'precision': 1.0, 'len': 1, 'is_signed_var': False, 'offset': 0.0, 'physical_range': '[0|1]', 'bit': 48, 'type': 'enum', 'order': 'motorola', 'physical_unit': ''}
Brake_rpt_6c::Brake_on_offType Brakerpt6c::brake_on_off(const std::uint8_t* bytes, int32_t length) const {
  Byte t0(bytes + 6);
