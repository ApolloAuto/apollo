...
class ObstacleReference {
 public:
  ...
  void Init(const ReferenceParam &ref_param, float width, float height);
  void UpdateReference(const CameraTrackingFrame *frame,
 ...
 private:
  void SyncGroundEstimator(const std::string &sensor,
    ...
    if (ground_estimator_mapper_.find(sensor) ==
      ...
      ground_estimator.Init(k_mat, img_width, img_height, algorithm::IRec(fx));
    }
  }
