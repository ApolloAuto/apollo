...
bool EightRoute::is_eight_route_pose(std::vector<FramePose> & poses, int pose_index) {
    if ( poses.size() == 0 || pose_index <= 0 || pose_index >= (int)poses.size() ) {
        AINFO << "params error. poses size:" << poses.size() << ",pose_index:" << pose_index;
        return true;
    }
   
    double yaw = get_yaw(poses[pose_index-1].tx, poses[pose_index-1].ty, poses[pose_index].tx, poses[pose_index].ty);
    ...
    AINFO << std::to_string(poses[pose_index].time_stamp) << ",yaw_diff:" << yaw_diff << ",dist:" << dist << ",during:" << during << ",vel:" << vel;
...
/*
double EightRoute::get_hz(std::vector<FramePose> & poses) {
    int start_index = time_to_index(poses, _start_time);
    int end_index = time_to_index(poses, _end_time);
    if ( start_index < 0 ) {
        AINFO << "start_time error. start_time:" << _start_time << ",start_index:" << start_index;
        return -1;
    }

    int size = poses.size();
    double eight_route_during;
    if ( end_index > 0 && end_index != size ) {
        eight_route_during = poses[end_index].time_stamp - poses[start_index].time_stamp;
    } else {
        eight_route_during = poses.back().time_stamp - poses[start_index].time_stamp;
    }
    if ( eight_route_during < 1e-8 ) {
        AINFO << "duration too smalll: " << eight_route_during;
    }
    double hz = (end_index - start_index + 1) / eight_route_during;
    return hz;
}

bool EightRoute::is_bad_pose_overload() {
    if ( _bad_pose_info.end_time > 0 
         && _bad_pose_info.end_time - _bad_pose_info.start_time > _sp_conf->eight_bad_pose_tolerance ) {
        return true;
    }
    return false;
}*/
...
double EightRoute::get_good_pose_during() {
    if ( _good_pose_info.start_time < 0 || _good_pose_info.end_time < 0 ) {
        return 0.0;
    }
    return _good_pose_info.end_time - _good_pose_info.start_time;
}

double EightRoute::get_eight_route_progress(std::vector<FramePose> & poses) {
    //double hz = get_hz(poses);
    int size = (int)poses.size();
    int start_index = time_to_index(poses, _start_time);
    // select first good pose
    while(start_index < size) {
        if (is_good_pose(poses, start_index) && is_eight_route_pose(poses, start_index)) {
    ...
    if ( start_index >= size ) {
        AINFO << "not find first good pose.start_time:" << _start_time << ",start_index:" << start_index << ",pose size:" << size;
        return 0.0;
    }
    ...
    _last_yaw = get_yaw(poses[start_index].tx, poses[start_index].ty, poses[start_index+1].tx, poses[start_index+1].ty);
    ...
    for(int i = start_index + 2; i < size; i++) {
        ...
        if (!is_eight_route_pose(poses, i)) {
            ...
            if (not_eight_count > _sp_conf->eight_bad_pose_tolerance) {
                AINFO << "not-eight route pose count reached upper limitation"; 
    ...
    if ( eight_route_during < 1e-8) {
        ...
        _return_state = ErrorCode::SUCCESS; // TODO 
        return 0.0;
    }
...
ErrorCode EightRoute::process(std::vector<FramePose>& poses) {
    ...
    if( _progress < _last_progress ) {
        _return_state = ErrorCode::ERROR_NOT_EIGHT_ROUTE; // update _last_progress or not ?
        return _return_state;
    }
