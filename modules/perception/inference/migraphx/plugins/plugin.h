...
//!
//! \note: Currently the following formats are supported
//! for layer inputs and outputs:
//! * zero or more index dimensions followed
//! by one channel and two spatial dimensions (e.g. CHW)
//! * one time series dimension followed
//! by one index dimension followed by one channel dimension (i.e. TNC)
//!
//! TensorRT can also return an invalid dims structure.
//! This structure is represented by nbDims == -1
//! and d[i] == 0 for all d.
//!
//! TensorRT can also return an "unknown rank" dims structure.
//! This structure is represented by nbDims == -1
//! and d[i] == -1 for all d.
//!
class Dims {
