 ...
 *****************************************************************************/
#include "modules/perception/lidar_detection/detector/point_pillars_detection/point_pillars_detection.h"
...
bool PointPillarsDetection::Init(const LidarDetectorInitOptions& options) {
  ...
  if (param_.preprocess().enable_ground_removal()) {
    z_min_ = std::max(z_min_,
                      static_cast<float>(
...
bool PointPillarsDetection::Detect(const LidarDetectorOptions& options,
  ...
  if (param_.preprocess().enable_downsample_beams()) {
    base::PointFCloudPtr downsample_beams_cloud_ptr(new base::PointFCloud());
    if (DownSamplePointCloudBeams(
          original_cloud_, downsample_beams_cloud_ptr,
          param_.preprocess().downsample_beams_factor())) {
  ...
  cur_cloud_ptr_->mutable_points_timestamp()->assign(cur_cloud_ptr_->size(),
                                                     0.0);
  if (param_.preprocess().enable_fuse_frames() &&
  ...
  if (param_.preprocess().enable_shuffle_points()) {
    num_points = std::min(num_points, param_.preprocess().max_num_points());
  ...
  CloudToArray(cur_cloud_ptr_, points_array,
...
void PointPillarsDetection::CloudToArray(const base::PointFCloudPtr& pc_ptr,
  ...
  for (size_t i = 0; i < pc_ptr->size(); ++i) {
    ...
    out_points_array[i * param_.preprocess().num_point_feature() + 0] = x;
    out_points_array[i * param_.preprocess().num_point_feature() + 1] = y;
    out_points_array[i * param_.preprocess().num_point_feature() + 2] = z;
    out_points_array[i * param_.preprocess().num_point_feature() + 3] =
        intensity / normalizing_factor;
    // delta of timestamp between prev and cur frames
    out_points_array[i * param_.preprocess().num_point_feature() + 4] =
