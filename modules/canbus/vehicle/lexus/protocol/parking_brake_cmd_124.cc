...
Parkingbrakecmd124* Parkingbrakecmd124::set_enable(
    bool enable) {
  enable_ = enable;
  return this;
 }
...
void Parkingbrakecmd124::set_p_enable(uint8_t* data,
    bool enable) {
  int x = enable;
...
// config detail: {'name': 'PARKING_BRAKE_CMD', 'offset': 0.0, 'precision': 1.0, 'len': 1, 'is_signed_var': False, 'physical_range': '[0|1]', 'bit': 8, 'type': 'bool', 'order': 'motorola', 'physical_unit': ''}
...
Parkingbrakecmd124* Parkingbrakecmd124::set_clear_faults(
    bool clear_faults) {
...
void Parkingbrakecmd124::set_p_clear_faults(uint8_t* data,
    bool clear_faults) {
  int x = clear_faults;
