...
const int Lka768::ID = 0x768;

void Lka768::Parse(const uint8_t* bytes, int32_t length,
                   Mobileye* mobileye) const {
  mobileye->mutable_lka_768()->set_lane_type(lane_type(bytes, length));
  mobileye->mutable_lka_768()->set_quality(quality(bytes, length));
  mobileye->mutable_lka_768()->set_model_degree(model_degree(bytes, length));
  mobileye->mutable_lka_768()->set_position(position(bytes, length));
  mobileye->mutable_lka_768()->set_curvature(curvature(bytes, length));
  mobileye->mutable_lka_768()->set_curvature_derivative(
      curvature_derivative(bytes, length));
  mobileye->mutable_lka_768()->set_width_right_marking(
      width_right_marking(bytes, length));
}
...
int Lka768::lane_type(const uint8_t* bytes, int32_t length) const {
  Byte t0(bytes + 0);
...
int Lka768::quality(const uint8_t* bytes, int32_t length) const {
  Byte t0(bytes + 0);
...
int Lka768::model_degree(const uint8_t* bytes, int32_t length) const {
  Byte t0(bytes + 0);
...
// '"meter"'}
double Lka768::position(const uint8_t* bytes, int32_t length) const {
  Byte t0(bytes + 2);
...
double Lka768::curvature(const uint8_t* bytes, int32_t length) const {
  Byte t0(bytes + 4);
  int32_t x = t0.get_byte(0, 8);
...
double Lka768::curvature_derivative(const uint8_t* bytes,
...
// '[0|2.55]', 'bit': 56, 'type': 'double', 'order': 'intel', 'physical_unit':
// '"m"'}
double Lka768::width_right_marking(const uint8_t* bytes, int32_t length) const {
  Byte t0(bytes + 7);
  int32_t x = t0.get_byte(0, 8);
