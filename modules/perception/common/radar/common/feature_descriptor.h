...
class FeatureDescriptor {
 public:
  explicit FeatureDescriptor(base::RadarPointFCloud* cloud) { cloud_ = cloud; }
  FeatureDescriptor() : cloud_(nullptr) {}
  ~FeatureDescriptor() = default;

  void SetCloud(base::RadarPointFCloud* cloud) { cloud_ = cloud; }
  ...
  void ComputeHistogram(int bin_size, float* feature) {
    ...
    for (int i = 0; i < pt_num; ++i) {
      base::RadarPointF& pt = cloud_->at(i);
 ...
 private:
  ...
  base::RadarPointFCloud* cloud_;
  base::RadarPointF min_pt_;
  base::RadarPointF max_pt_;
  base::RadarPointF center_pt_;
};  // class FeatureDescriptor
