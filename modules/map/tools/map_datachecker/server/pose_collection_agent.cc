...
void PoseCollectionAgent::on_bestgnsspos_callback(
  ...
  sp_pj_transformer_->latlong_to_utm(1, 1, &pose.tx, &pose.ty, &pose.tz);
...
std::shared_ptr<std::vector<FramePose>> PoseCollectionAgent::get_poses() const {
  if (sp_pose_collection_ == nullptr) {
    return nullptr;
  }
  return sp_pose_collection_->get_poses();
}
