...
class DecoderBase {
 public:
  DecoderBase(const RS_Param &param);
  virtual ~DecoderBase();
  virtual E_DECODER_RESULT processMsopPkt(const uint8_t *pkt,
                                          std::vector<vpoint> &pointcloud_vec,
                                          int &height);
 ...
 protected:
  ...
  virtual int32_t decodeMsopPkt(const uint8_t *pkt, std::vector<vpoint> &vec,
                                int &height) = 0;
...
E_DECODER_RESULT DecoderBase<vpoint>::processMsopPkt(
    const uint8_t *pkt, std::vector<vpoint> &pointcloud_vec, int &height) {
  if (pkt == NULL) {
