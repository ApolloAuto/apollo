...
bool TrafficLightDetection::Init(
  ...
  std::vector<int> shape_input = {max_batch_size_, resize_height,
                                  resize_width, 3};
  ...
  mean_buffer_.reset(new base::Blob<float>(1,
                                           resize_height,
                                           resize_height,
                                           3));
  ...
  image_.reset(new base::Image8U(resize_height,
                                 resize_width,
                                 base::Color::BGR));
  return true;
}
...
bool
TrafficLightDetection::Inference(std::vector<base::TrafficLightPtr> *lights,
...
bool TrafficLightDetection::Detect(
    const TrafficLightDetectorOptions& options, CameraFrame* frame) {
  ...
  for (auto &light : lights_ref) {
    if (light->region.outside_image ||
        OutOfValidRegion(light->region.projection_roi,
...
bool TrafficLightDetection::SelectOutputBoxes(
  ...
  for (int candidate_id = 0; candidate_id < result_box_num; candidate_id++) {
    ...
    std::vector<float>::iterator biggest = std::max_element(std::begin(score),
                                                            std::end(score));
    tmp->region.detect_class_id = base::TLDetectionClass(
        std::distance(std::begin(score), biggest) - 1);
    ...
    if (static_cast<int>(tmp->region.detect_class_id) >= 0) {
      ...
      RefineBox(tmp->region.detection_roi,
          crop_box_list.at(img_id).width,
          crop_box_list.at(img_id).height,
...
void TrafficLightDetection::ApplyNMS(
    std::vector<base::TrafficLightPtr>* lights, double iou_thresh) {
  ...
  int idx = 0;
  auto parted_itr = std::stable_partition(lights->begin(), lights->end(),
