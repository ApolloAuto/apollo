 ...
 *****************************************************************************/
#include "modules/perception/submodules/camera_obstacle_detection_submodule.h"
...
CameraObstacleDetectionSubmodule::~CameraObstacleDetectionSubmodule() {}

bool CameraObstacleDetectionSubmodule::Init() {
...
void CameraObstacleDetectionSubmodule::OnReceiveImage(
  ...
  AINFO << "Enter CameraObstacleDetectionSubmodule::Proc(), "
...
int CameraObstacleDetectionSubmodule::InitConfig() {
  ...
  std::string camera_names_str = camera_detection_param.camera_names();
  boost::algorithm::split(camera_names_, camera_names_str,
                          boost::algorithm::is_any_of(","));
  if (camera_names_.size() != 2) {
    AERROR << "Now CameraObstacleDetectionSubmodule only support 2 cameras";
    return cyber::FAIL;
  }

  std::string input_camera_channel_names_str =
      camera_detection_param.input_camera_channel_names();
  ...
  camera_perception_init_options_.root_dir =
      camera_detection_param.camera_obstacle_perception_conf_dir();
  camera_perception_init_options_.conf_file =
      camera_detection_param.camera_obstacle_perception_conf_file();
  camera_perception_init_options_.lane_calibration_working_sensor_name =
      camera_detection_param.lane_calibration_working_sensor_name();
  camera_perception_init_options_.use_cyber_work_root = true;
  frame_capacity_ = camera_detection_param.frame_capacity();
  image_channel_num_ = camera_detection_param.image_channel_num();
  enable_undistortion_ = camera_detection_param.enable_undistortion();
  enable_visualization_ = camera_detection_param.enable_visualization();
  ...
  image_based_cipv_ =
      static_cast<float>(camera_detection_param.image_based_cipv());

  debug_level_ = static_cast<int>(camera_detection_param.debug_level());
  enable_cipv_ = camera_detection_param.enable_cipv();

  std::string format_str = R"(
      CameraObstacleDetectionSubmodule InitConfig success
      camera_names:    %s, %s
...
int CameraObstacleDetectionSubmodule::InitSensorInfo() {
  if (camera_names_.size() != 2) {
...
int CameraObstacleDetectionSubmodule::InitAlgorithmPlugin() {
...
int CameraObstacleDetectionSubmodule::InitCameraFrames() {
  if (camera_names_.size() != 2) {
    AERROR << "invalid camera_names_.size(): " << camera_names_.size();
    return cyber::FAIL;
  }
...
int CameraObstacleDetectionSubmodule::InitProjectMatrix() {
...
int CameraObstacleDetectionSubmodule::InitCameraListeners() {
  for (size_t i = 0; i < camera_names_.size(); ++i) {
    ...
    std::function<void(const ImageMsgType &)> camera_callback =
        std::bind(&CameraObstacleDetectionSubmodule::OnReceiveImage, this,
...
int CameraObstacleDetectionSubmodule::InitMotionService() {
  ...
  std::function<void(const MotionServiceMsgType &)> motion_service_callback =
      std::bind(&CameraObstacleDetectionSubmodule::OnMotionService, this,
                std::placeholders::_1);
...
void CameraObstacleDetectionSubmodule::OnMotionService(
...
void CameraObstacleDetectionSubmodule::SetCameraHeightAndPitch() {
  camera_obstacle_pipeline_->SetCameraHeightAndPitch(
      camera_height_map_, name_camera_pitch_angle_diff_map_,
      default_camera_pitch_);
}

int CameraObstacleDetectionSubmodule::InternalProc(
...
int CameraObstacleDetectionSubmodule::MakeProtobufMsg(
...
int CameraObstacleDetectionSubmodule::ConvertObjectToPb(
...
int CameraObstacleDetectionSubmodule::ConvertObjectToCameraObstacle(
    const base::ObjectPtr &object_ptr,
...
int CameraObstacleDetectionSubmodule::ConvertLaneToCameraLaneline(
...
int CameraObstacleDetectionSubmodule::MakeCameraDebugMsg(
...
bool CameraObstacleDetectionSubmodule::SetCameraHeight(
