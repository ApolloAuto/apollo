...
double Select::Calc2dGaussianScore(base::Point2DI p1, base::Point2DI p2, float sigma1, float sigma2) {
    return std::exp(
            -0.5
...
void Select::SelectTrafficLights(
    ...
    for (size_t row = 0; row < hdmap_bboxes->size(); ++row) {
        ...
        for (size_t col = 0; col < refined_bboxes.size(); ++col) {
            ...
            double distance_score = Calc2dGaussianScore(center_hd, center_refine, gaussian_score, gaussian_score);
            ...
            if ((detection_roi & crop_roi) != detection_roi) {
                AINFO << "detection_roi outside crop_roi, set score to 0."
                      << " detection_roi: " << detection_roi.x << " " << detection_roi.y << " " << detection_roi.width
                      << " " << detection_roi.height << " crop_roi: " << crop_roi.x << " " << crop_roi.y << " "
