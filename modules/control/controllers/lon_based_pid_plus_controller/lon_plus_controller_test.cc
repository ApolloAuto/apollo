...
const char data_path[]
        = "/apollo/modules/control/controllers/lon_based_pid_plus_controller/longitudinal_controller_test/";

class LonPLusControllerTest : public ::testing::Test, LonPlusController {
public:
    virtual void SetUp() {
        FLAGS_v = 3;
        std::string controllers_dir = "/apollo/modules/control/controllers/lon_based_pid_plus_controller/";
        std::string control_conf_file = controllers_dir + "longitudinal_controller_test/conf/controller_conf.pb.txt";

        ACHECK(cyber::common::GetProtoFromFile(control_conf_file, &longitudinal_conf_));

        timestamp_ = Time::Now().ToSecond();

        controller_.reset(new LonPlusController());
    ...
    void ComputeLongitudinalErrors(
        ...
        LonPlusController::ComputeLongitudinalErrors(trajectory, preview_time, ts, debug);
    }

    common::Status Init() {
        return LonPlusController::Init(injector_);
    }

protected:
    LocalizationPb LoadLocalizationPb(const std::string &filename) {
        LocalizationPb localization;
        ACHECK(cyber::common::GetProtoFromFile(filename, &localization)) << "Failed to open file " << filename;
    ...
    ChassisPb LoadChassisPb(const std::string &filename) {
        ...
        ACHECK(cyber::common::GetProtoFromFile(filename, &chassis_pb)) << "Failed to open file " << filename;
    ...
    TrajectoryPb LoadPlanningTrajectoryPb(const std::string &filename) {
        ...
        ACHECK(cyber::common::GetProtoFromFile(filename, &trajectory_pb)) << "Failed to open file " << filename;

        trajectory_pb.mutable_header()->set_timestamp_sec(timestamp_);
        return trajectory_pb;
    }

    LonBasedPidPlusControllerConf longitudinal_conf_;
    std::unique_ptr<LonPlusController> controller_;
    double timestamp_ = 0.0;
};

TEST_F(LonPLusControllerTest, ComputeLongitudinalErrors) {
    ...
    double preview_station_error_expected = 0.5141842423 - station_reference_expected;
    double preview_speed_reference_expected = 1.8638888597500001;
    double preview_speed_error_expected = 1.8638888597500001;
    double preview_acceleration_reference_expected = 0.57851417037899999;
    ...
    EXPECT_NEAR(debug.preview_acceleration_reference(), preview_acceleration_reference_expected, 0.001);
