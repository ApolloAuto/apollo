...
namespace idl
{
  /*This routine force a 2x2 orthogonal matrix R in place (i.e., force it to be rotation maxtrix with positive determinant), 
  ...
  /*This routine orthogonalizes a 3x3 near Rotation matrix R in place, i.e., force it to be orthogonal.*/
  template <typename T>
...
/*Rodrigues' solver for computing the matrix exponential of a 3x3 skew-symmetric matrix*/
template <typename T>
inline void i_rot_rodrigues_3x3_solver(T sinc, T mcos, T a0_sqr, T a1_sqr, T a2_sqr, const T a[3], T R[9])
{
  T tmp1, tmp2;
  R[0] = (T)1.0 - (a1_sqr + a2_sqr)*mcos;
  R[4] = (T)1.0 - (a0_sqr + a2_sqr)*mcos;
  R[8] = (T)1.0 - (a0_sqr + a1_sqr)*mcos;
  tmp1 = a[0] * a[1] * mcos; tmp2 = a[2] * sinc; R[1] = tmp1 - tmp2; R[3] = tmp1 + tmp2;
  tmp1 = a[2] * a[0] * mcos; tmp2 = a[1] * sinc; R[2] = tmp1 + tmp2; R[6] = tmp1 - tmp2;
  tmp1 = a[1] * a[2] * mcos; tmp2 = a[0] * sinc; R[5] = tmp1 - tmp2; R[7] = tmp1 + tmp2;
}

template <typename T>
inline void i_rot_rodrigues_3x3_solver(T sinc, T mcos, T a0_sqr, T a1_sqr, T a2_sqr, const T s_da[3], const T c_da[3], const T a[3], T R[9], T D[9][3])
{
  ...
  tmp1 = a[0] * mcos_x_a1; tmp2 = a[2] * sinc; R[1] = tmp1 - tmp2; R[3] = tmp1 + tmp2;
  tmp1 = a[2] * mcos_x_a0; tmp2 = a[1] * sinc; R[2] = tmp1 + tmp2; R[6] = tmp1 - tmp2;
  tmp1 = a[1] * mcos_x_a2; tmp2 = a[0] * sinc; R[5] = tmp1 - tmp2; R[7] = tmp1 + tmp2;
  ...
  /*Rodrigues' formula for computing the matrix exponential of a 3x3 skew-symmetric matrix
    The 1st argument is the input (a 3-vector) and the 2nd argument is the output (a 3x3 rotation matrix)*/
template <typename T>
inline void i_rot_rodrigues_3x3(const T a[3], T R[9])
{
  ...
  {
    /*theta is too small; use a third-order Taylor approximation for sin(theta) and cos(theta)*/
    sinc = (T)1.0 - theta2 / (T)6.0;
    mcos = (T)0.5 - theta2 / (T)24.0;
  }
  else if (theta > Constant<T>::PI())
  {
    /*if ||a||>PI, the it maybe replaced by a(1-2PI/||a||), which represenrs the same rotation*/
...
/*Rodrigues' formula for computing the matrix exponential of a 3x3 skew-symmetric matrix
The 1st argument is the input (a 3-vector),
the 2nd argument is the output (a 3x3 rotation matrix),
The 3rd argument is the derivative (a 9x3 matrix)*/
template <typename T>
...
the 2nd argument is the output (a 3x3 rotation matrix),
The 3rd argument is the derivative (a 9x3 matrix)
Keep this slow implementation as baseline - Liang July/31/2015*/
template <typename T>
inline void i_rot_rodrigues_3x3_slow(const T a[3], T R[9], T D[9][3])
{
  ...
  /*for (int i = 0; i<9; ++i)
  {
  D[i][0] = (T)(e_x[i] * s_da[0] + sinc*e_x_da[i][0] + e_x2[i] * c_da[0] + mcos * e_x2_da[i][0]);
  D[i][1] = (T)(e_x[i] * s_da[1] + sinc*e_x_da[i][1] + e_x2[i] * c_da[1] + mcos * e_x2_da[i][1]);
  D[i][2] = (T)(e_x[i] * s_da[2] + sinc*e_x_da[i][2] + e_x2[i] * c_da[2] + mcos * e_x2_da[i][2]);
  }*/
...
/*The inverse Rodrigues' formula for computing the logarithm of a 3x3 rotation matrix
  The 1st argument is the input (a 3x3 rotation matrix) and the 2nd argument is the output (a 3-vector rotation vector)*/
template <typename T>
inline void i_rot_invert_rodrigues_3x3(const T R[9], T v[3]/*unnormalized rotation vector*/, T &theta/*rotation angle in radians*/)
{
  T r[3]; 
  ...
  /*compute the eigenvector of R corresponding to the eigenvalue 1, every rotation matrix must have this eigenvalue*/
  i_eigenvector_from_eigenvalue(R_ref, (T)(1.0), Q_ref, h, htA, ss, Ac_ref, 3, iv);
  i_copy3(Q + 6, v);/*eigen vector of R, unit rotation axis*/
  r[0] = R[7] - R[5];
  r[1] = R[2] - R[6];
  r[2] = R[3] - R[1];
  T c = (i_trace_3x3(R) - (T)1.0) * (T)0.5; /*the trace of R is 1 + 2\cos(\theta), equivalent to the sum of its eigenvalues*/
  T s = i_dot3(r, v) * (T)0.5;
  theta = i_atan2(s, c);
  i_scale3(v, theta);
}

/*This routine finds a rotation matrix R that rotates unit vector a onto unit vector b. 
  a and b are both assumed to be unit vectors. Corner cases are a and b are parallel*/
template<typename T>
...
/*Generate a random 3x3 rotation matrix R, if force_proper is set to 1, then R is forced to be a proper rotation matrix with det(R) = 1*/
template <typename T>
