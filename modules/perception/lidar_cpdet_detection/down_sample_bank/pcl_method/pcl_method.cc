...
#include "modules/perception/lidar_cpdet_detection/down_sample_bank/pcl_method/pcl_method.h"
...
bool PclPCDownSample::Init(const DownSampleInitOptions& options) {
  std::string config_file =
      GetConfigFile(options.config_path, options.config_file);
  PclMethodConfig config;
  ...
  AINFO << "PclPCDownSample plugin init success.";
  return true;
}

bool PclPCDownSample::Process(const DownSampleOptions& options,
