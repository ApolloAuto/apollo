...
bool TLPreprocessorSubnode::InitInternal() {
  ...
  common::adapter::AdapterManager::AddImageLongCallback(
  ...
  common::adapter::AdapterManager::AddImageShortCallback(
...
void TLPreprocessorSubnode::SubCameraImage(
  ...
  preprocessor_.get_max_cached_image_lights_array_size(
      &max_cached_image_lights_array_size);
  // tf 频率实际为 100Hz, 0.01 秒一帧，一共缓存了
  // max_cached_image_lights_array_size * 0.005 时间的 tf 信息
...
bool TLPreprocessorSubnode::GetSignals(
    double ts, CarPose *pose, std::vector<apollo::hdmap::Signal> *signals) {
  // get pose
...
bool TLPreprocessorSubnode::VerifyLightsProjection(
  ...
  if (!preprocessor_.select_camera_by_lights_projection(
          ts, pose, signals, image_lights, &selected_camera_id)) {
