...
void Status310::Parse(const std::uint8_t* bytes, int32_t length,
                      Wey* chassis) const {
  chassis->mutable_status_310()->set_longitudeaccvalid(
      longitudeaccvalid(bytes, length));
  chassis->mutable_status_310()->set_lateralaccevalid(
      lateralaccevalid(bytes, length));
  chassis->mutable_status_310()->set_vehdynyawratevalid(
      vehdynyawratevalid(bytes, length));
  chassis->mutable_status_310()->set_flwheelspdvalid(
      flwheelspdvalid(bytes, length));
  chassis->mutable_status_310()->set_frwheelspdvalid(
      frwheelspdvalid(bytes, length));
  chassis->mutable_status_310()->set_rlwheelspdvalid(
      rlwheelspdvalid(bytes, length));
  chassis->mutable_status_310()->set_rrwheelspdvalid(
      rrwheelspdvalid(bytes, length));
  chassis->mutable_status_310()->set_vehiclespdvalid(
      vehiclespdvalid(bytes, length));
  chassis->mutable_status_310()->set_longitudedrivingmode(
      longitudedrivingmode(bytes, length));
  chassis->mutable_status_310()->set_engspdvalid(
      engspdvalid(bytes, length));
  chassis->mutable_status_310()->set_accepedaloverride(
      accepedaloverride(bytes, length));
  chassis->mutable_status_310()->set_brakepedalstatus(
      brakepedalstatus(bytes, length));
  chassis->mutable_status_310()->set_espbrakelightsts(
      espbrakelightsts(bytes, length));
  chassis->mutable_status_310()->set_epbswtpositionvalid(
      epbswtpositionvalid(bytes, length));
  chassis->mutable_status_310()->set_epbsts(
      epbsts(bytes, length));
  chassis->mutable_status_310()->set_currentgearvalid(
      currentgearvalid(bytes, length));
  chassis->mutable_status_310()->set_epstrqsnsrsts(
      epstrqsnsrsts(bytes, length));
  chassis->mutable_status_310()->set_eps_interferdetdvalid(
      eps_interferdetdvalid(bytes, length));
  chassis->mutable_status_310()->set_epshandsdetnsts(
      epshandsdetnsts(bytes, length));
  chassis->mutable_status_310()->set_eps_handsdetnstsvalid(
      eps_handsdetnstsvalid(bytes, length));
  chassis->mutable_status_310()->set_steerwheelanglesign(
      steerwheelanglesign(bytes, length));
  chassis->mutable_status_310()->set_steerwheelspdsign(
      steerwheelspdsign(bytes, length));
  chassis->mutable_status_310()->set_driverdoorsts(
      driverdoorsts(bytes, length));
  chassis->mutable_status_310()->set_rldoorsts(
      rldoorsts(bytes, length));
  chassis->mutable_status_310()->set_passengerdoorsts(
      passengerdoorsts(bytes, length));
  chassis->mutable_status_310()->set_rrdoorsts(
      rrdoorsts(bytes, length));
  chassis->mutable_status_310()->set_frontfoglmpsts(
      frontfoglmpsts(bytes, length));
  chassis->mutable_status_310()->set_rearfoglmpsts(
      rearfoglmpsts(bytes, length));
  chassis->mutable_status_310()->set_lowbeamsts(
      lowbeamsts(bytes, length));
  chassis->mutable_status_310()->set_highbeamsts(
      highbeamsts(bytes, length));
  chassis->mutable_status_310()->set_leftturnlampsts(
      leftturnlampsts(bytes, length));
  chassis->mutable_status_310()->set_rightturnlampsts(
      rightturnlampsts(bytes, length));
  chassis->mutable_status_310()->set_bcm_availsts(
