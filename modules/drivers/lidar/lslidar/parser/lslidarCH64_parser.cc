...
void LslidarCH64Parser::Unpack(
    ...
    for (size_t point_idx = 0; point_idx < POINTS_PER_PACKET; point_idx++) {
        ...
        if (line_num % 8 == 0 || line_num % 8 == 1 || line_num % 8 == 2
            || line_num % 8 == 3) {
            z_sin_altitude = sin(-13.33 * DEG_TO_RAD
                                 + floor(line_num / 4) * 1.33 * DEG_TO_RAD)
                    + 2
                            * cos(firings[point_idx].azimuth / 2
                                  + 1.05 * DEG_TO_RAD)
                            * sin((line_num % 4) * 0.33 * DEG_TO_RAD);

        } else if (
                line_num % 8 == 4 || line_num % 8 == 5 || line_num % 8 == 6
                || line_num % 8 == 7) {
            z_sin_altitude = sin(-13.33 * DEG_TO_RAD
                                 + floor(line_num / 4) * 1.33 * DEG_TO_RAD)
                    + 2
        ...
        point_time
                = packet_end_time - 1726 * (POINTS_PER_PACKET - point_idx - 1);
