...
void LslidarParser::setup() {
  ...
  for (uint16_t rot_index = 0; rot_index < ROTATION_MAX_UNITS; ++rot_index) {
		float rotation_f = ROTATION_RESOLUTION * static_cast<float>(rot_index * M_PI) / 180.0f;
...
void LslidarParser::ComputeCoords(const float &raw_distance,
  ...
  double cos_rot_angle = cos_azimuth_table[rotation] * corrections.cos_rot_correction +
      sin_azimuth_table[rotation] * corrections.sin_rot_correction;
  double sin_rot_angle = sin_azimuth_table[rotation] * corrections.cos_rot_correction -
...
void LslidarParser::ComputeCoords2(int Laser_ring, int Type, const float &raw_distance,
                                   LaserCorrection &corrections,
	...
	for (int i = 0; i < 128; i++) {
		...
		if(abs(prism_angle[0]) < 1e-6 && abs(prism_angle[1]) < 1e-6 && abs(prism_angle[2]) < 1e-6 && abs(prism_angle[3]) < 1e-6)
		{
  ...
  if(Type == CH16)
  {
		...
		abs(prism_angle[2]) < 1e-6 && abs(prism_angle[3]) < 1e-6 ){
			corrections.sin_vert_correction = sin_scan_laser_altitude[Laser_ring / 4 + 2] +
  ...
  {
	  corrections.sin_vert_correction=sin_scan_laser_altitude[Laser_ring/4]+2*cos_azimuth_half* sin_scan_mirror_altitude[Laser_ring%4];
  }
  else if(Type == CH64)
  {
	  if (Laser_ring % 8 == 0 || Laser_ring % 8 == 1 || Laser_ring % 8 == 2 || Laser_ring % 8 == 3) {
			corrections.sin_vert_correction = sin(-13.33 * DEG_TO_RAD + floor(Laser_ring / 4) * 1.33* DEG_TO_RAD) +
							 2 * cos(rotation / 2 + 1.05* DEG_TO_RAD ) *
							 sin((Laser_ring % 4) * 0.33* DEG_TO_RAD);
		} 
		else if (Laser_ring % 8 == 4 || Laser_ring % 8 == 5 || Laser_ring % 8 == 6 || Laser_ring % 8 == 7) {
			corrections.sin_vert_correction = sin(-13.33 * DEG_TO_RAD + floor(Laser_ring / 4) * 1.33* DEG_TO_RAD) +
							 2 * cos(rotation / 2 - 1.05* DEG_TO_RAD ) *
							 sin((Laser_ring % 4) * 0.33* DEG_TO_RAD);
		}
  ...
  else if(Type == CH64w)
  {
		...
		_R_ = theat2_c[Laser_ring] * theat1_c[Laser_ring] * cos_xita - theat2_s[Laser_ring] * theat1_s[Laser_ring];
        sin_theat = theat1_s[Laser_ring] + 2 * _R_ * theat2_s[Laser_ring];
        cos_theat = sqrt(1 - pow(sin_theat, 2));
        cos_H_xita = (2 * _R_ * theat2_c[Laser_ring] * cos_xita - theat1_c[Laser_ring]) / cos_theat;
  ...
  else if(Type == CH128)
  {
	  ...
	  corrections.sin_vert_correction = sin_scan_laser_altitude_ch128[Laser_ring / 4] +
                         2 * cos_azimuth_half * sinTheta_2[Laser_ring];
  }
  else if(Type == CH128X1)
  {
		double _R_ = cosTheta_2[Laser_ring] * cosTheta_1[Laser_ring] *
                    cos_azimuth_half - sinTheta_2[Laser_ring] * sinTheta_1[Laser_ring];
		corrections.sin_vert_correction = sinTheta_1[Laser_ring] + 2 * _R_ * sinTheta_2[Laser_ring];
  }
  corrections.cos_vert_correction = sqrt(1 - pow(corrections.sin_vert_correction, 2));
...
LslidarParser *LslidarParserFactory::CreateParser(Config source_config) {
  ...
  else if(config.model() == LSLIDAR_C32_V4 ||config.model() == LSLIDAR_C16_V4
  ||config.model() == LSLIDAR_C8_V4 ||config.model() == LSLIDAR_C1_V4){
