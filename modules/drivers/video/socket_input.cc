...
int SocketInput::get_frame_packet(std::shared_ptr<CompressedImage>& h265Pb) 
{
  ...
  while (true) {
    ...
    //    _pkgNum++;
    //    _bytesNum += (pdu_len - sizeof(RTP_Header));
    //	AINFO << "recive " << pdu_len << " from port " << _port;

    hw_pdu_packet *pdu_pkg = (hw_pdu_packet *)_pdu;
    ...
    if (local_seq - pre_seq != 1 && (pre_seq > 1) && (local_seq > 0)) {
      AERROR << "error: port " << _port << " package sequence is wrong, curent/pre " << local_seq <<
        "/" << pre_seq;
    }
    pre_seq = local_seq;

    //AINFO << "receive " << pdu_len << " from port " << _port << " rtp sequence number is " << local_seq;
    ...
    if (ntohl(pdu_pkg->header.magic0) == HW_CAMERA_MAGIC0 &&
      ...
      if (frame_id - _frame_id != 1 && _frame_id > 1 && frame_id > 1) {
        AERROR << "error: port " << _port << " Frame losing, pre_frame_id/frame_id " << 
          _frame_id << "/" << frame_id;
      }
      _frame_id = frame_id;

      cyber::Time image_time(ntohl(pdu_pkg->header.ts_sec), 1000 * ntohl(pdu_pkg->header.ts_usec));
      //AINFO << "image_time second: " << ntohl(pdu_pkg->header.ts_sec) << " usec: " << ntohl(pdu_pkg->header.ts_usec);
      ...
      h265Pb->set_frame_type((int)pdu_pkg->header.FrameType);
      AINFO << "port " << _port << ", receive frameSize = " << ntohl(pdu_pkg->header.FrameSize) << 
        " frame type = " << (int)pdu_pkg->header.FrameType << " PhyNo = " << (int)pdu_pkg->header.PhyNo <<
        " frame_id = " << frame_id;
      ...
      if (total > 0) {
        pkg_len = (int)(pdu_len - sizeof(RTP_Header));
        memcpy(frame_data, pdu_data + sizeof(RTP_Header), pkg_len);
        total -= pkg_len;
        frame_data += pkg_len;
        //	        AINFO << "receive pkg data " << pkg_len << "/" << total << "/" << frame_len;
      } 
      if (total <= 0){
        total = 0;
        //	        AINFO << "receive frame data " << pkg_len << "/" << total << "/" << frame_len;
        if (frame_len > 0) {
...
bool SocketInput::input_available(int timeout) 
{
  ...
  do {
    ...
    if ((fds[0].revents & POLLERR) || (fds[0].revents & POLLHUP) ||
      ...
      AERROR << "h265 camera port " << _port  << "poll() reports h265camera error";
      return false;
    }
