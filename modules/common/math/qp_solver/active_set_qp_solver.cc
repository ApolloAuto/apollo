...
bool ActiveSetQpSolver::Solve() {
  ...
  double* h_matrix = new double[kNumOfMatrixElements];  // NOLINT

  const int kNumOfOffsetRows = offset_.rows();
  double* g_matrix = new double[kNumOfOffsetRows];  // NOLINT
  int index = 0;
  ...
  CHECK_EQ(index, kernel_matrix_.rows() * kernel_matrix_.cols());
