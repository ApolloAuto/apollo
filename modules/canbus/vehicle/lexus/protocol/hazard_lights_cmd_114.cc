...
// config detail: {'name': 'HAZARD_LIGHTS_CMD', 'offset': 0.0, 'precision': 1.0, 'len': 1, 'is_signed_var': False, 'physical_range': '[0|1]', 'bit': 8, 'type': 'bool', 'order': 'motorola', 'physical_unit': ''}
...
Hazardlightscmd114* Hazardlightscmd114::set_enable(
    bool enable) {
  enable_ = enable;
  return this;
 }
...
void Hazardlightscmd114::set_p_enable(uint8_t* data,
    bool enable) {
  int x = enable;
...
Hazardlightscmd114* Hazardlightscmd114::set_clear_faults(
    bool clear_faults) {
...
void Hazardlightscmd114::set_p_clear_faults(uint8_t* data,
    bool clear_faults) {
  int x = clear_faults;
