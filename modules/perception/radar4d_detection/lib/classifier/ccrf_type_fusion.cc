...
#include "modules/perception/radar4d_detection/lib/classifier/ccrf_type_fusion.h"
...
bool CCRFOneShotTypeFusion::Init(const TypeFusionInitOption& options) {
  std::string config_file = "ccrf_type_fusion.pb.txt";
  ...
  std::string classifiers_property_file_path =
      GetConfigFile(
          options.config_path, config.classifiers_property_file_path());
...
bool CCRFOneShotTypeFusion::TypeFusion(const TypeFusionOption& options,
                                       ObjectPtr object) {
  if (object == nullptr) {
    return false;
  }
...
bool CCRFOneShotTypeFusion::FuseOneShotTypeProbs(const ObjectPtr& object,
  ...
  const auto& vecs = object->radar4d_supplement.raw_probs;
  const auto& names = object->radar4d_supplement.raw_classification_methods;
  if (vecs.empty()) {
    return false;
  }
...
bool CCRFSequenceTypeFusion::Init(const TypeFusionInitOption& options) {
  TypeFusionInitOption one_shot_fuser_options;
  ...
  std::string transition_property_file_path =
      GetConfigFile(
          options.config_path, config.transition_property_file_path());
...
bool CCRFSequenceTypeFusion::TypeFusion(const TypeFusionOption& options,
