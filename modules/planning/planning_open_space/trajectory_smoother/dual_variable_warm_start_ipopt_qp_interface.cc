...
#include "modules/planning/planning_open_space/trajectory_smoother/dual_variable_warm_start_ipopt_qp_interface.h"
...
DualVariableWarmStartIPOPTQPInterface::DualVariableWarmStartIPOPTQPInterface(
    ...
    min_safety_distance_ = planner_open_space_config.dual_variable_warm_start_config().min_safety_distance();
}
...
DualVariableWarmStartIPOPTQPInterface::DualVariableWarmStartIPOPTQPInterface(
    ...
    l_warm_up_ = Eigen::MatrixXd::Zero(obstacles_edges_sum_, horizon_ + 1);
    n_warm_up_ = Eigen::MatrixXd::Zero(4 * obstacles_num_, horizon_ + 1);
    min_safety_distance_ = dual_variable_warm_start_config.min_safety_distance();
}
...
bool DualVariableWarmStartIPOPTQPInterface::eval_f(int n, const double* x, bool new_x, double& obj_value) {
    eval_obj(n, x, &obj_value);
    return true;
}

bool DualVariableWarmStartIPOPTQPInterface::eval_grad_f(int n, const double* x, bool new_x, double* grad_f) {
...
bool DualVariableWarmStartIPOPTQPInterface::eval_g(int n, const double* x, bool new_x, int m, double* g) {
    ...
    if (values == nullptr) {
        ...
        for (int i = 0; i < horizon_ + 1; ++i) {
            ...
            for (int j = 0; j < obstacles_num_; ++j) {
                ...
                // with respect to l
                for (int k = 0; k < current_edges_num; ++k) {
                    values[nz_index] = std::cos(xWS_(2, i)) * Aj(k, 0) + std::sin(xWS_(2, i)) * Aj(k, 1);  // v0~vn
                    ++nz_index;
                }
                ...
                // with respect to l
                for (int k = 0; k < current_edges_num; ++k) {
                    values[nz_index] = -std::sin(xWS_(2, i)) * Aj(k, 0) + std::cos(xWS_(2, i)) * Aj(k, 1);  // y0~yn
                    ++nz_index;
                }
                ...
                for (int k = 0; k < current_edges_num; ++k) {
                    values[nz_index] = (xWS_(0, i) + std::cos(xWS_(2, i)) * offset_) * Aj(k, 0)
                            + (xWS_(1, i) + std::sin(xWS_(2, i)) * offset_) * Aj(k, 1) - bj(k, 0);  // ddk
                    ++nz_index;
                }
...
void DualVariableWarmStartIPOPTQPInterface::check_solution(
        ...
        for (int j = 0; j < obstacles_num_; ++j) {
            ...
            if (g_l[constraint_index + 0] > g[constraint_index + 0]
                ...
                AERROR << "G' * mu + R' * A * lambda == 0 constraint fails, "
                       << "constraint_index: " << constraint_index + 0 << ", g: " << g[constraint_index + 0];
            }

            if (g_l[constraint_index + 1] > g[constraint_index + 1]
                || g_u[constraint_index + 1] < g[constraint_index + 1]) {
                AERROR << "G' * mu + R' * A * lambda == 0 constraint fails, "
                       << "constraint_index: " << constraint_index + 1 << ", g: " << g[constraint_index + 1];
            }
            ...
            if (g_l[constraint_index + 2] > g[constraint_index + 2]
                ...
                AERROR << "-g' * mu + (A * t - b) * lambda) >= d_min constraint fails, "
                       << "constraint_index: " << constraint_index + 2 << ", g: " << g[constraint_index + 2];
            }
    ...
    for (int i = 0; i < lambda_horizon_; ++i) {
        if (g_l[constraint_index] > g[constraint_index] || g_u[constraint_index] < g[constraint_index]) {
...
bool DualVariableWarmStartIPOPTQPInterface::eval_constraints(int n, const T* x, int m, T* g) {
    ...
    for (int i = 0; i < horizon_ + 1; ++i) {
        ...
        for (int j = 0; j < obstacles_num_; ++j) {
            ...
            // G' * mu + R' * A * lambda == 0
            g[constraint_index + 0] = x[n_index] - x[n_index + 2] + cos(xWS_(2, i)) * tmp1 + sin(xWS_(2, i)) * tmp2;

            g[constraint_index + 1] = x[n_index + 1] - x[n_index + 3] - sin(xWS_(2, i)) * tmp1 + cos(xWS_(2, i)) * tmp2;
