...
//  AERROR << "gps_base_usec_ :" << gps_base_usec_;
...
//    if (abs(adjust_angle) <1e-3 && abs(adjust_angle_two)  <1e-3 && abs(adjust_angle_three)  <1e-3  && abs(adjust_angle_four)  <1e-3  ) {
        if (difop_ptr[0] == 0xa5 && difop_ptr[1] == 0xff && difop_ptr[2] == 0x00 && difop_ptr[3] == 0x5a){
            if((difop_ptr[1202] >= 0x02 && difop_ptr[1203] >= 0x80) || difop_ptr[1202] == 0x03){
                lslidar_type = 3;
                startpos = 245;
                //Horizontal correction Angle
                adjust_angle = (difop_ptr[186]*256 + difop_ptr[187]);         //Angle correction A1
                if(adjust_angle > 32767){
                    adjust_angle = adjust_angle - 65535;
                }
                adjust_angle_two = (difop_ptr[190]*256 + difop_ptr[191]);     //Angle correction A2
                if(adjust_angle_two > 32767){
                    adjust_angle_two = adjust_angle_two - 65535;
                }
                adjust_angle_three = (difop_ptr[188]*256 + difop_ptr[189]);   //Angle correction A3
                if(adjust_angle_three > 32767){
                    adjust_angle_three = adjust_angle_three - 65535;
                }

                adjust_angle_four = (difop_ptr[192]*256 + difop_ptr[193]);    //Angle correction A4
                ...
                if(difop_ptr[185] == 0 || difop_ptr[185] == 1)
                {
                    ...
                    for(int i = 0; i < LSC32_SCANS_PER_FIRING; i++)
                    {
                        ...
                        if(1 ==  config_.degree_mode()){
                            cos_scan_altitude_caliration[i] = std::cos(static_cast<float>(scan_altitude_original_A3[i] * M_PI) / 180.0f);
                            sin_scan_altitude_caliration[i] = std::sin(static_cast<float>(scan_altitude_original_A3[i] * M_PI) / 180.0f);
                            scan_altitude_A[i] = scan_altitude_original_A3[i];
                        }

                        //0.33度校准四列
                        if(2 == config_.degree_mode()){
                            cos_scan_altitude_caliration[i] = std::cos(static_cast<float>(scan_altitude_original_C3[i] * M_PI) / 180.0f);
                            sin_scan_altitude_caliration[i] = std::sin(static_cast<float>(scan_altitude_original_C3[i] * M_PI) / 180.0f);
                ...
                startpos = 882;
                //Horizontal correction Angle
                adjust_angle = (difop_ptr[34]*256 + difop_ptr[35]);         /*Angle correction A1*/
                if(adjust_angle > 32767){
                    adjust_angle = adjust_angle - 65535;
                }

                adjust_angle_two = (difop_ptr[42]*256 + difop_ptr[43]);     /*Angle correction A2*/
                if(adjust_angle_two > 32767){
                    adjust_angle_two = adjust_angle_two - 65535;
                }

                adjust_angle_three = (difop_ptr[66]*256 + difop_ptr[67]);   /*Angle correction A3*/
                if(adjust_angle_three > 32767){
                    adjust_angle_three = adjust_angle_three - 65535;
                }

                adjust_angle_four = (difop_ptr[68]*256 + difop_ptr[69]);    /*Angle correction A4*/
                if(adjust_angle_four > 32767){
                    adjust_angle_four = adjust_angle_four - 65535;
                }
                ...
                for(int i = 0; i < LSC32_SCANS_PER_FIRING; i++)
                {
                    //均匀1度校准两列
                    if(1 ==  config_.degree_mode()){
                        cos_scan_altitude_caliration[i] = std::cos(static_cast<float>(scan_altitude_original_A[i] * M_PI) / 180.0f);
                        sin_scan_altitude_caliration[i] = std::sin(static_cast<float>(scan_altitude_original_A[i] * M_PI) / 180.0f);
                        scan_altitude_A[i] = scan_altitude_original_A[i];
                    }

                    //0.33度校准四列
                    if(2 ==  config_.degree_mode()){
                        cos_scan_altitude_caliration[i] = std::cos(static_cast<float>(scan_altitude_original_C[i] * M_PI) / 180.0f);
                        sin_scan_altitude_caliration[i] = std::sin(static_cast<float>(scan_altitude_original_C[i] * M_PI) / 180.0f);
...
//    double scan_altitude_A_real_value[32] = {-15.32,0.0,-13.52,1,-14,2,-13,3,-12,4,-11,5,-10,6,-9,7,-8,8,-7,9,-6,10,-5,11,-4,12,-3,13,-2,14,-1,14.28};
//
//    for (int i = 0; i < 32; ++i) {
////        AERROR << "scan_altitude_A : " << scan_altitude_A[i];
//        scan_altitude_A[i] = scan_altitude_A_real_value[i];
//    }
  ...
  for (size_t i = 0; i < packets_size; ++i) {
    ...
    //AERROR << "current_packet_time :" << current_packet_time  << " previous_packet_time :" << previous_packet_time;
...
void Lslidar32Parser::Unpack(const LslidarPacket& pkt,
                              std::shared_ptr<PointCloud> pc ,int packet_number) {
  float x, y, z;
  float azimuth = 0.0f;
  ...
  uint64_t  packet_end_time;
	...
	if (config_vert_angle) {
		for (int i = 0; i < LSC32_SCANS_PER_FIRING; i++) {
		  //均匀1度校准两列
		   if(1 ==  config_.degree_mode()){
			cos_scan_altitude_caliration[i] = std::cos(static_cast<float>(scan_altitude_A[i] * M_PI) / 180.0f);
			sin_scan_altitude_caliration[i] = std::sin(static_cast<float>(scan_altitude_A[i] * M_PI) / 180.0f);
		  }

		  //0.33度校准四列
		  if(2 ==  config_.degree_mode()){
			cos_scan_altitude_caliration[i] = std::cos(static_cast<float>(scan_altitude_C[i] * M_PI) / 180.0f);
			sin_scan_altitude_caliration[i] = std::sin(static_cast<float>(scan_altitude_C[i] * M_PI) / 180.0f);
		  }
		}
		config_vert_angle = false;
	}

//	AERROR <<"c32 " <<int(pkt.data()[0])<< "  "  <<int(pkt.data()[1]) ;

  const raw_packet_t* raw = (const raw_packet_t*)pkt.data().c_str();
  if (!config_.time_synchronization()){
//    packet_end_time = *(reinterpret_cast<uint64_t>(const_cast<uint64_t>(pkt.stamp())));
...
//  packet_end_time = gps_base_usec_ - (gps_base_usec_ %1000000) + raw->gps_timestamp % 1000000;
 
//  if(last_packet_time != 0)
//  {
//	  if(packet_end_time < last_packet_time)
//		  packet_end_time = packet_end_time + 1000000;
//	  else if(packet_end_time - last_packet_time > 1000000)
//		  packet_end_time = packet_end_time - 1000000;
//  }
//
//  last_packet_time = packet_end_time;
  ...
  for (int block = 0; block < BLOCKS_PER_PACKET; block++, block_num++) {
    ...
    for (int firing = 0, k = 0; firing < LSC32_FIRINGS_PER_BLOCK; ++firing) {
      for (int dsr = 0; dsr < LSC32_SCANS_PER_FIRING; ++dsr, k += RAW_SCAN_SIZE) {
        point_count++;
		...
		//intensity = intensity / 2;	//points with intensity above 170 cannot be displayed
		
		float distance2 = (distance * DISTANCE_RESOLUTION) * config_.distance_unit();
		...
		if(2 == config_.return_mode()){
                  if(previous_packet_time >1e-6) {
                    point_time = current_packet_time - (SCANS_PER_PACKET - point_count  - 1) * (current_packet_time - previous_packet_time)/(SCANS_PER_PACKET);
                  } else{              //fist packet
                    point_time = current_packet_time;
                  }
//			point_time = packet_end_time * 1000 - 49152 * (5 - block/2) - 1536 * (31 - dsr);
		}else{
                  if(previous_packet_time >1e-6) {
                    point_time = current_packet_time - (SCANS_PER_PACKET - point_count - 1) * (current_packet_time - previous_packet_time)/(SCANS_PER_PACKET);
                  } else{              //fist packet
                    point_time = current_packet_time;
                  }//			point_time = packet_end_time * 1000 - 49152 * (11 - block) - 1536 * (31 - dsr);
		}
		
        if (distance2 > config_.max_range() || distance2 < config_.min_range()) {
			...
			if ((azimuth_corrected_f < config_.scan_start_angle()) || (azimuth_corrected_f > config_.scan_end_angle())) continue;
            if (packet_number == 0) {
                if (azimuth_corrected_f > 30000) {
//                                    AERROR << "azimuth_corrected >300000  ";
                    continue;
                }
            }
...
//                                    AERROR << "azimuth_corrected < 10000 ";
                    continue;
                }
            }

			//以结构为中心
            float rotation_azimuth = ROTATION_RESOLUTION * static_cast<float>(azimuth_corrected_f * M_PI) / 180.0f;
            azimuth_corrected_offset_f = azimuth_corrected_f*ROTATION_RESOLUTION - LSC32_AZIMUTH_TOFFSET;
            float rotation_azimuth_offset = static_cast<float>(azimuth_corrected_offset_f * M_PI) / 180.0f;

			PointXYZIT* point = pc->add_point();
			point->set_timestamp(point_time);
			point->set_intensity(intensity);

            if(intensity>172)  intensity=172;  //todo 如果不设置172，cyber_visualizer中显示会有问题，点云会显示不完整
            // 如果想要获取真实的强度值，则注释这一行
            point->set_intensity(intensity);
			...
			if (config_.calibration()) {
				...
				x = distance2 * cos_scan_altitude_caliration[dsr]  * cosf(rotation_azimuth) + (LSC32_DISTANCE_TOFFSET * cosf(rotation_azimuth_offset)) * DISTANCE_RESOLUTION;
				y = -(distance2 * cos_scan_altitude_caliration[dsr]  * sinf(rotation_azimuth) + (LSC32_DISTANCE_TOFFSET * sinf(rotation_azimuth_offset)) * DISTANCE_RESOLUTION);
