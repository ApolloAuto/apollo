...
TEST_F(GraphSegmentorTest, test_segment_graph) {
  {
    ...
    EXPECT_EQ(0, segmentor.GetUniverse()->GetSetsNum());
    segmentor.SegmentGraph(num_vertices_, num_edges_, edges_, true);
    EXPECT_EQ(3, segmentor.GetUniverse()->GetSetsNum());
  }
  {
    GraphSegmentor segmentor;
    segmentor.Init(6.0);
    segmentor.SegmentGraph(num_vertices_, num_edges_, edges_);
    EXPECT_EQ(1, segmentor.GetUniverse()->GetSetsNum());
  }
  {
    GraphSegmentor segmentor;
    segmentor.Init(2.0);
    segmentor.SegmentGraph(num_vertices_, num_edges_, edges_);
    EXPECT_EQ(4, segmentor.GetUniverse()->GetSetsNum());
  }
}
