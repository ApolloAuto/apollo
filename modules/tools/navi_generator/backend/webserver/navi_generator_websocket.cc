...
void NaviGeneratorWebSocket::handleReadyState(CivetServer *server,
                                              Connection *conn) {
  {
...
void NaviGeneratorWebSocket::handleClose(CivetServer *server,
...
bool NaviGeneratorWebSocket::BroadcastData(const std::string &data,
                                           bool skippable) {
...
bool NaviGeneratorWebSocket::SendBinaryData(Connection *conn,
                                            const std::string &data,
                                            bool skippable) {
  return SendData(conn, data, skippable, WEBSOCKET_OPCODE_BINARY);
}

bool NaviGeneratorWebSocket::SendData(Connection *conn, const std::string &data,
...
thread_local unsigned char NaviGeneratorWebSocket::current_opcode_ = 0x00;
thread_local std::stringstream NaviGeneratorWebSocket::data_;

bool NaviGeneratorWebSocket::handleData(CivetServer *server, Connection *conn,
                                        int bits, char *data, size_t data_len) {
  AINFO << "go to NaviGeneratorWebSocket::handleData";
...
bool NaviGeneratorWebSocket::handleJsonData(Connection *conn,
                                            const std::string &data) {
  AINFO << "go to NaviGeneratorWebSocket::handleJsonData";
...
bool NaviGeneratorWebSocket::handleBinaryData(Connection *conn,
                                              const std::string &data) {
  auto type = "Binary";
