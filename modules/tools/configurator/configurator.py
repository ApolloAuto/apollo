...
class Configurator(object):
    ...
    def __init__(self, stdscr):
        ...
        with open(self.META_DATA_FILE) as f:
            for line in f:
                module_name, proto_file, proto_class, conf_file = line.strip().split(
                    ' ')
                if module_name == 'MODULE' or proto_file == '#' or\
                    proto_class == "#" or conf_file == '#':
                    pass
                else:
                    moduleconf = ModuleConf(module_name, proto_file, proto_class, conf_file, self.stdscr)
    ...
    def setup(self):
        ...
        for idx in xrange(len(self.moduleconf)):
            ...
            self.stdscr.addstr(lidx, 65, self.moduleconf[idx].conf_file,
                curses.color_pair(1 if self.moduleconf[idx].found_conf == False else 2))

        if self.select == len(self.moduleconf):
            self.stdscr.addstr(len(self.moduleconf)+3, 2, "Exit", curses.A_REVERSE)
        else:
            self.stdscr.addstr(len(self.moduleconf)+3, 2, "Exit", curses.A_BOLD)

        self.stdscr.addstr(self.winy-9,2,"[Up][Down]: Choose different item")
        self.stdscr.addstr(self.winy-8,2,"[Enter] Select item/Make Change")
        self.stdscr.addstr(self.winy-7,2,"[Left][Right]: Index through repeated item")
        ...
        self.stdscr.addstr(self.winy-3,2, "Change will be lost if panel closed, or changing repeated item index")
