...
unordered_map<string, ::std::unique_ptr<SimControlBase>>
    DynamicModelFactory::s_dynamic_model_map_;
...
bool DynamicModelFactory::RegisterDynamicModel(std::string& dm_library_path,std::string& dm_name){
  ...
  try {
    ...
    // 直接在这里创建类指针 指向DM类
    SimControlBase* dynamic_model_ptr = shared_library->GetSymbol(dm_name);
    // 这里应该是调用函数 返回生成的指针
...
std::unique_ptr<SimControlBase> DynamicModelFactory::GetModelType(std::string dynamic_model_name) {
  // 我觉得应该配置成字典
  auto iter = s_dynamic_model_map_.find(dynamic_model_name);
  if(iter==s_dynamic_model_map_.end()){
    // 返回空指针
  ...
  return s_dynamic_model_map_[dynamic_model_name];
}
