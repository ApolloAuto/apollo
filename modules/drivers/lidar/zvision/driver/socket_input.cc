  ...
  namespace drivers
  {
    namespace zvision
    {
      ...
      void SocketInput::init(const int &port)
      {
        ...
        AINFO << "Zvision socket fd is " << sockfd_ << ", port " << port_;
        printf("Zvision socket init ok, port is %d\n", port_);
      }

      /** @brief Get one zvision packet. */
      int SocketInput::get_firing_data_packet(ZvisionPacket *pkt)
      {
        ...
        while (true)
        {
          ...
          #ifdef ez6_sample_a
          // sample A 
          uint8_t bytes[FIRING_DATA_PACKET_SIZE_EZ6A];
          ssize_t nbytes =
              recvfrom(sockfd_, bytes, FIRING_DATA_PACKET_SIZE_EZ6A, 0, nullptr, nullptr);
          #endif

          #ifdef ez6_sample_b
          // sample B
          uint8_t bytes[FIRING_DATA_PACKET_SIZE_EZ6B];
          ssize_t nbytes =
              recvfrom(sockfd_, bytes, FIRING_DATA_PACKET_SIZE_EZ6B, 0, nullptr, nullptr);
          #endif
          ...
          #ifdef ez6_sample_a
          // sample A 
          if ((size_t)nbytes == FIRING_DATA_PACKET_SIZE_EZ6A)
          {
            // read successful, done now
            // printf("read successful, done now \n");
            pkt->set_data(bytes, FIRING_DATA_PACKET_SIZE_EZ6A);
            break;
          }
          #endif
          ...
          #ifdef ez6_sample_b
          // sample B
          if ((size_t)nbytes == FIRING_DATA_PACKET_SIZE_EZ6B)
          {
            // read successful, done now
            // printf("read successful, done now \n");
            pkt->set_data(bytes, FIRING_DATA_PACKET_SIZE_EZ6B);
            break;
          }
          #endif

          if ((size_t)nbytes != CALIBRATION_DATA_PACKET_SIZE && (size_t)nbytes != IMU_DATA_PACKET_SIZE)
            AERROR << "Incomplete Zvision rising data packet read: " << nbytes
      ...
      bool SocketInput::input_available(int timeout)
      {
        ...
        do
        {
          ...
          if (retval == 0)
          { // poll() timeout?
            AWARN << "Zvision port " << port_ << " poll() timeout";
            return false;
          }
          ...
          { // device error?
            AERROR << "Zvision port " << port_ << "poll() reports Zvision error";
            return false;
          }
