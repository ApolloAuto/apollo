...
class OfflineLidarDetection {
 public:
  OfflineLidarDetection() = default;

  ~OfflineLidarDetection() = default;

  bool Init() {
    BaseLidarDetector* detector =
        BaseLidarDetectorRegisterer::GetInstanceByName(FLAGS_detector_name);
 ...
 protected:
  std::unique_ptr<BaseLidarDetector> detector_;
...
int main(int argc, char** argv) {
  ...
  apollo::perception::lidar::OfflineLidarDetection test;
  if (!test.Init()) {
    AINFO << "Failed to setup OfflineLidarDetection";
    return -1;
  }
  return test.Run() ? 0 : -1;
}
