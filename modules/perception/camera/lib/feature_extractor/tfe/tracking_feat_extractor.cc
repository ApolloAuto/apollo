...
void TrackingFeatureExtractor::init_roipooling(const FeatureExtractorInitOptions
                                               &options,
                                               const tracking_feature::
  ...
  feature_extractor_layer_ptr->pooling_layer
      .reset(new inference::ROIPoolingLayer<float>(pooled_h, pooled_w,
                                                   use_floor, 1, feat_channel));
  feature_extractor_layer_ptr->top_blob.reset(new base::Blob<float>(1,
                                           feat_blob_->channels(),
                                           pooled_h,
                                           pooled_w));
...
bool TrackingFeatureExtractor::Extract(const FeatureExtractorOptions &options,
  ...
  for (auto feature_extractor_layer_ptr : roi_poolings_) {
    feature_extractor_layer_ptr->rois_blob
        ->Reshape({static_cast<int>(frame->detected_objects.size()), 5});
    float *rois_data =
    ...
    feature_extractor_layer_ptr->pooling_layer
        ->ForwardGPU({feat_blob_, feature_extractor_layer_ptr->rois_blob},
