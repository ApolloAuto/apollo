 ...
 * @file
 */
#include "modules/planning/planning_open_space/trajectory_smoother/dual_variable_warm_start_slack_osqp_interface.h"
...
DualVariableWarmStartSlackOSQPInterface::DualVariableWarmStartSlackOSQPInterface(
    ...
    check_mode_ = dual_variable_warm_start_config.debug_osqp();
    beta_ = dual_variable_warm_start_config.beta();
    osqp_config_ = dual_variable_warm_start_config.osqp_config();
}
...
bool DualVariableWarmStartSlackOSQPInterface::optimize() {
    ...
    for (int i = 0; i < horizon_ + 1; ++i) {
        ...
        for (int j = 0; j < obstacles_num_; ++j) {
            for (int k = 0; k < obstacles_edges_num_(j, 0); ++k) {
                l_warm_up_(r_index, i) = lambda_norm[i * obstacles_num_ + j] * work->solution->x[variable_index];
                ++variable_index;
                ++r_index;
            }
        }
    ...
    for (int i = 0; i < horizon_ + 1; ++i) {
        ...
        for (int j = 0; j < obstacles_num_; ++j) {
            for (int k = 0; k < 4; ++k) {
                n_warm_up_(r_index, i) = lambda_norm[i * obstacles_num_ + j] * work->solution->x[variable_index];
                ++r_index;
                ++variable_index;
            }
        }
