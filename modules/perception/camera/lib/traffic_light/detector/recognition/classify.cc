...
void ClassifyBySimple::Init(
  ...
  image_.reset(new base::Image8U(resize_height_,
                                 resize_width_,
                                 base::Color::BGR));
...
void
ClassifyBySimple::Perform(const CameraFrame* frame,
...
void ClassifyBySimple::Prob2Color(const float *out_put_data, float threshold,
  ...
  int max_color_id = 0;
  std::vector<base::TLColor> status_map = {base::TLColor::TL_BLACK,
      base::TLColor::TL_RED, base::TLColor::TL_YELLOW, base::TLColor::TL_GREEN};
  ...
  max_color_id = (*max_prob > threshold) ?
      static_cast<int>(std::distance(prob.begin(), max_prob)) : 0;
