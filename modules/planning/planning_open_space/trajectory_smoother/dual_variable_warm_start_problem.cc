...
DualVariableWarmStartProblem::DualVariableWarmStartProblem(const PlannerOpenSpaceConfig& planner_open_space_config) {
    planner_open_space_config_ = planner_open_space_config;
    dual_variable_warm_start_config_ = planner_open_space_config_.dual_variable_warm_start_config();
}
...
bool DualVariableWarmStartProblem::Solve(
    ...
    if (dual_variable_warm_start_config_.qp_format() == OSQP) {
    ...
    } else if (dual_variable_warm_start_config_.qp_format() == SLACKQP) {
    ...
    } else if (dual_variable_warm_start_config_.qp_format() == IPOPTQP) {
        ...
        app->Options()->SetIntegerValue(
                "print_level", dual_variable_warm_start_config_.ipopt_config().ipopt_print_level());
        app->Options()->SetIntegerValue(
                "mumps_mem_percent", dual_variable_warm_start_config_.ipopt_config().mumps_mem_percent());
        app->Options()->SetNumericValue("mumps_pivtol", dual_variable_warm_start_config_.ipopt_config().mumps_pivtol());
        app->Options()->SetIntegerValue("max_iter", dual_variable_warm_start_config_.ipopt_config().ipopt_max_iter());
        app->Options()->SetNumericValue("tol", dual_variable_warm_start_config_.ipopt_config().ipopt_tol());
        app->Options()->SetNumericValue(
                "acceptable_constr_viol_tol",
                dual_variable_warm_start_config_.ipopt_config().ipopt_acceptable_constr_viol_tol());
        app->Options()->SetNumericValue(
                "min_hessian_perturbation",
                dual_variable_warm_start_config_.ipopt_config().ipopt_min_hessian_perturbation());
        app->Options()->SetNumericValue(
                "jacobian_regularization_value",
                dual_variable_warm_start_config_.ipopt_config().ipopt_jacobian_regularization_value());
        app->Options()->SetStringValue(
                "print_timing_statistics",
                dual_variable_warm_start_config_.ipopt_config().ipopt_print_timing_statistics());
        app->Options()->SetStringValue(
                "alpha_for_y", dual_variable_warm_start_config_.ipopt_config().ipopt_alpha_for_y());
        app->Options()->SetStringValue("recalc_y", dual_variable_warm_start_config_.ipopt_config().ipopt_recalc_y());
    ...
    } else if (dual_variable_warm_start_config_.qp_format() == IPOPT) {
        ...
        for (int c = 0; c < l_warm_up->cols(); ++c) {
            for (int r = 0; r < l_warm_up->rows(); ++r) {
                l_max_diff1 = std::max(l_max_diff1, std::abs(l_warm_up->coeff(r, c) - l_warm_up_ipopt(r, c)));
                l_max_diff2 = std::max(l_max_diff2, std::abs(l_warm_up->coeff(r, c) - l_warm_up_ipoptqp(r, c)));
                l_max_diff3 = std::max(l_max_diff3, std::abs(l_warm_up_ipoptqp(r, c) - l_warm_up_ipopt(r, c)));
            }
        }
        ...
        for (int c = 0; c < n_warm_up->cols(); ++c) {
            for (int r = 0; r < n_warm_up->rows(); ++r) {
                n_max_diff1 = std::max(n_max_diff1, std::abs(n_warm_up->coeff(r, c) - n_warm_up_ipopt(r, c)));
                n_max_diff2 = std::max(n_max_diff2, std::abs(n_warm_up->coeff(r, c) - n_warm_up_ipoptqp(r, c)));
                n_max_diff3 = std::max(n_max_diff3, std::abs(n_warm_up_ipoptqp(r, c) - n_warm_up_ipopt(r, c)));
            }
        }
