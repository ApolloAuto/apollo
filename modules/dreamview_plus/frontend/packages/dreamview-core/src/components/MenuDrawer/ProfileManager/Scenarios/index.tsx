...
function Operations(props: IOperations) {
    const { status, id, currentScenarioId, onUpdateDownloadProgress, recordName } = props;
    const { isPluginConnected, pluginApi } = useWebSocketServices();
    const usSubScribe = useRef<any>({
        do: () => true,
    });

    const onDelete = useCallback(() => false, []);

    const onDownload = useCallback(() => {
        // do download
        const throttleFn = throttle(onUpdateDownloadProgress, 0.3);
...
function Scenarios() {
    ...
    const updateDownloadProgress = useCallback((r: IScenarioSet) => {
        setOriginData((prev) => {
    ...
    const columns = useMemo(
        () => getColumns(t, refreshList, updateDownloadProgress, currentScenarioSetId),
        [t, refreshList, updateDownloadProgress, currentScenarioSetId],
    );
