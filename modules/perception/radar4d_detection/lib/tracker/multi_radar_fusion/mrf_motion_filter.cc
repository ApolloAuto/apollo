...
#include "modules/perception/radar4d_detection/lib/tracker/multi_radar_fusion/mrf_motion_filter.h"
...
bool MrfMotionFilter::Init(const MrfFilterInitOptions& options) {
  std::string config_file = "mrf_motion_filter.pb.txt";
  if (!options.config_file.empty()) {
    config_file = options.config_file;
  }
  config_file = GetConfigFile(
      options.config_path, config_file);
  MrfMotionFilterConfig config;
  ...
  motion_measurer_.reset(new MrfMotionMeasurement);

  motion_refiner_.reset(new MrfMotionRefiner);
  MrfMotionRefinerInitOptions mrf_motion_refiner_options;
  mrf_motion_refiner_options.config_path = options.config_path;
  ACHECK(motion_refiner_->Init(mrf_motion_refiner_options));
  return true;
}

void MrfMotionFilter::UpdateWithObject(const MrfFilterOptions& options,
...
void MrfMotionFilter::UpdateWithoutObject(const MrfFilterOptions& options,
                                          double timestamp,
                                          MrfTrackDataPtr track_data) {}

void MrfMotionFilter::InitializeTrackState(TrackedObjectPtr new_object) {
...
void MrfMotionFilter::KalmanFilterUpdateWithPartialObservation(
    const MrfTrackDataConstPtr& track_data,
...
void MrfMotionFilter::StateGainAdjustment(
    const MrfTrackDataConstPtr& track_data,
...
void MrfMotionFilter::OnlineCovarianceEstimation(
    const MrfTrackDataConstPtr& track_data, TrackedObjectPtr object) {
...
void MrfMotionFilter::ConvergenceEstimationAndBoostUp(
    const MrfTrackDataConstPtr& track_data,
    const TrackedObjectConstPtr& latest_object, TrackedObjectPtr new_object) {
  new_object->boostup_need_history_size =
...
void MrfMotionFilter::ComputeConvergenceConfidence(
    const MrfTrackDataConstPtr& track_data, TrackedObjectPtr new_object,
...
void MrfMotionFilter::BoostupState(const MrfTrackDataConstPtr& track_data,
                                   TrackedObjectPtr new_object) {
  ...
  while (boostup_used_history_size < actual_boostup_history_size &&
    ...
    Eigen::Vector3d measured_velocity_on_project_dir =
        algorithm::Calculate2DXYProjectVector<double>(measured_velocity,
...
void MrfMotionFilter::UpdateConverged(const MrfTrackDataConstPtr& track_data,
