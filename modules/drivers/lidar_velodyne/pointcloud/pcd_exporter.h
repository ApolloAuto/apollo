...
#ifndef MODULES_DRIVERS_DRIVERS_LIDAR_VELODYNE_POINTCLOUD_PCD_EXPORTER_H_
#define MODULES_DRIVERS_DRIVERS_LIDAR_VELODYNE_POINTCLOUD_PCD_EXPORTER_H_
...
class PCDExporter {
 ...
 private:
  ...
  FILE *stamp_file_handle_ = nullptr;
  // File name of pcd_pose file
  std::string pose_file_;
  FILE *pose_file_handle_ = nullptr;
  ...
  float time_offset_ = 0.1f;
  float loc_threshold_ = 0.1f;
  // Message counter, use as index when use_seq_ = false
  unsigned int pc_msg_count_ = 1;
   ...
   * @brief Query pose
   */
  bool get_pose(const ros::Time &time, Eigen::Matrix4d &pose);  // NOLINT
...
#endif  // MODULES_DRIVERS_DRIVERS_LIDAR_VELODYNE_POINTCLOUD_PCD_EXPORTER_H_
