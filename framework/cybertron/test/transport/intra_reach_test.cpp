...
class IntraReachTest : public ::testing::Test {
 protected:
  using Upper = IntraUpperReach<proto::UnitTest>;
  using Lower = IntraLowerReach<proto::UnitTest>;
  using UpperPtr = std::shared_ptr<UpperReach<proto::UnitTest>>;
  using LowerPtr = std::shared_ptr<LowerReach<proto::UnitTest>>;

  IntraReachTest() : channel_name_("intra_channel") {}

  virtual ~IntraReachTest() {}
...
TEST_F(IntraReachTest, constructor) {
  RoleAttributes attr;
...
TEST_F(IntraReachTest, enable_and_disable) {
  // repeated call
  upper_reach_a_->Enable();
  ...
  msg->set_class_name("IntraReachTest");
  ...
  for (auto& item : msgs) {
    EXPECT_EQ(item.class_name(), "IntraReachTest");
