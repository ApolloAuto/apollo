...
NTCAN_RESULT EsdCanTest(const int can_id, NTCAN_HANDLE* handle) {
  ...
  if (ret == NTCAN_SUCCESS) {
    ...
    AERROR << "Failed to open ESD-CAN device " << can_id
           << ", error: " << ret << " (" << StatusString(ret) << ")";
    return ret;
  }
  ...
  if (ret != NTCAN_SUCCESS) {
    AERROR << "Cannot get status of ESD-CAN, ret="
  ...
  if (ret != NTCAN_SUCCESS) {
    AERROR << "NTCAN_IOCTL_GET_BITRATE_ for device with error: "
           << ret << " (" << StatusString(ret) << ")";
    return ret;
  }
  return ret;
