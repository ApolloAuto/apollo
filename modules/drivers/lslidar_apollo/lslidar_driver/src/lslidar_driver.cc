...
void LslidarDriver::getFPGA_GPSTimeStamp(
    lslidar_msgs::LslidarPacketPtr& packet) {
  unsigned char head2[] = {packet->data[0], packet->data[1], packet->data[2],
                           packet->data[3]};
  ...
  if (head2[0] == 0xA5 && head2[1] == 0xFF) {
    ...
    ROS_DEBUG("ROS TS: %f, GPS: y:%d m:%d d:%d h:%d m:%d s:%d; FPGA: us:%lu",
              timeStamp.toSec(), cur_time.tm_year, cur_time.tm_mon,
              cur_time.tm_mday, cur_time.tm_hour, cur_time.tm_min,
              cur_time.tm_sec, packet_timestamp);
  }
}
