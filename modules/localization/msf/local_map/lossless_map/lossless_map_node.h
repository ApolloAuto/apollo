...
public:
     ...
     */
    void set_value(const Eigen::Vector3d& coordinate,
     ...
     * @param <coordinate> The 3D global coordinate. The z is used as the altitude for the layer match.
     * @param <intensity> The reflectance intensity.
     */
    void set_value_layer(const Eigen::Vector3d& coordinate,
                         unsigned char intensity);
    /**@brief Given the 3D global coordinate, get the map cell average intensity of each layer. */
    void get_value(const Eigen::Vector3d& coordinate, std::vector<unsigned char>& values) const;
    /**@brief Given the 3D global coordinate, get the map cell variance of the intensity of each layer. */
    void get_var(const Eigen::Vector3d& coordinate, std::vector<float>& vars) const;
    /**@brief Given the 3D global coordinate, get the map cell's average altitude of each layer. */
    void get_alt(const Eigen::Vector3d& coordinate,
                            std::vector<float>& alts) const;
    /**@brief Given the 3D global coordinate, get the map cell's variance of the altitude of each layer. */
    void get_alt_var(const Eigen::Vector3d& coordinate, std::vector<float>& alt_vars) const;
    /**@brief Given the 3D global coordinate, get the map cell's count of the samples of each layer. */
    ...
    /**@brief Given the 3D global coordinate, get the map cell variance of the intensity. */
    ...
    /**@brief Given the 3D global coordinate, get the map cell's variance of the altitude */
    float get_alt_var(const Eigen::Vector3d& coordinate) const;
    /**@brief Given the 3D global coordinate, get the map cell's count of the samples. */
    ...
    unsigned char get_value(unsigned int row, unsigned int col) const {
        return static_cast<LosslessMapMatrix*>(_map_matrix)->get_map_cell(row, col).get_value();
    }
    /**@brief Get the map cell variance of the intensity. */
    float get_var(unsigned int row, unsigned int col) const {
        return static_cast<LosslessMapMatrix*>(_map_matrix)->get_map_cell(row, col).get_var();
    }
    /**@brief Get the map cell's average altitude. */
    float get_alt(unsigned int row, unsigned int col) const {
        return static_cast<LosslessMapMatrix*>(_map_matrix)->get_map_cell(row, col).get_alt();
    }
    /**@brief Get the map cell's variance of the altitude */
    float get_alt_var(unsigned int row, unsigned int col) const {
        return static_cast<LosslessMapMatrix*>(_map_matrix)->get_map_cell(row, col).get_alt_var();
    }
    /**@brief Get the map cell's count of the samples. */
    unsigned int get_count(unsigned int row, unsigned int col) const {
        return static_cast<LosslessMapMatrix*>(_map_matrix)->get_map_cell(row, col).get_count();
    }
    /**@brief Get the constant map cell given the coordinate. */
    inline const LosslessMapSingleCell& get_first_map_cell(unsigned int row, unsigned int col) const {
        return static_cast<LosslessMapMatrix*>(_map_matrix)->get_map_cell(row, col)._map_cells[0];
    }
    inline LosslessMapSingleCell& get_first_map_cell(unsigned int row, unsigned int col) {
        return static_cast<LosslessMapMatrix*>(_map_matrix)->get_map_cell(row, col)._map_cells[0];
    }
