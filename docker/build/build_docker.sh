...
function determine_images_in_out_x86_64() {
    ...
    if [[ "${stage}" == "base" ]]; then
        ...
        if [[ "${dist}" == "stable" ]]; then
            ...
            DEV_IMAGE_IN="${APOLLO_REPO}:dev-x86_64-${UBUNTU_LTS}-20210129_0240"
            IMAGE_OUT="${APOLLO_REPO}:runtime-x86_64-${UBUNTU_LTS}-${timestamp}"
        else
            echo "Testing Runtime Docker for Apollo not supported. Exiting..."
            exit 1
        fi
    else
...
function print_usage() {
    ...
    echo "${TAB}-m,--mode   \"download\" for using prebuilt packages if possible, \"build\" for building from source."
    ...
    echo "${TAB}--dry       Dry run only (for testing purpose)"
