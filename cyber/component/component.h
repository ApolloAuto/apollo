...
class Component : public ComponentBase {
 public:
  ...
  bool Initialize(const ComponentConfig& config) override {
    ...
    if (cyber_likely(is_reality_mode)) {
      ...
      if (!CreateSimulatedReaders(config, std::make_index_sequence<sizeof...(M)>{})) {
        AERROR << "Component create reader failed." << std::endl;
        return false;
      }
      return true;
    ...
    auto func =
        [self](const std::shared_ptr<M0>& msg0, const std::shared_ptr<M>& ...msg) {
          auto ptr = self.lock();
          if (ptr) {
  ...
  bool Process(const std::shared_ptr<M0> msg0, const std::shared_ptr<M> ...msg) {
 ...
 private:
   ...
   * @return returns true if successful, otherwise returns false
   */
  virtual bool Proc(const std::shared_ptr<M0>& msg0, const std::shared_ptr<M>& ...msg) = 0;

  template<typename T>
  std::shared_ptr<ReaderBase> CreateReader(const ComponentConfig& config, size_t i) {
  ...
  template<size_t ...N>
  bool CreateReaders(const ComponentConfig& config, std::index_sequence<N...>) {
    readers_ = std::vector<std::shared_ptr<ReaderBase> >({CreateReader<M0>(config, 0), CreateReader<M>(config, N+1)...});
  ...
  template<size_t ...N>
  bool CreateSimulatedReaders(const ComponentConfig& config, std::index_sequence<N...>) {
    auto readers = std::vector<std::shared_ptr<ReaderBase> >({CreateReader<M>(config, N+1)...});
    auto blockers = std::make_tuple(blocker::BlockerManager::Instance()->GetBlocker<M>(config.readers(N+1).channel()) ...);
    ...
    auto func = [self, blockers](const std::shared_ptr<M0>& msg0) {
      auto ptr = self.lock();
      if (ptr) {
        std::vector<bool> isEmpty = {std::get<N>(blockers)->IsPublishedEmpty() ...};
        bool hasEmpty = (std::find(isEmpty.begin(), isEmpty.end(), true) != isEmpty.end());
        if (!hasEmpty) {
          ptr->Process(msg0, std::get<N>(blockers)->GetLatestPublishedPtr() ...);
        }
      } else {
