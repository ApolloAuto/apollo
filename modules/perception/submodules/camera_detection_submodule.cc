 ...
 *****************************************************************************/
#include "modules/perception/submodules/camera_detection_submodule.h"
...
CameraDetectionSubmodule::~CameraDetectionSubmodule() {}

bool CameraDetectionSubmodule::Init() {
...
void CameraDetectionSubmodule::OnReceiveImage(
  ...
  AINFO << "Enter CameraDetectionSubmodule::Proc(), "
...
int CameraDetectionSubmodule::InitConfig() {
  ...
  if (camera_names_.size() != 2) {
    AERROR << "Now CameraDetectionSubmodule only support 2 cameras";
    return cyber::FAIL;
  }
  ...
  std::string format_str = R"(
      CameraDetectionSubmodule InitConfig success
      camera_names:    %s, %s
...
int CameraDetectionSubmodule::InitSensorInfo() {
  if (camera_names_.size() != 2) {
...
int CameraDetectionSubmodule::InitAlgorithmPlugin() {
...
int CameraDetectionSubmodule::InitCameraFrames() {
  if (camera_names_.size() != 2) {
    AERROR << "invalid camera_names_.size(): " << camera_names_.size();
    return cyber::FAIL;
  }
...
int CameraDetectionSubmodule::InitProjectMatrix() {
...
int CameraDetectionSubmodule::InitCameraListeners() {
  for (size_t i = 0; i < camera_names_.size(); ++i) {
    ...
    std::function<void(const ImageMsgType &)> camera_callback =
        std::bind(&CameraDetectionSubmodule::OnReceiveImage, this,
...
int CameraDetectionSubmodule::InitMotionService() {
  ...
  std::function<void(const MotionServiceMsgType &)> motion_service_callback =
      std::bind(&CameraDetectionSubmodule::OnMotionService, this,
                std::placeholders::_1);
...
void CameraDetectionSubmodule::OnMotionService(
...
int CameraDetectionSubmodule::MakeProtobufMsg(
...
int CameraDetectionSubmodule::ConvertObjectToPb(
...
int CameraDetectionSubmodule::ConvertObjectToCameraObstacle(
    const base::ObjectPtr &object_ptr,
...
int CameraDetectionSubmodule::ConvertLaneToCameraLaneline(
...
int CameraDetectionSubmodule::MakeCameraDebugMsg(
