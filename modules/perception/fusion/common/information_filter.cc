...
bool InformationFilter::Correct(
  ...
  global_uncertainty_ = global_uncertainty_.inverse(); // global_uncertainty now stores information matrix
  tmp_states_ = global_uncertainty_ * global_states_; // tmp_states_ is information vector
  cur_observation_uncertainty_ = cur_observation_uncertainty_.inverse(); // cur_observation_uncertainty_ is now the inverse of covariance matrix
  if (last_observation_init_) {
    last_observation_ = last_to_cur_transform_matrix_ * last_observation_; // propate to current time
    last_observation_uncertainty_ = // propate to current time
