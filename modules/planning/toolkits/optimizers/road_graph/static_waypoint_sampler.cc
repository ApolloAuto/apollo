...
bool StaticWaypointSampler::SamplePathWaypoints(
  ...
  const float kStaticStep = 10.0;
  const auto &ref_line = reference_line_info_->reference_line();
  const auto &waypoint =
      ref_line.reference_points().front().lane_waypoints().front();
  const auto &first_lane_id = waypoint.lane->id().id();
  const auto lane_info =
      PlanningContext::Instance()->FindLaneStaticS(first_lane_id);
  const float route_s = init_sl_point_.s() + lane_info.anchor_s +
                        (waypoint.s - lane_info.start_s);
  float init_s = init_sl_point_.s() - std::fmod(route_s, kStaticStep);
  if (init_s < 0.0f) {
    init_s = 0.0f;
  }

  const float total_length =
      std::fmin(init_s + std::fmax(init_point.v() * 8.0, kMinSampleDistance),
