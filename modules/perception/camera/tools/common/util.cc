...
bool FillCameraKMatrix(CameraFrame* frame, const std::string& file_name) {
  // We can read intrinsic two ways:
  // 1. SensorManager
  // 2. Read from file
  // Here we choose to read directly from the file for easy debugging
  base::BrownCameraDistortionModel model;
  bool res = common::LoadBrownCameraIntrinsic(file_name, &model);
  if (!res) {
    AERROR << "Load brown camera intrinsic file failed! " << file_name;
    return false;
  }
...
bool FillImage(CameraFrame* frame, const std::string& file_name) {
  ...
  bool res = frame->data_provider->FillImageData(
                  image.rows, image.cols, image.data, "bgr8");
  if (!res) {
