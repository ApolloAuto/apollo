 ...
 *****************************************************************************/
#include "modules/perception/traffic_light_detection/detector/caffe_detection/detection.h"
...
bool TrafficLightDetection::Init(const trafficlight::TrafficLightDetectorInitOptions &options) {
    ...
    if (!cyber::common::GetProtoFromFile(config_file, &detection_param_)) {
        AERROR << "Failed to load traffic light detection parameters: " << config_file;
        return false;
    }
    ...
    net_inputs_ = inference::GetBlobNames(model_info.inputs());
    ...
    rt_net_.reset(inference::CreateInferenceByName(
            model_type, proto_file, weight_file, net_outputs_, net_inputs_, model_path));
...
bool TrafficLightDetection::Inference(std::vector<base::TrafficLightPtr> *lights, camera::DataProvider *data_provider) {
    ...
    for (size_t i = 0; i < batch_num; ++i) {
        ...
        if (!camera::OutOfValidRegion(cbox, img_width, img_height) && cbox.Area() > 0) {
            ...
            inference::ResizeGPU(
                    *image_, input_img_blob, img_width, resize_index, mean_[0], mean_[1], mean_[2], true, 1.0);
            resize_index++;
        }
...
bool TrafficLightDetection::Detect(camera::TrafficLightFrame *frame) {
    ...
    const auto &data_provider = frame->data_provider.get();
    ...
    for (auto &light : lights_ref) {
        if (light->region.outside_image || camera::OutOfValidRegion(light->region.projection_roi, img_width, img_height)
...
bool TrafficLightDetection::SelectOutputBoxes(
    ...
    auto model_type = detection_param_.info().framework();
    int result_box_num, each_box_length;
    if (model_type == common::TensorRT) {
    ...
    for (int candidate_id = 0; candidate_id < result_box_num; candidate_id++) {
        ...
        if (static_cast<int>(tmp->region.detect_class_id) >= 0) {
            ...
            if (camera::OutOfValidRegion(
                        tmp->region.detection_roi, crop_box_list.at(img_id).width, crop_box_list.at(img_id).height)
...
void TrafficLightDetection::ApplyNMS(std::vector<base::TrafficLightPtr> *lights, double iou_thresh) {
    if (lights == nullptr) {
