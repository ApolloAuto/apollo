...
DistanceApproachIPOPTFixedTsInterface::DistanceApproachIPOPTFixedTsInterface(
  ...
  l_start_index_ = control_start_index_ + (horizon_ + 1);
...
bool DistanceApproachIPOPTFixedTsInterface::get_nlp_info(
  ...
  nnz_jac_g = 20 * horizon_ +
              2 * horizon_ - 1
              + tmp +
              (num_of_variables_ - (horizon_ + 1) + 2);
  CHECK_EQ(nnz_g, nnz_jac_g)
      << "nnz_jac not match:"
      << "nnz_g: " << nnz_g
      << "nnz_jac_g: " << nnz_jac_g;
  */
...
bool DistanceApproachIPOPTFixedTsInterface::get_bounds_info(
  ...
  */
  ...
  for (int i = 0; i < horizon_ + 1; ++i) {
    for (int j = 0; j < obstacles_num_; ++j) {
      // a. norm(A'*lambda) <= 1
      g_l[constraint_index] = 0.0; // previous is -inf
...
bool DistanceApproachIPOPTFixedTsInterface::eval_jac_g(
  ...
  // eval_jac_g_ser(n, x, new_x, m, nele_jac, iRow, jCol, values);
  
  // AERROR << "nelejac: " << nele_jac;
  // AERROR << "nnz_g: " << nnz_g;
  if (values == nullptr) {
    // return the structure. This is a symmetric matrix, fill the lower left
    // triangle only.
    for (int idx = 0; idx < nnz_g; idx++) {
      iRow[idx] = rind_g[idx];
      jCol[idx] = cind_g[idx];
    /*
      CHECK_EQ(iRow[idx], rind_g[idx]) << "jacobian row check fail! "
        << "idx: " << idx
        << "iRow: " << iRow[idx]
        << "rind_g: " << rind_g[idx];

      CHECK_EQ(jCol[idx], cind_g[idx]) << "jacobian col check fail! "
        << "idx: " << idx
        << "jCol: " << jCol[idx]
        << "cind_g: " << cind_g[idx];
    */
    ...
    // triangle only
    sparse_jac(tag_g, m, n, 1, const_cast<double*>(x), &nnz_g,
                &rind_g, &cind_g, &jacval, options_L);
    for (int idx = 0; idx < nnz_L; idx++) {
      /*
      if (std::abs(values[idx] - jacval[idx]) > 1e-6) {
          AERROR << "jacobian check fail at "
              << "idx, "
              << ", values: " << values[idx]
              << ", jacval: " << jacval[idx];
      }
...
bool DistanceApproachIPOPTFixedTsInterface::eval_jac_g_ser(
  ...
  */
  if (values == nullptr) {
    ...
    ADEBUG << "dynamic constraints to : " << nz_index;
    ...
    // First one, with respect to time
    // iRow[nz_index] = constraint_index;
    // jCol[nz_index] = time_index;
    // ++nz_index;
    ...
    ADEBUG << "control constraints to: " << nz_index;
    ...
    */
    ...
    ADEBUG << "obs constaint jacobian to : " << nz_index;
    ...
    */
    ...
    ADEBUG << "state constraint jacobian to: " << nz_index;
    ...
    for (int i = 0; i < horizon_; ++i) {
      ...
      values[nz_index] =
          ts_ *
          (x[state_index + 3] + ts_ * 0.5 * x[control_index + 1]) *
          std::sin(x[state_index + 2] +
                   ts_ * 0.5 * x[state_index + 3] *
      ...
      values[nz_index] =
          ...
          (ts_ *
           ...
           ts_ *
               (x[state_index + 3] +
                ts_ * 0.5 * x[control_index + 1]) *
               (-1) * ts_ * 0.5 * std::tan(x[control_index]) /
               wheelbase_ *
               std::sin(x[state_index + 2] +
                        ts_ * 0.5 * x[state_index + 3] *
      ...
      values[nz_index] =
          ...
          (ts_ *
           ...
           ts_ *
               (x[state_index + 3] +
                ts_ * 0.5 * x[control_index + 1]) *
                ts_ * 0.5 * std::tan(x[control_index]) /
                wheelbase_ *
                std::cos(x[state_index + 2] +
                         ts_ * 0.5 * x[state_index + 3] *
...
void DistanceApproachIPOPTFixedTsInterface::finalize_solution(
  ...
  time_result_(0, horizon_) = ts_; // x[time_index];
...
//***************    start ADOL-C part ***********************************
template <class T>
bool DistanceApproachIPOPTFixedTsInterface::eval_obj(int n, const T* x, T* obj_value) {
  ADEBUG << "eval_obj";
  // Objective is :
  // min control inputs
  // min input rate
...
template <class T>
bool DistanceApproachIPOPTFixedTsInterface::eval_constraints(
...
*/
  ...
  ADEBUG << "eval_constraints done!";
  return true;
}

bool DistanceApproachIPOPTFixedTsInterface::check_g(int n, const double* x, int m,
                                             const double* g) {
  int kN = n;
  int kM = m;
  double x_u_tmp[kN];
  ...
  // m4 : obstacle constraints
  int m4 = m2 + 4 * obstacles_num_ * (horizon_ + 1);
  ...
  // lambda_horizon_
  int m10 = m8 + lambda_horizon_;

  // miu_horizon_
  int m11 = m10 + miu_horizon_;

  AERROR << "dynamics constatins to: " << m1;
  AERROR << "control rate constraints (only steering) to: " << m2;
  // AERROR << "sampling time equality constraints to: " << m3;
  AERROR << "obstacle constraints to: " << m4;
  AERROR << "start conf constraints to: " << m5;
  AERROR << "constraints on x,y,v to: " << m6;
  AERROR << "end constraints to: " << m7;
  AERROR << "control bnd to: " << m8;
  // AINFO << "time interval constraints to: " << m9;
  AERROR << "lambda constraints to: " << m10;
  AERROR << "miu constraints to: " << m11;
  AERROR << "total variables: " << num_of_variables_;

  for (int idx = 0; idx < m; ++idx) {
    if (g[idx] > g_u_tmp[idx] + delta_v || g[idx] < g_l_tmp[idx] - delta_v) {
      AERROR << "constratins idx unfeasible: " << idx << ", g: " << g[idx]
...
void DistanceApproachIPOPTFixedTsInterface::generate_tapes(
    int n, int m, int* nnz_h_lag) {
  ...
  options_L[2] = 0;
  options_L[3] = 0;
  sparse_jac(tag_g, m, n, 0, &xp[0], &nnz_g, &rind_g, &cind_g, &jacval,
             options_L);
