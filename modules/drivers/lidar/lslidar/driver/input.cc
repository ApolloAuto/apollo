...
InputSocket::InputSocket(uint16_t port,std::string lidar_ip,int packet_size) : Input(port,lidar_ip,packet_size){
  port_=port;
  sockfd_ = -1;
...
//  myAddress.sin_addr.s_addr = inet_addr(lidar_ip.c_str());
...
int InputSocket::GetPacket(LslidarPacket *pkt) {
  ...
  while (true)
  {
    ...
    ssize_t nbytes = recvfrom(sockfd_, bytes, packet_size_, 0, (sockaddr*)&sender_address, &sender_address_len);
    if (nbytes < 0)
    {
...
InputPCAP::InputPCAP(uint16_t port, std::string lidar_ip, int packet_size,double packet_rate, std::string filename,
                         bool read_once, bool read_fast, double repeat_delay)
            : Input(port,lidar_ip,packet_size), packet_rate_(packet_rate), filename_(filename)
    {
        pcap_ = NULL;
        empty_ = true;
    ...
    int InputPCAP::GetPacket(LslidarPacket *pkt)
    {
        ...
        while (!apollo::cyber::IsShutdown())
        {
            ...
            if ((res = pcap_next_ex(pcap_, &header, &pkt_data)) >= 0)
            {
                ...
                // selected IP address.
                if (!lidar_ip_.empty() && (0 == pcap_offline_filter(&pcap_packet_filter_, header, pkt_data)))
                    continue;
...
//                }
            ...
            {
                AINFO<< "Error " << res << " reading lslidar packet: "  << pcap_geterr(pcap_);
                return -1;
            }
