...
TEST(DstExistFusionTest, test_update_with_measurement) {
  ...
  DstExistanceFusion existence_fusion(track);
  ...
  EXPECT_NEAR(existence_fusion.GetExistanceProbability(), 0.74, 1e-6);
  ...
  DstExistanceFusion existence_fusion_1(track_1);
  ...
  EXPECT_NEAR(existence_fusion_1.GetExistanceProbability(), 0.74, 1e-6);

  // radar will not update toic score
  existence_fusion_1.UpdateWithMeasurement(radar_sensor_object_ptr,
                                           151192277.124567989, 0);
  EXPECT_NEAR(existence_fusion_1.GetToicScore(), 0.71, 1e-2);
  EXPECT_NEAR(existence_fusion_1.GetExistanceProbability(), 0.74, 1e-3);
}
*/
...
TEST(DstExistFusionTest, test_update_without_measurement) {
  ...
  EXPECT_NEAR(existence_fusion.GetExistanceProbability(), 0.455, 1e-6);
  existence_fusion.UpdateWithoutMeasurement(
  ...
  EXPECT_NEAR(existence_fusion_1.GetExistanceProbability(), 0.5, 1e-3);
  ...
  DstExistanceFusion existence_fusion_2(track_2);
