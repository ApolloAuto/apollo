...
/**
 * @file lane_follow_command_processor.h
 **/
...
class LaneFollowCommandProcessor
    : public MotionCommandProcessorBase<LaneFollowCommand> {
 private:
  /**
   ...
   */
  bool Convert(const std::shared_ptr<LaneFollowCommand>& command,
  ...
  bool ProcessSpecialCommand(
      ...
      const std::shared_ptr<apollo::planning::PlanningCommand>&
          planning_command) const override;
};

CYBER_PLUGIN_MANAGER_REGISTER_PLUGIN(
    apollo::external_command::LaneFollowCommandProcessor, CommandProcessorBase)
