...
bool PullOver::IsPullOver() const {
  auto* planning_status = mutable_planning_status();
...
PullOver::ValidateStopPointCode PullOver::IsValidStop(
  ...
  const auto& pull_over_status = mutable_planning_status()->pull_over();
...
bool PullOver::CheckPullOverComplete() {
  ...
  if (adc_end_edge_s > stop_point_sl.s()) {
    mutable_planning_status()->mutable_pull_over()->set_status(
        PullOverStatus::DONE);
...
int PullOver::BuildPullOverStop(const PointENU& stop_point) {
  ...
  auto* pull_over_status = mutable_planning_status()->mutable_pull_over();
...
int PullOver::BuildInLaneStop(const PointENU& pull_over_stop_point) {
  ...
  mutable_planning_status()->clear_pull_over();
...
int PullOver::BuildStopDecision(const std::string& vistual_obstacle_id_postfix,
  ...
  const auto& pull_over_reason =
      mutable_planning_status()->pull_over().reason();
