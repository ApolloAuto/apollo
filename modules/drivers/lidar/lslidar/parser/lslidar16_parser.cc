        ...
        namespace lslidar {
            ...
            Lslidar16Parser::Lslidar16Parser(const Config &config)
                ...
                for (uint16_t rot_index = 0; rot_index < ROTATION_MAX_UNITS; ++rot_index) {
                    float rotation = ROTATION_RESOLUTION * static_cast<float>(rot_index * M_PI) / 180.0f;
            ...
            void Lslidar16Parser::GeneratePointcloud(
                ...
                if (config_.config_vert()) {
                    if (2 == version_data) {
                        for (int i = 0; i < 16; i++) {
                            uint16_t vert_angle = static_cast<uint16_t>(difop_ptr[234 + 2 * i] * 256 +
                                                                        difop_ptr[234 + 2 * i + 1]);
                        ...
                        for (int i = 0; i < 16; i++) {
                            uint16_t vert_angle = static_cast<uint16_t>(difop_ptr[245 + 2 * i] * 256 +
                                                                        difop_ptr[245 + 2 * i + 1]);
...
//                double scan_altitude_real_value[16] = {-0.261799,0.0174533,-0.226893,0.0513127,-0.191986,0.084823,
//                                                       -0.15708,0.118857,-0.122173,0.152716,-0.0872665,0.185703,
//                                                       -0.0523599,0.219911,-0.0174533,0.253945};
//                for (int i = 0; i < 16; ++i) {
////                    AERROR << "scan_altitude: " <<scan_altitude[i];
//                    scan_altitude[i] = scan_altitude_real_value[i];
//                }
...
//    packet_end_time = *(reinterpret_cast<uint64_t>(const_cast<uint64_t>(pkt.stamp())));
                ...
                for (int block = 0; block < BLOCKS_PER_PACKET; block++, block_num++) {
                    ...
                    for (int firing = 0, k = 0; firing < LSC16_FIRINGS_PER_BLOCK; ++firing) {
                        for (int dsr = 0; dsr < LSC16_SCANS_PER_FIRING; ++dsr, k += RAW_SCAN_SIZE) {
                            point_count++;
                            ...
                            if (2 == config_.return_mode()) {
                                ...
                                if (previous_packet_time > 1e-6) {
                                    point_time = current_packet_time - (SCANS_PER_PACKET - point_count - 1) *
                                                                       (current_packet_time - previous_packet_time) /
                                                                       (SCANS_PER_PACKET);
                                } else {              //fist packet
                                    point_time = current_packet_time;
                                }
//			point_time = packet_end_time * 1000 - 100000 * (5 - block/2) - 3125 * (31 - firing * 16 - dsr);
                            } else {
                                // modify
                                if (previous_packet_time > 1e-6) {
                                    point_time = current_packet_time - (SCANS_PER_PACKET - point_count - 1) *
...
//          if(2 == config_.return_mode()){
//              point_time = packet_end_time * 1000 - 49152 * (5 - block/2) - 3125 * (31 - dsr);
//          }else{
//              point_time = packet_end_time * 1000 - 49152 * (11 - block) - 3125 * (31 - dsr);
//          }
