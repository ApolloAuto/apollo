...
Spline2d::Spline2d(const std::vector<double>& t_knots, const std::size_t order)
    : t_knots_(t_knots), spline_order_(order) {
  if (t_knots.size() > 1) {
    for (std::size_t i = 1; i < t_knots_.size(); ++i) {
...
bool Spline2d::set_splines(const Eigen::MatrixXd& params,
  ...
  if (2 * t_knots_.size() * order !=
      2 * order + static_cast<std::size_t>(params.rows())) {
    return false;
  }
...
// returned;
Spline2dSeg* Spline2d::mutable_smoothing_spline(const std::size_t index) {
  return &splines_[index];
}

const Spline2dSeg& Spline2d::smoothing_spline(const std::size_t index) const {
  return splines_[index];
}

const std::vector<double>& Spline2d::t_knots() const { return t_knots_; }

std::size_t Spline2d::spline_order() const { return spline_order_; }
