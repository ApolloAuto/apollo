...
bool ReedShepp::SetRSP(int size, double lengths[], char types[],
                       std::vector<ReedSheppPath>* all_possible_paths) {
  ReedSheppPath path;
...
bool ReedShepp::GenerateLocalConfigurations(
  ...
  for (auto& path : *all_possible_paths) {
    ...
    pd = d;
    for (size_t i = 0; i < path.segs_types.size(); i++) {
      ...
      index--;
      if (i >= 1 && path.segs_lengths.at(i - 1) * path.segs_lengths.at(i) > 0) {
        pd = -d - ll;
      } else {
        pd = d - ll;
      }
    ...
    path.gear = pgear;
    for (size_t i = 0; i < path.segs_lengths.size(); i++) {
...
void ReedShepp::Interpolation(int index, double pd, char m, double ox,
