...
public:
    bool OpenSpaceROI() {
        ...
        double start_s = center_line_s - planner_open_space_config_.roi_config().roi_longitudinal_range_start();
        double end_s = center_line_s + planner_open_space_config_.roi_config().roi_longitudinal_range_end();
        ...
        Vec2d start_right = Vec2d(start_right_width * start_right_vec_cos, start_right_width * start_right_vec_sin);
        start_right = start_right + start_point;
        Vec2d start_left = Vec2d(start_left_width * start_left_vec_cos, start_left_width * start_left_vec_sin);
        start_left = start_left + start_point;
        Vec2d end_right = Vec2d(end_right_width * end_right_vec_cos, end_right_width * end_right_vec_sin);
    ...
    bool VPresentationObstacle(const std::string& lane_id, const std::string& parking_id) {
        ...
        ACHECK(cyber::common::GetProtoFromFile(FLAGS_planner_open_space_config_filename, &planner_open_space_config_))
    ...
    bool LoadMap(const std::string& lane_id, const std::string& parking_id) {
        ...
        LaneSegment nearby_lanesegment
                = LaneSegment(nearby_lane, nearby_lane->accumulate_s().front(), nearby_lane->accumulate_s().back());
    ...
    std::vector<double>* GetROIXYBoundary() {
    ...
    std::vector<double>* GetEndPose() {
        return &open_space_end_pose_;
    }
    double GetOriginHeading() {
        return origin_heading_;
    }
    Vec2d GetOriginPose() {
        return origin_point_;
    }
    Polygon2d GetParkingSpotBox() {
        return parking_spot_box_;
    }
...
bool ROITest(
    ...
    std::vector<std::vector<Vec2d>>* unrotated_roi_boundary_ = test_ptr->GetNoRotateROIParkingBoundary();
