...
#include "modules/cyber_recorder_gui_simple/player/play_task_producer.h"
...
PlayTaskProducer::PlayTaskProducer(
        const TaskBufferPtr& task_buffer,
...
bool PlayTaskProducer::ReadRecordInfo() {
    ...
    for (auto& file : play_param_.files_to_play) {
        ...
        // std::cout << "header.begin_time()=" << header.begin_time() << std::endl;
        // std::cout << "earliest_begin_time_=" << earliest_begin_time_ << std::endl;

        // std::cout << "header.end_time()=" << header.end_time() << std::endl;
        // std::cout << "latest_end_time_=" << latest_end_time_ << std::endl;

        if (header.begin_time() < earliest_begin_time_) {
            earliest_begin_time_ = header.begin_time();
            ADEBUG << "update header.begin_time()";
        }
...
bool PlayTaskProducer::UpdatePlayParam() {
    if (play_param_.begin_time_ns < earliest_begin_time_) {
        AINFO << play_param_.begin_time_ns << "  " << play_param_.base_begin_time_ns;
        play_param_.begin_time_ns = earliest_begin_time_;
        play_param_.base_begin_time_ns = earliest_begin_time_;
        AINFO << "play_param_.begin_time_ns < earliest_begin_time_"
               << "   play_param_.begin_time_ns =" << play_param_.begin_time_ns
               << "   play_param_.base_begin_time_ns =" << play_param_.base_begin_time_ns
               << "       earliest_begin_time_=" << earliest_begin_time_;
    }
    if (play_param_.start_time_s > 0) {
        play_param_.begin_time_ns = play_param_.base_begin_time_ns
...
void PlayTaskProducer::Reset(const double& progress_s) {
    ...
    record_viewer_ptr_ = std::make_shared<RecordViewer>(
            record_readers_, play_param_.begin_time_ns, play_param_.end_time_ns, play_param_.channels_to_play);
...
bool PlayTaskProducer::CreateWriters() {
    ...
    return CreatePlayTaskWriter(record_info_channel, "apollo.cyber.proto.RecordInfo");
}
