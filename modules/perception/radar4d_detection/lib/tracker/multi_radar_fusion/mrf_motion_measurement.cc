...
void MrfMotionMeasurement::ComputeMotionMeasurment(
...
void MrfMotionMeasurement::MeasurementSelection(
...
void MrfMotionMeasurement::MeasurementQualityEstimation(
    const TrackedObjectConstPtr& latest_object, TrackedObjectPtr new_object) {
  // 1. point size diff (only for same sensor)
  int pre_num = static_cast<int>(
      latest_object->object_ptr->radar4d_supplement.cloud_world.size());
