...
#ifndef ADU_PERCEPTION_OBSTACLE_VISUALIZER_CAMERA_H
#define ADU_PERCEPTION_OBSTACLE_VISUALIZER_CAMERA_H
...
public:
    ...
    void fit_screen_region(const Eigen::Vector2i& topLeft, Eigen::Vector2i& bottomRight);
    void center_scene();
    ...
    Eigen::Quaterniond get_rotatation_by_mouse_from_qgwidget(int preX, int preY, int x, int y);
    Eigen::Quaterniond deformed_ball_quaternion(int preX, 
    int preY, int x, int y, double cx, double cy);
    ...
    return static_cast<double>(_screen_width) / static_cast<double>(_screen_height); 
    };
    ...
    virtual void get_ortho_width_height(GLdouble& halfWidth, GLdouble& halfHeight) const;
    ...
    setscreen_widthandheight(int(100.0*aspect), 100); 
    };
    ...
    void set_scene_bounding_box(const Eigen::Vector3d& min, const Eigen::Vector3d& max);
    ...
    Eigen::Vector3d unprojectedcoordinates_of(const Eigen::Vector3d& src, 
                                              const Frame* frame = NULL) const;
    void get_projectedcoordinates_of(const double src[3], 
                                     double res[3], const Frame* frame = NULL) const;
    void get_unprojectedcoordinates_of(const double src[3], 
                                       double res[3], const Frame* frame = NULL) const;
    void convert_click_to_line(const Eigen::Vector2i& pixel, 
                               Eigen::Vector3d& orig, Eigen::Vector3d& dir) const;
