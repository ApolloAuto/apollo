...
TEST(ProjectionCacheFrame, test) {
  ProjectionCacheObject projection_cache_object;
  projection_cache_object.SetStartInd(2);
  projection_cache_object.SetEndInd(5);
  ProjectionCacheFrame projection_cache_frame("lidar", 1.0);
  projection_cache_frame.AddObject(2, projection_cache_object);
  ...
  ProjectionCacheObject* projection_cache_object_ptr =
      new ProjectionCacheObject();
  EXPECT_TRUE(
      projection_cache_frame.QueryObject(2, projection_cache_object_ptr));
  EXPECT_FALSE(
      projection_cache_frame.QueryObject(1, projection_cache_object_ptr));
  delete projection_cache_object_ptr;
}

TEST(ProjectionCache, test) {
  ProjectionCacheObject projection_cache_object;
  projection_cache_object.SetStartInd(2);
  projection_cache_object.SetEndInd(5);
  ...
  projection_cache.AddObject("camera_obstacle", 1.0, 1,
                             projection_cache_object);
  ProjectionCacheObject* projection_cache_object_ptr =
      new ProjectionCacheObject();
  EXPECT_FALSE(projection_cache.QueryObject(
      "lidar2", 2.0, "camera_obstacle", 1.0, 1, projection_cache_object_ptr));
  EXPECT_FALSE(projection_cache.QueryObject(
      "lidar", 1.0, "camera_obstacle", 2.0, 1, projection_cache_object_ptr));
  EXPECT_FALSE(projection_cache.QueryObject(
      "lidar", 1.0, "camera_obstacle", 1.0, 2, projection_cache_object_ptr));
  EXPECT_TRUE(projection_cache.QueryObject("lidar", 1.0, "camera_obstacle", 1.0,
                                           1, projection_cache_object_ptr));
  delete projection_cache_object_ptr;
}

TEST(BoundedScalePositiveProbability, test) {
  double p = 0.2;
  double max_p = 0.4;
  double min_p = 0.3;
...
TEST(ScalePositiveProbability, test) {
  double p = 0.2;
  double max_p = 0.4;
  double th_p = 0.3;
...
TEST(FuseTwoProbabilities, test) {
  double p1 = 1.0;
  double p2 = 1.0;
  EXPECT_EQ(FuseTwoProbabilities(p1, p2), 1.0);
}

TEST(FuseMultipleProbabilities, test) {
...
TEST(ComputePtsBoxLocationSimilarity, test) {
  ProjectionCachePtr projection_cache_ptr = new ProjectionCache;
  ProjectionCacheObject projection_cache_object;
  base::BBox2DF camera_bbox = base::BBox2DF(100, 100, 200, 200);
  EXPECT_EQ(ComputePtsBoxLocationSimilarity(
                projection_cache_ptr, projection_cache_object, camera_bbox),
            1e-6);
  projection_cache_ptr->AddPoint(Eigen::Vector2f(101, 101));
  projection_cache_ptr->AddPoint(Eigen::Vector2f(101, 102));
  projection_cache_object.SetStartInd(0);
  projection_cache_object.SetEndInd(2);
  EXPECT_GT(ComputePtsBoxLocationSimilarity(
                projection_cache_ptr, projection_cache_object, camera_bbox),
            1 - 1e-3);
  delete projection_cache_ptr;
}

TEST(ComputePtsBoxShapeSimilarity, test) {
  ProjectionCachePtr projection_cache_ptr = new ProjectionCache;
  base::BBox2DF camera_bbox = base::BBox2DF(100, 100, 200, 200);
  ProjectionCacheObject projection_cache_object;
  EXPECT_EQ(ComputePtsBoxShapeSimilarity(projection_cache_ptr,
                                         projection_cache_object, camera_bbox),
            1e-3);
  projection_cache_object.SetStartInd(0);
  projection_cache_object.SetEndInd(2);
  projection_cache_object.SetBox(base::BBox2DF(100, 100, 200, 200));
  projection_cache_ptr->AddPoint(Eigen::Vector2f(100, 100));
  projection_cache_ptr->AddPoint(Eigen::Vector2f(200, 200));
  EXPECT_GE(ComputePtsBoxShapeSimilarity(projection_cache_ptr,
                                         projection_cache_object, camera_bbox),
            1 - 1e-3);
  delete projection_cache_ptr;
}

TEST(ComputePtsBoxSimilarity, test) {
  ProjectionCachePtr projection_cache_ptr = new ProjectionCache;
  ProjectionCacheObject projection_cache_object;
  base::BBox2DF camera_bbox = base::BBox2DF(100, 100, 200, 200);
  projection_cache_ptr->AddPoint(Eigen::Vector2f(100, 100));
  ...
  projection_cache_object.SetEndInd(2);
  projection_cache_object.SetBox(base::BBox2DF(100, 100, 200, 200));
  EXPECT_GT(ComputePtsBoxSimilarity(projection_cache_ptr,
                                    projection_cache_object, camera_bbox),
            1 - 1e-3);
  delete projection_cache_ptr;
  projection_cache_ptr = nullptr;
}

TEST(ComputeRadarCameraXSimilarity, test) {
  double velo_ct_x = 100;
  double camera_ct_x = 100;
  double size_x = 100;
  XSimilarityParams x_similarity_param;
  EXPECT_GT(ComputeRadarCameraXSimilarity(velo_ct_x, camera_ct_x, size_x,
                                          x_similarity_param),
            x_similarity_param.scale_positive_max_p_ - 1e-3);
}

TEST(ComputeRadarCameraYSimilarity, test) {
  double velo_ct_y = 110;
  double camera_ct_y = 100;
  double size_y = 100;
  YSimilarityParams y_similarity_param;
  EXPECT_GT(ComputeRadarCameraYSimilarity(velo_ct_y, camera_ct_y, size_y,
                                          y_similarity_param),
            y_similarity_param.bounded_scale_positive_max_p_ - 1e-3);
}

TEST(ComputeRadarCameraHSimilarity, test) {
  ...
  EXPECT_GT(ComputeRadarCameraHSimilarity(
                sensor_radar_ptr, camera_const_sensor_ptr, size_y,
                radar_box2d_vertices, h_similiar_params),
            h_similiar_params.scale_positive_max_p_ - 1e-3);
}

TEST(ComputeRadarCameraWSimilarity, test) {
  std::vector<Eigen::Vector2d> radar_box2d_vertices;
  radar_box2d_vertices.push_back(Eigen::Vector2d(0, 0));
  ...
  EXPECT_GT(
      ComputeRadarCameraWSimilarity(radar, width, size_x, radar_box2d_vertices,
                                    w_similarity_params),
      w_similarity_params.bounded_scale_positive_max_p_ - 1e-3);
}

TEST(ComputeRadarCameraLocSimilarity, test) {
  base::ObjectPtr camera_object_ptr =
      std::shared_ptr<base::Object>(new base::Object);
  ...
  EXPECT_GT(ComputeRadarCameraLocSimilarity(radar_ct, camera_const_sensor_ptr,
                                            world2camera_pose,
                                            loc_similiarity_params),
            loc_similiarity_params.scale_positive_max_p_ - 1e-3);
}

TEST(ComputeRadarCameraVelocitySimilarity, test) {
  base::ObjectPtr camera_object_ptr =
      std::shared_ptr<base::Object>(new base::Object);
