 ...
 *****************************************************************************/
#include "modules/perception/submodules/lidar_segmentation_submodule.h"
...
uint32_t LidarSegmentationSubmodule::s_seq_num_ = 0;
std::mutex LidarSegmentationSubmodule::s_mutex_;

bool LidarSegmentationSubmodule::Init() {
  LidarSegmentationSubmoduleConfig comp_config;
  if (!GetProtoConfig(&comp_config)) {
    return false;
  }
  ADEBUG << "Lidar Segmentation Submodule Configs: "
  ...
  if (!InitAlgorithmPlugin()) {
    AERROR << "Failed to init lidar segmentation submodule algorithm plugin.";
    return false;
  }
  return true;
}

bool LidarSegmentationSubmodule::Proc(
...
bool LidarSegmentationSubmodule::InitAlgorithmPlugin() {
  ...
  if (segmentor_ == nullptr) {
    AERROR << "sensor_name_ "
           << "Failed to get lidar segmentation instance";
    return false;
  }
  ...
  if (!segmentor_->Init(init_options)) {
    AINFO << "sensor_name_ "
          << "Failed to init lidar segmentation.";
    return false;
  }
...
bool LidarSegmentationSubmodule::InternalProc(
