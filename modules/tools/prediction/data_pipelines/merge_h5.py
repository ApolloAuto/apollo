        ...
        if os.path.isdir(path):
            ...
            for i, h5_file in enumerate(h5_files):
                ...
                features = np.concatenate(
                    (features, feature), axis=0) if features is not None else feature
        else:
        ...
        if os.path.isdir(path):
            ...
            for i, npy_file in enumerate(npy_files):
                ...
                features_go = np.concatenate(
                    (features_go, feature_go), axis=0) if features_go is not None else feature_go
                features_cutin = np.concatenate(
                    (features_cutin,
                     feature_cutin),
                    axis=0) if features_cutin is not None else feature_cutin
        else:
            print("Fail to find", path)
            os._exit(-1)
