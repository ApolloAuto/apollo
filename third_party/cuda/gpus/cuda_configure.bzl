...
`cuda_configure` depends on the following environment variables:
  ...
  * `CUDA_CLANG`: Wheter to use clang as a cuda compiler.
  * `CLANG_CUDA_COMPILER_PATH`: The clang compiler path that will be used for
    both host and device code compilation if CUDA_CLANG is 1.
...
def _enable_cuda(repository_ctx):
  if "NEED_CUDA" in repository_ctx.os.environ:
    enable_cuda = repository_ctx.os.environ["NEED_CUDA"].strip()
...
def _cuda_version(repository_ctx, cuda_toolkit_path, cpu_value):
  ...
  # Check whether CUDA_VERSION was set by the user and fail if it does not
  # match the detected version.
  environ_version = ""
  if _CUDA_VERSION in repository_ctx.os.environ:
    environ_version = repository_ctx.os.environ[_CUDA_VERSION].strip()
  if environ_version and not _matches_version(environ_version, full_version):
    auto_configure_fail(
        ("CUDA version detected from nvcc (%s) does not match " +
         "CUDA_VERSION (%s)") % (full_version, environ_version))
...
def _cudnn_version(repository_ctx, cudnn_install_basedir, cpu_value):
  ...
  # Check whether CUDNN_VERSION was set by the user and fail if it does not
  # match the detected version.
  environ_version = ""
  if _CUDNN_VERSION in repository_ctx.os.environ:
    environ_version = repository_ctx.os.environ[_CUDNN_VERSION].strip()
  if environ_version and not _matches_version(environ_version, full_version):
...
def _compute_capabilities(repository_ctx):
  ...
  if _CUDA_COMPUTE_CAPABILITIES not in repository_ctx.os.environ:
    return _DEFAULT_CUDA_COMPUTE_CAPABILITIES
  capabilities_str = repository_ctx.os.environ[_CUDA_COMPUTE_CAPABILITIES]
...
def _use_cuda_clang(repository_ctx):
  if "CUDA_CLANG" in repository_ctx.os.environ:
    enable_cuda = repository_ctx.os.environ["CUDA_CLANG"].strip()
    if enable_cuda == "1":
