    ...
    std::shared_ptr<MapDataCheckerAgent> agent, bool &init_success)
{
    ...
    _node = apollo::cyber::CreateNode(std::string("cybernode_map_datachecker"));
    if (!_node)
    {
...
int MapDataCheckerCyberNode::create_channel_subscriber() {
    ...
    _bestgnsspos_reader = _node->CreateReader<apollo::drivers::gnss::GnssBestPose> (
        FLAGS_topic_bestgnsspos,
        [this](const std::shared_ptr<const apollo::drivers::gnss::GnssBestPose> &msg) {
            //AINFO << "call on_inspvax_callback";
            _agent->get_sp_pose_collection_agent()->on_bestgnsspos_callback(msg);
        }
    );
