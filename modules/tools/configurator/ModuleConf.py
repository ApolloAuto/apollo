...
class ModuleConf(object):
    ...
    def show(self):
        cornerx = 2
        cornery = 10
        self.field = Field(self.proto, self.stdscr, self.proto.DESCRIPTOR, cornerx,
                           cornery, self.proto, self.conf_file, False)
...
class Field(object):
    ...
    """
    def __init__(self, item, win, descriptor, cornerx, cornery, proto_root, conf_file, is_repeated):
        self.item = item
        self.select = 0
    ...
    def setup(self):        
        ...
        if self.descriptor.containing_type is not None and \
            ...
            printstring = self.descriptor.name + " [Index: " + str(self.index) + "]"
            self.win.addstr(1, 2, printstring, curses.A_BOLD)
            
            self.prefetch(self.item[self.index], self.descriptor, self.basex, self.basey, [self.index])
        else:
           ...
           descriptor.type == descriptor.TYPE_MESSAGE:
            for descript, item in entity.ListFields():                
                ...
                if descript.label == descript.LABEL_REPEATED:
                    ele = element(row, col, descript, item, self.cornerx, self.cornery, 
                                  self.item, list(descript_path), True)
                    self.elelist.append(ele)
                elif descript.type == descript.TYPE_MESSAGE:
                    self.win.addstr(row, col, descript.name + ": ")
                    row, col = self.prefetch(item, descript, row + 1, col + 2, descript_path)
                    row = row - 1
                    col = col - 2
                else:
    ...
    def print_out(self):
        ...
        for i in range(len(self.elelist)):
            self.elelist[i].print_element(self.win, True if i == self.select else False)
        self.win.refresh()
            ...
            if user_input == curses.KEY_DOWN:
                ...
                if self.select <len(self.elelist):
                    if self.elelist[self.select].is_repeated:
                        ...
                        field = Field(proto, self.win, descript, self.cornerx + 2, self.cornery + 10, 
...
class element(object):
    def __init__(self, row, col, descriptor, value, cornerx, cornery, proto_root, descript_path, is_repeated):
        self.row = row
        self.col = col
        self.descriptor = descriptor
        self.value = value
    ...
    def print_element(self, win, highlight):
        ...
        if self.is_repeated:
            ...
            enum_type = self.descriptor.enum_type.values_by_number[self.value].name
    ...
    def get_input(self, options, select):
        for i in range(len(options)):
            self.win.addstr(i, 0, options[i], curses.A_REVERSE if select == i else curses.A_NORMAL)
        self.win.refresh()
        self.win.keypad(1)
        while True:
            user_input = self.win.getch()
