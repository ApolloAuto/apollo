...
void ParserNodelet::onInit() {
  ...
  nh.param("imu_topic", imu_topic,
           std::string("/apollo/sensor/gnss/imu"));
  ...
  nh.param("eph", eph_topic,
           std::string("/apollo/sensor/gnss/rtk_eph"));
  nh.param("observation", observation_topic,
           std::string("/apollo/sensor/gnss/rtk_obs"));

  _data_parser.reset(new DataParser(nh, raw_data_topic,
                                    imu_topic, ins_stat_topic,
                                    corr_imu_topic, odometry_topic,
                                    gnss_status_topic, ins_status_topic,
                                    bestpos_topic, eph_topic,
                                    observation_topic));
