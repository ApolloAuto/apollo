...
//      BaseLaneDetectorRegisterer::GetInstanceByName("DenselineLaneDetector");
  ...
  for (int i = 1; i < 5; i++) {
    std::string impath =
        cv::format("/apollo/modules/perception/testdata/"
            "camera/lib/lane/postprocessor/"
            "denseline/data/test_%d.jpg", i);
    ...
    std::vector<std::vector<LanePointInfo> >
      laneline_point_set = lane_postprocessor->GetLanelinePointSet();
    std::vector<LanePointInfo>
      laneline_infer_set = lane_postprocessor->GetAllInferLinePointSet();
    ...
    lane_postprocessor->GetLaneCCs(&lane_map, &lane_map_width,
      &lane_map_height, &ccs, &select_ccs);
    // delete detector;
  }
}
