...
class StopSignUnprotectedStageIntersectionCruiseTest : public ::testing::Test {
 public:
  virtual void SetUp() {
...
TEST_F(StopSignUnprotectedStageIntersectionCruiseTest, Init) {
  StopSignUnprotectedStageIntersectionCruise
      stop_sign_unprotected_stage_intersection_cruise(
      stop_sign_unprotected_config_.stage_config(3));
  EXPECT_EQ(stop_sign_unprotected_stage_intersection_cruise.Name(),
      ScenarioConfig::StageType_Name(
      stop_sign_unprotected_config_.stage_config(3).stage_type()));
}
