...
本产品采用的是模块化锂电池组，电池组有正面和侧面两个操作面。产品外形如下图所示：
![Battery_1](images/Battery_1.png)![Battery_2](images/Battery_2.png)
首先把电池箱可靠固定到整车安装位置，再将电池箱充放电接口与通讯接口与相应整车上的对接插件进行可靠的电气连接后，此时按下控制面板上的电源按钮，按钮出现红色背光，为整车上电状态，电池箱对外放电；若再次按下电源按钮，按钮弹起，红色背光消失，为整车下电状态，电池箱不对外放电。
![Battery_3](images/Battery_3.png)![Battery_BAT_4](images/Battery_BAT_4.png)
    

## 2.2 取放方式

电池组安装在底盘下方的电池舱内部，安装方式如下： 

1. 确保车辆停放在安全位置，全车处于断电状态，即控制面板上的电源按钮位于关闭位置（弹起状态且红色背光灯不亮）；
2. 将电池组放入电池舱内，并用4个螺栓将电池可靠固定，如下图所示：
   ![Battery_5](images/Battery_5.png)
3. 将高压充/放电连接器、低压通讯连接器与整车上的相应连接器连接妥当，如下图所示：
   ![Battery_6](images/Battery_6.png)![Battery_7](images/Battery_7.png)![Battery_8](images/Battery_8.png)![Battery_9](images/Battery_9.png)![Battery_10](images/Battery_10.png)
4. 最后将电池舱盖板用螺栓与整车可靠固定，确保盖板不会晃动，完成电池箱的安装。
   如果需要拆卸电池组，步骤相似，具体可参照以上安装步骤。
   •	警告：在确保全车断电即电源按钮处于关闭状态下进行操作。
...
- 请先参照本手册2.2的操作，并确保充电接口可靠连接；
- 取出随车充电枪，可靠的对插到车辆正后方充电口处，进行充电操作：
    ![Battery_11](images/Battery_11.png)![Battery_12](images/Battery_12.png)
    上电步骤：先插充/放电插头，后接通220V电源
    下电步骤：先关电源开关，后拔充/放电插头；
    警告
    •	警告：充电时，严禁触碰电池组输出端口！
...
1. 上电
   •   按照2-2中的操作步骤，将电池组装入底盘电池舱且所有的电气插件可靠连接；
   •   将电源按钮置于开启位置（按钮红色背光亮起），整车上电，此时车辆可以执行车辆具备的所有功能，如下图所示：
   ![Battery_BAT_13](images/Battery_BAT_13.png)

2. 断电
   •   将电源按钮置于关闭位置，即可完成整车断电，如下图所示：
   ![Battery_BAT_4](images/Battery_BAT_4.png)
...
![AB_CONTROL](images/AB_CONTROL.JPG)
...
车辆前进时，当前4个超声波（8号~11号）中，有任意一个检测到达到触发值时，就会触发AEB功能，正前方9~10号超声波设置的安全距离大于等于90cm；两侧的（8号和11号）超声波安全距离大于等于40cm；

2. 倒车触发AEB条件


车辆倒车时，当检测到正后方两个超声波#3-#4中，有一个达到触发值就会触发AEB功能，后侧的安全距离大于等于40cm；	
...
- 工控机用户名:`apollo`
- 工控机密码:`apollo`
- 工控机网卡固定IP可参考文件`/etc/netplan/01-network-manager-all.yaml`
```
...
A：首先检查遥控器是不是已经放权，以及急停按钮是否处在弹起状态，以及A/B键是否复位，如果依然无法进入自动驾驶，请重启车辆确认是否可以进入自动驾驶，如不可行，可通过第8章的方法查看底盘是否有故障上报。
...
- 注：读到的超声波的值，乘以0.01724就是实际的距离，单位为：cm
