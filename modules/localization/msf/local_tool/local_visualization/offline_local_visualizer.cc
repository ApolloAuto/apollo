...
OfflineLocalVisualizer::OfflineLocalVisualizer() : 
    _map_config(), _resolution_id(0), _zone_id(0),
    _visual_engine()  {
    _map_folder = "";
    _pcd_folder = "";
    _gnss_loc_file = "";
...
bool OfflineLocalVisualizer::Init(const std::string &map_folder,
    ...
    success = _visual_engine.Init(_map_folder, _map_config, 
...
void OfflineLocalVisualizer::Visualize() {
    for (unsigned int idx = 0; idx < _pcd_timestamps.size(); idx++) {
        ...
        apollo::localization::msf::velodyne::LoadPcds(pcd_file_path, 
                idx, lidar_pose, pt3ds, intensities, false);
...
void OfflineLocalVisualizer::PoseInterpolationByTime(const std::vector<Eigen::Affine3d> &in_poses, 
    ...
    for (size_t i = 0; i < ref_timestamps.size(); i++) {
        ...
        if (index < in_timestamps.size()) {
            if (index >= 1) {
                ...
                double t = (cur_timestamp - ref_timestamp) 
