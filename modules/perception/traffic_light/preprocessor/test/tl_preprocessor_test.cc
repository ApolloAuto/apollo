...
TEST_F(TLPreprocessorTest, test_add_cached_lights_projections) {
  ...
  for (size_t i = 0; i < 300; ++i) {
    ASSERT_TRUE(
        _preprocessor->AddCachedLightsProjections(pose, signals, timestamp));
  }
...
TEST_F(TLPreprocessorTest, test_select_image) {
...

  {
    ...
    _preprocessor->select_image(pose, lights_on_image_array,
...
TEST_F(TLPreprocessorTest, test_set_get_max_cached_image_lights_array_size) {
  ...
  ASSERT_TRUE(_preprocessor->get_max_cached_image_lights_array_size(
