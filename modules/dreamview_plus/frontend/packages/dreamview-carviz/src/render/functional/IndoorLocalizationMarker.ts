...
// 起始点绘制工具
export default class IndoorLocalizationMarker extends BaseMarker {
    ...
    constructor(protected context: IPathwayMarkerContext) {
        super(context);
        this.name = 'IndoorLocalizationMarker';
    ...
    triggerCallback(operation: FunctionalOperation) {
        if (typeof this.createPathwayMarkerCallback === 'function') {
            this.createPathwayMarkerCallback(operation, {
                positions: this.indoorLocationMarkerPosition,
                lastPosition: this.lastPosition,
                origin: 'indoor_localization',
            });
        }
    }
    ...
    // 返回所有点的坐标
    get indoorLocationMarkerPosition() {
        const { coordinates } = this.context;
