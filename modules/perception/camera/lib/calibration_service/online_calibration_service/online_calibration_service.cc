...
bool OnlineCalibrationService::Init(
  ...
  for (auto iter = name_intrinsic_map.begin();
                iter != name_intrinsic_map.end(); iter++) {
    ...
    name_camera_status_map_.insert(std::pair<std::string, CameraStatus>(
                                                  iter->first, camera_status));
  }
...
bool OnlineCalibrationService::QueryDepthOnGroundPlane(int x,
                                                       int y,
                                                       double *depth) const {
  ...
  bool success = common::IBackprojectPlaneIntersectionCanonical(
                                  pixel,
                                  &(iter->second.k_matrix[0]),
                                  &(iter->second.ground_plane[0]),
                                  point);
  if (!success) {
    *depth = 0.0;
    return false;
