    ...
    for (size_t block_idx = 0; block_idx < BLOCKS_PER_PACKET; ++block_idx) {
        ...
        for (size_t scan_fir_idx = 0; scan_fir_idx < SCANS_PER_FIRING_C32; ++scan_fir_idx) {
            ...
            firings.distance[block_idx * 32 + scan_fir_idx] =
                    static_cast<double>(raw_distance.distance) * DISTANCE_RESOLUTION * distance_unit_;
...
void LslidarCXV4Parser::Unpack(const LslidarPacket& pkt,
        ...
        if (!is_get_scan_altitude_ && static_cast<uint16_t>(data[1211]) == 0x46) {            //old C32w
...
//                    ROS_INFO("firings.azimuth[fir_idx] +=389;");
            ...
            double conversionAngle = (vertical_angle_ == 90) ? CX4_conversionAngle_90 : CX4_conversionAngle_;
            if (coordinate_opt) {
...
//              && (-x_coord >= config_.bottom_left_y() && -x_coord <= config_.top_right_y()))  return;

            // computer the time of the point
            if (last_packet_time > 1e-6) {
                point_time =
                        current_packet_time - (current_packet_time - previous_packet_time) * (SCANS_PER_PACKET - fir_idx - 1) / SCANS_PER_PACKET;
            } else {
                point_time = current_packet_time;
            }
//            int remapped_scan_idx = (fir_idx % 32) % 4 * 8 + fir_idx % 32 / 4;
...
//            new_point.distance = firings.distance[fir_idx];
            ...
            if (firings.distance[fir_idx] > config_.max_range() || firings.distance[fir_idx] < config_.min_range()) {
            ...
            if ((firings.azimuth[fir_idx] < config_.scan_start_angle()) || (firings.azimuth[fir_idx] > config_.scan_end_angle())) continue;
            if (packet_number == 0) {
...
//                                    AERROR << "azimuth_corrected < 10000 ";
            ...
            if(firings.intensity[fir_idx]>172)  firings.intensity[fir_idx]=172;  //todo 如果不设置172，cyber_visualizer中显示会有问题，点云会显示不完整
            // 如果想要获取真实的强度值，则注释这一行
