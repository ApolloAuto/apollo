...
Apollo默认的算法流程为Detection->Recognition。Detection实现障碍物检测，Recognition实现障碍物匹配跟踪。在可以轻松更改或替换为不同的算法。每种算法的输入都是原始点云信息，输出都是目标级障碍物信息。本篇文档将介绍如何引入新的lidar检测算法，添加新算法的步骤如下：
