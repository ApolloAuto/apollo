...
void MeasureAnchorPointVelocity(
  ...
  Eigen::Vector3d measured_anchor_point_velocity =
    ...
    old_object->belief_anchor_point;
...
void MeasureBboxCenterVelocity(
  ...
  Eigen::Vector3d measured_bbox_center_velocity_with_old_dir = (new_center
      - old_center);
  ...
  Eigen::Vector3d project_dir = new_object->anchor_point -
    old_object->belief_anchor_point;
...
void MeasureBboxCornerVelocity(
  ...
  Eigen::Vector3d old_bbox_corner =
    old_center + old_dir * old_size(0) * 0.5 +
  ...
  Eigen::Vector3d ref_location
    = new_object->sensor_to_local_pose.translation();
  ...
  double min_center_distance
    = (ref_location - nearest_new_bbox_corner).norm();
  ...
  if (length_change < max_change_thresh
      && width_change < max_change_thresh) {
  ...
  for (size_t i = 0; i < 4; ++i) {
    ...
    Eigen::Vector3d bbox_corner_velocity_on_project_dir =
      common::Calculate2DXYProjectVector<double>(
          bbox_corner_velocity, project_dir);
