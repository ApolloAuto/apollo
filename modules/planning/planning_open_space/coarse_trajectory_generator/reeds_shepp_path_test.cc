...
public:
    virtual void SetUp() {
        ...
        reedshepp_test = std::unique_ptr<ReedShepp>(new ReedShepp(vehicle_param_, warm_start_conf_));
...
protected:
    ...
    WarmStartConfig warm_start_conf_;
...
TEST_F(reeds_shepp, test_set_1) {
    std::shared_ptr<Node3d> start_node
            = std::shared_ptr<Node3d>(new Node3d(0.0, 0.0, 10.0 * M_PI / 180.0, XYbounds_, warm_start_conf_));
    std::shared_ptr<Node3d> end_node
            = std::shared_ptr<Node3d>(new Node3d(7.0, -8.0, 50.0 * M_PI / 180.0, XYbounds_, warm_start_conf_));
...
TEST_F(reeds_shepp, test_set_3) {
    std::shared_ptr<Node3d> start_node
            = std::shared_ptr<Node3d>(new Node3d(0.0, 10.0, -10.0 * M_PI / 180.0, XYbounds_, warm_start_conf_));
    std::shared_ptr<Node3d> end_node
            = std::shared_ptr<Node3d>(new Node3d(-7.0, -8.0, -50.0 * M_PI / 180.0, XYbounds_, warm_start_conf_));
    std::shared_ptr<ReedSheppPath> optimal_path = std::shared_ptr<ReedSheppPath>(new ReedSheppPath());
    if (!reedshepp_test->ShortestRSP(start_node, end_node, optimal_path)) {
...
TEST_F(reeds_shepp, test_set_4) {
    std::shared_ptr<Node3d> start_node
            = std::shared_ptr<Node3d>(new Node3d(0.0, 10.0, -10.0 * M_PI / 180.0, XYbounds_, warm_start_conf_));
    std::shared_ptr<Node3d> end_node
            = std::shared_ptr<Node3d>(new Node3d(-7.0, -8.0, 150.0 * M_PI / 180.0, XYbounds_, warm_start_conf_));
    std::shared_ptr<ReedSheppPath> optimal_path = std::shared_ptr<ReedSheppPath>(new ReedSheppPath());
    if (!reedshepp_test->ShortestRSP(start_node, end_node, optimal_path)) {
        ADEBUG << "generating short RSP not successful";
    }
