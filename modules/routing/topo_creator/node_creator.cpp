...
double get_length_by_rate(double cur_s, double cur_total_length, double target_length) {
...
void NodeCreator::get_pb_node(const Lane& lane, const std::string& road_id, Node* pb_node) {
    init_node_info(lane, road_id, pb_node);
    init_node_cost(lane, pb_node);
}

void NodeCreator::add_out_boundary(const LaneBoundary& bound,
                      double lane_length,
    ...
    for (int i = 0; i < bound.boundary_type_size(); ++i) {
        ...
        range->mutable_start()->set_s(get_length_by_rate(bound.boundary_type(i).s(),
                                                         bound.length(),
                                                         lane_length));
        if (i != bound.boundary_type_size() - 1) {
            range->mutable_end()->set_s(get_length_by_rate(bound.boundary_type(i + 1).s(),
                                                           bound.length(),
                                                           lane_length));
        } else {
...
void NodeCreator::init_node_info(const Lane& lane, const std::string& road_id, Node* const node) {
    ...
    add_out_boundary(lane.right_boundary(), lane_length, node->mutable_right_out());
...
void NodeCreator::init_node_cost(const Lane& lane, Node* const node) {
    double lane_length = get_lane_length(lane);
    double speed_limit = (lane.has_speed_limit()) ? lane.speed_limit() : BASE_SPEED;
    double ratio = (speed_limit >= BASE_SPEED) ? (1 / sqrt (speed_limit / BASE_SPEED)) : 1.0;
