...
{
    double z = distance_to_scene_center() - zclipping_coefficient()*scene_radius();

    // Prevents negative or null zNear values.
    const double zMin = znear_coefficient() * zclipping_coefficient() * scene_radius();
    if (z < zMin)
    switch (type())
    {
...
{
    // make ORTHOGRAPHIC frustum fit PERSPECTIVE (at least in plane normal to view_direction(), passing
    // through RAP). Done only when CHANGING type since _ortho_coef may have been changed with a
...
void Camera::get_ortho_width_height(GLdouble& halfWidth, GLdouble& halfHeight) const
{
    const double dist = _ortho_coef * fabs(cameracoordinates_of(revolve_around_point())(2));
...
void Camera::compute_projection_matrix() const
{
    ...
    // same as gluPerspective( 180.0*field_of_view()/M_PI, aspect_ratio(), znear(), zfar() );
    break;
    }
...
void Camera::load_projection_matrix_stereo(bool leftBuffer) const
{
    ...
    case Camera::ORTHOGRAPHIC:
    std::cout<<"Camera::setProjectionMatrixStereo: Stereo not available with Ortho mode"<<std::endl;
    break;
    }
}
...
void Camera::load_model_view_matrix_stereo(bool leftBuffer) const
{
    ...
    double shift     = halfWidth * io_distance() / physicalscreen_width(); // * current window width / full screen width
...
/*! Fills \p m with the product of the ModelView and Projection matrices.
    ...
    Calls get_model_view_matrix() and get_projection_matrix() and then fills \p m with the product of these two matrices. */
...
/*! Similar to setscene_radius() and setscene_center(), but the scene limits are defined by a (world
    axis aligned) bounding box. */
void Camera::set_scene_bounding_box(const Eigen::Vector3d& min, const Eigen::Vector3d& max)
{
...
/*! setscene_center() to the result of point_under_pixel(\p pixel).
    ...
    Returns \c true if a point_under_pixel() was found and scene_center() was actually changed.

    See also set_revolve_around_point_from_pixel(). See the point_under_pixel() documentation. */
...
/*! Changes the revolve_around_point() to \p rap (defined in the world coordinate system). */
void Camera::set_revolve_around_point(const Eigen::Vector3d& rap)
{
    ...
    // _ortho_coef is used to compensate for changes of the revolve_around_point, so that the image does
...
double Camera::pixelgl_ratio(const Eigen::Vector3d& position) const
{
    ...
    {
    case Camera::PERSPECTIVE :
    return 2.0 * fabs((frame()->coordinates_of(position))(2)) * tan(field_of_view() / 2.0) / screen_height();
    case Camera::ORTHOGRAPHIC :
    {
    GLdouble w = 0.0;
    GLdouble h = 0.0;
...
Eigen::Vector3d Camera::point_under_pixel(const Eigen::Vector2i& pixel, bool& found) const
{
    double depth = 0.0;
    // Qt uses upper corner for its origin while GL uses the lower corner.
    glReadPixels(pixel(0), screen_height()-1-pixel(1), 1, 1, GL_DEPTH_COMPONENT, GL_FLOAT, &depth);
    found = depth < 1.0;
...
void Camera::fit_sphere(const Eigen::Vector3d& center, double radius)
{
    ...
    case Camera::ORTHOGRAPHIC :
    {
    distance = (center-revolve_around_point()).dot(view_direction()) + (radius / _ortho_coef);
    break;
    }
    }
...
void Camera::fit_bounding_box(const Eigen::Vector3d& min, const Eigen::Vector3d& max)
{
...
void Camera::fit_screen_region(const Eigen::Vector2i& topLeft, Eigen::Vector2i& bottomRight)
{
    const Eigen::Vector3d vd = view_direction();
    const double distToPlane = distance_to_scene_center();
    const Eigen::Vector2i center = (topLeft + bottomRight) / 2.0;

    Eigen::Vector3d orig;
    Eigen::Vector3d dir;
    convert_click_to_line( center, orig, dir );
    Eigen::Vector3d newCenter = orig + distToPlane / dir.dot(vd) * dir;

    convert_click_to_line( Eigen::Vector2i(topLeft(0), center(1)), orig, dir );
    const Eigen::Vector3d pointX = orig + distToPlane / dir.dot(vd) * dir;

    convert_click_to_line( Eigen::Vector2i(center(0), topLeft(1)), orig, dir );
    const Eigen::Vector3d pointY = orig + distToPlane / dir.dot(vd) * dir;
    ...
    case Camera::PERSPECTIVE :
    {
    const double distX = (pointX-newCenter).norm() / sin(horizontalfield_of_view() / 2.0);
    const double distY = (pointY-newCenter).norm() / sin(field_of_view() / 2.0);
    distance = std::max(distX, distY);
    break;
    }
    case Camera::ORTHOGRAPHIC :
    {
    const double dist = (newCenter-revolve_around_point()).dot(vd);
    //#CONNECTION# get_ortho_width_height
    const double distX = (pointX-newCenter).norm() / _ortho_coef / ((aspect_ratio() < 1.0) ? 1.0 : aspect_ratio());
    const double distY = (pointY-newCenter).norm() / _ortho_coef / ((aspect_ratio() < 1.0) ? 1.0 / aspect_ratio() : 1.0);
    distance = dist + std::max(distX, distY);
    break;
    }
    }

    Eigen::Vector3d newPos(newCenter - distance * vd);
    frame()->set_position(newPos);
}
...
{
    Eigen::Vector3d trans(static_cast<double>(deltaX), static_cast<double>(-deltaY), 0.0);
    // Scale to fit the screen mouse displacement
    switch (type())
    {
    case Camera::PERSPECTIVE :
    trans *= 2.0 * tan(field_of_view() / 2.0) *
    fabs((frame()->coordinates_of(revolve_around_point()))(2)) / screen_height();
    break;
    case Camera::ORTHOGRAPHIC :
    {
    GLdouble w = 0.0;
...
void Camera::rotate_by_mouse_from_qgwidget(int preX, int preY, int x, int y)
{
    ...
    Eigen::Quaterniond rot = deformed_ball_quaternion(preX,preY, x, y, trans[0], trans[1]);
    frame()->rotate_around_point(rot, revolve_around_point());
}

Eigen::Quaterniond Camera::get_rotatation_by_mouse_from_qgwidget(int preX, int preY, int x, int y) {
    Eigen::Vector3d trans = projectedcoordinates_of(revolve_around_point());
    Eigen::Quaterniond rot = deformed_ball_quaternion(preX,preY, x, y, trans[0], trans[1]);
    return rot;
}
...
void Camera::rotate_by_mouse(int deltaX, int deltaY)
{
    ...
    //frame()->rotate_around_point(rot, revolve_around_point());
    frame()->rotate(rot);
    frame()->set_position((position()-revolve_around_point()).norm() * tgtVec + revolve_around_point());
...
void Camera::rotate(Eigen::Vector3d i_axis, double i_angle)
{
    ...
    frame()->rotate_around_point(Eigen::Quaterniond(i_angle, i_axis(0), i_axis(1), i_axis(2)), revolve_around_point());
}

void Camera::setup_vector(const Eigen::Vector3d& up, bool noMove)
{
    Eigen::Quaterniond q = Eigen::Quaterniond().FromTwoVectors(Eigen::Vector3d(0.0, 1.0, 0.0),frame()->transform_of(up));

    if (!noMove){
    frame()->set_position(revolve_around_point() - (frame()->orientation() * q)._transformVector(frame()->coordinates_of(revolve_around_point())));}
...
static void setFromRotatedBasis(Eigen::Quaterniond& q, const Eigen::Vector3d& X, const Eigen::Vector3d& Y, const Eigen::Vector3d& Z)
{
    Eigen::Matrix3d m;
    double normX = X.norm();
    double normY = Y.norm();
...
void Camera::set_from_model_view_matrix(const GLdouble* const modelViewMatrix)
{
    ...
    set_orientation(q);
    set_position(-q._transformVector(Eigen::Vector3d(modelViewMatrix[12], modelViewMatrix[13], modelViewMatrix[14])));
}
...
void Camera::set_from_projection_matrix(const double matrix[12])
{
    ...
    Eigen::Vector3d line_0(matrix[ind(0, 0)],matrix[ind(0, 1)],matrix[ind(0, 2)]);
    Eigen::Vector3d line_1(matrix[ind(1, 0)],matrix[ind(1, 1)],matrix[ind(1, 2)]);
    Eigen::Vector3d line_2(matrix[ind(2, 0)],matrix[ind(2, 1)],matrix[ind(2, 2)]);
    ...
    const Eigen::Vector3d cam_pos = Eigen::Vector3d(det(matrix[ind(0, 1)],matrix[ind(0, 2)],matrix[ind(0,3)],
...
void Camera::get_cameracoordinates_of(const double src[3], double res[3]) const
{
    Eigen::Vector3d r = cameracoordinates_of(Eigen::Vector3d(src[0],src[1],src[2]));
    for (int i = 0; i < 3; ++i){
    res[i] = r[i];}
}
...
{
    Eigen::Vector3d r = worldcoordinates_of(Eigen::Vector3d(src[0],src[1],src[2]));
    for (int i = 0; i < 3; ++i){
    res[i] = r[i];}
}
...
Eigen::Vector3d Camera::projectedcoordinates_of(const Eigen::Vector3d& src, const Frame* frame) const
{
    GLdouble x = 0.0;
    GLdouble y = 0.0;
    GLdouble z = 0.0;
    ...
    gluProject(tmp(0),tmp(1),tmp(2), _model_view_matrix, _projection_matrix, viewport,    &x,&y,&z);
    }
    else
    gluProject(src(0),src(1),src(2), _model_view_matrix, _projection_matrix, viewport,    &x,&y,&z);

    return Eigen::Vector3d(x,y,z);
}

Eigen::Vector3d Camera::unprojectedcoordinates_of(const Eigen::Vector3d& src, const Frame* frame) const
{
    GLdouble x = 0.0;
    GLdouble y = 0.0;
    GLdouble z = 0.0;
    static GLint viewport[4];
    get_viewport(viewport);
    gluUnProject(src(0),src(1),src(2), _model_view_matrix,    _projection_matrix,    viewport,    &x,&y,&z);
    if (frame){
...
void Camera::get_projectedcoordinates_of(const double src[3], double res[3], const Frame* frame) const
{
    Eigen::Vector3d r = projectedcoordinates_of(Eigen::Vector3d(src[0],src[1],src[2]), frame);
    for (int i = 0; i < 3; ++i){
    res[i] = r[i];}
}

void Camera::get_unprojectedcoordinates_of(const double src[3], double res[3], const Frame* frame) const
{
    Eigen::Vector3d r = unprojectedcoordinates_of(Eigen::Vector3d(src[0],src[1],src[2]), frame);
    for (int i = 0; i < 3; ++i){
    res[i] = r[i];}
}

void Camera::convert_click_to_line(const Eigen::Vector2i& pixel, Eigen::Vector3d& orig, Eigen::Vector3d& dir) const
{
    switch (type())
    {
    case Camera::PERSPECTIVE:
    orig = position();
    dir = Eigen::Vector3d( ((2.0 * pixel(0) / screen_width()) - 1.0) * tan(field_of_view() / 2.0) * aspect_ratio(),
     ((2.0 * (screen_height() - pixel(1)) / screen_height()) - 1.0) * tan(field_of_view() / 2.0),
     -1.0 );
    dir = worldcoordinates_of(dir) - orig;
    dir.normalize();
    break;
    ...
    GLdouble h = 0.0; 
    get_ortho_width_height(w,h);
    orig = Eigen::Vector3d((2.0 * pixel(0) / screen_width() - 1.0) * w, -(2.0 * pixel(1) / screen_height() - 1.0) * h, 0.0);
    orig = worldcoordinates_of(orig);
    dir = view_direction();
    break;
    }
    }
...
void Camera::draw(bool drawFarPlane, double scale) const
{
    ...
    points[0](0) = points[1](0) = scale * double(hw);
    points[0](1) = points[1](1) = scale * double(hh);
    break;
    }
    }
    ...
    glVertex3f( points[farIndex](0),    points[farIndex](1), -points[farIndex](2));
    glVertex3f(0.0f, 0.0f, 0.0f);
    glVertex3f(-points[farIndex](0),    points[farIndex](1), -points[farIndex](2));
    glVertex3f(0.0f, 0.0f, 0.0f);
    glVertex3f(-points[farIndex](0), -points[farIndex](1), -points[farIndex](2));
...
{
    // If you change the size value, change angle computation in deformed_ball_quaternion().
...
Eigen::Quaterniond Camera::deformed_ball_quaternion(int preX, int preY, int x, int y, double cx, double cy)
{
