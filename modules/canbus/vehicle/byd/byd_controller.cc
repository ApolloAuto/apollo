...
#include "modules/canbus/vehicle/byd/byd_controller.h"

#include "modules/common/log.h"

#include "modules/canbus/can_comm/can_sender.h"
#include "modules/canbus/vehicle/byd/byd_message_manager.h"
...
ErrorCode LincolnController::Init(const VehicleParameter &params,
    ...
    if (is_initialized_) {
      AINFO << "BydController has already been initiated.";
    ...
    // sender part
    brake_e5_ = dynamic_cast<BrakeE5 *>(
        message_manager_->GetMutableProtocolDataById(BrakeE5::ID));
    if (brake_e5_ == nullptr) {
      AERROR << "BrakeE5 does not exist in the BydMessageManager!";
      return ErrorCode::CANBUS_ERROR;
    }

    throttle_e5_ = dynamic_cast<ThrottleE5 *>(
        message_manager_->GetMutableProtocolDataById(ThrottleE5::ID));
    if (throttle_e5_ == nullptr) {
      AERROR << "ThrottleE5 does not exist in the BydMessageManager!";
      return ErrorCode::CANBUS_ERROR;
    }

    steering_e5_ = dynamic_cast<SteeringE5 *>(
        message_manager_->GetMutableProtocolDataById(SteeringE5::ID));
    if (steering_e5_ == nullptr) {
      AERROR << "SteeringE5 does not exist in the BydMessageManager!";
      return ErrorCode::CANBUS_ERROR;
    }

    gear_e5_ = dynamic_cast<GearE5 *>(
        message_manager_->GetMutableProtocolDataById(GearE5::ID));
    if (gear_e5_ == nullptr) {
      AERROR << "GearE5 does not exist in the BydMessageManager!";
      return ErrorCode::CANBUS_ERROR;
    }

    turnsignal_e5_ = dynamic_cast<TurnsignalE5 *>(
        message_manager_->GetMutableProtocolDataById(TurnsignalE5::ID));
    if (turnsignal_e5_ == nullptr) {
      AERROR << "TurnsignalE5 does not exist in the BydMessageManager!";
      return ErrorCode::CANBUS_ERROR;
    }

    an_sender_->AddMessage(BrakeE5::ID, brake_e5_, false);
    can_sender_->AddMessage(ThrottleE5::ID, throttle_e5_, false);
    can_sender_->AddMessage(SteeringE5::ID, steering_e5_, false);
    can_sender_->AddMessage(GearE5::ID, gear_e5_, false);
    can_sender_->AddMessage(TurnsignalE5::ID, turnsignal_e5_, false);
...
bool BydController::Start() {
  if (!is_initialized_) {
    AERROR << "BydController has NOT been initiated.";
    return false;
  }
...
void LincolnController::Stop() {
  if (!is_initialized_) {
    AERROR << "BydController stops or starts improperly!";
    return;
  }
  ...
  if (thread_ != nullptr && thread_->joinable()) {
    thread_->join();
    thread_.reset();
    AINFO << "BydController stopped.";
  }
}

Chassis BydController::chassis() {
  chassis_.Clear();
...
void BydController::Emergency() {
  set_driving_mode(Chassis::EMERGENCY_MODE);
  ResetProtocol();
  set_chassis_error_code(Chassis::CHASSIS_ERROR);
}

ErrorCode BydController::EnableAutoMode() {
  if (driving_mode() == Chassis::COMPLETE_AUTO_DRIVE) {
    AINFO << "already in COMPLETE_AUTO_DRIVE mode";
    return ErrorCode::OK;
  }
  brake_e5_->set_enable();
  throttle_e5_->set_enable();
  steering_e5_->set_enable();
...
ErrorCode BydController::DisableAutoMode() {
  ResetProtocol();
  can_sender_->Update();
...
ErrorCode BydController::EnableSteeringOnlyMode() {
  if (driving_mode() == Chassis::COMPLETE_AUTO_DRIVE ||
  ...
  brake_e5_->set_disable();
  throttle_e5_->set_disable();
  steering_e5_->set_enable();
...
void BydController::Gear(Chassis::GearPosition gear_position) {
  if (!(driving_mode() == Chassis::COMPLETE_AUTO_DRIVE ||
  ...
  switch (gear_position) {
    case Chassis::GEAR_NEUTRAL: {
      gear_e5_->set_gear_neutral();
      break;
    }
    case Chassis::GEAR_REVERSE: {
      gear_e5_->set_gear_reverse();
      break;
    }
    case Chassis::GEAR_DRIVE: {
      gear_e5_->set_gear_drive();
      break;
    }
    case Chassis::GEAR_PARKING: {
      gear_e5_->set_gear_park();
      break;
    }
    case Chassis::GEAR_LOW: {
      gear_e5_->set_gear_low();
      break;
    }
    case Chassis::GEAR_NONE: {
      gear_e5_->set_gear_none();
      break;
    }
...
// -> pedal
void BydController::Brake(double pedal) {
  if (!(driving_mode() == Chassis::COMPLETE_AUTO_DRIVE ||
        driving_mode() == Chassis::AUTO_SPEED_ONLY)) {
    AINFO << "The current drive mode does not need to set acceleration.";
    return;
  }
  brake_e5_->set_pedal(pedal);
}
...
void LincolnController::Throttle(double pedal) {
  ...
  throttle_e5_->set_pedal(pedal);
}

// byd default, -470 ~ 470, left:+, right:-
// need to be compatible with control module, so reverse
// steering with old angle speed
// angle:-99.99~0.00~99.99, unit:%, left:-, right:+
void BydController::Steer(double angle) {
  if (!(driving_mode() == Chassis::COMPLETE_AUTO_DRIVE ||
        driving_mode() == Chassis::AUTO_STEER_ONLY)) {
  ...
  // reverse sign
  steering_e5_->set_steering_angle(real_angle)->set_steering_angle_speed(200);
}

// steering with new angle speed
// angle:-99.99~0.00~99.99, unit:%, left:-, right:+
// angle_spd:0.00~99.99, unit:deg/s
void BydController::Steer(double angle, double angle_spd) {
  if (!(driving_mode() == Chassis::COMPLETE_AUTO_DRIVE ||
        driving_mode() == Chassis::AUTO_STEER_ONLY)) {
    AINFO << "The current driving mode does not need to set steer.";
    return;
  ...
  steering_e5_->set_steering_angle(real_angle)
      ->set_steering_angle_speed(real_angle_spd);
}

void BydController::SetEpbBreak(const ControlCommand &command) {
...
void BydController::SetBeam(const ControlCommand &command) {
...
void BydController::SetHorn(const ControlCommand &command) {
...
void BydController::SetTurningSignal(const ControlCommand &command) {
  // Set Turn Signal
  auto signal = command.signal().turn_signal();
  if (signal == Signal::TURN_LEFT) {
    turnsignal_e5_->set_turn_left();
  } else if (signal == Signal::TURN_RIGHT) {
    turnsignal_e5_->set_turn_right();
  } else {
    turnsignal_e5_->set_turn_none();
  }
}

void BydController::ResetProtocol() {
  message_manager_->ResetSendMessages();
}

bool BydController::CheckChassisError() {
  // steer fault
  ChassisDetail chassis_detail;
  message_manager_->GetChassisDetail(&chassis_detail);
...
void BydController::SecurityDogThreadFunc() {
  ...
  while (can_sender_->IsRunning()) {
    ...
    if (elapsed < default_period) {
      ...
      AERROR
          << "Too much time consumption in BydController looping process:"
          << elapsed.count();
      start = end;
    }
  }
...
bool BydController::CheckResponse(const int32_t flags, bool need_wait) {
  // for byd, CheckResponse commonly takes 300ms. We leave a 100ms buffer
  // for it.
...
void BydController::set_chassis_error_mask(const int32_t mask) {
...
void BydController::set_chassis_error_code(
