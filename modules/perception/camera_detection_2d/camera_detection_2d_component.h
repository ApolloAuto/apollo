...
#include "modules/perception/camera_detection_2d/proto/camera_detection_2d.pb.h"
...
class CameraDetection2dComponent final
    : public cyber::Component<drivers::Image> {
 public:
  CameraDetection2dComponent() = default;
  ~CameraDetection2dComponent() = default;
  /**
 ...
 private:
  bool InitCameraFrame(const CameraDetection2d& detection_param);

  bool InitObstacleDetector(const CameraDetection2d& detection_param);

  bool InitTransformWrapper(const CameraDetection2d& detection_param);
 ...
 private:
  ...
  DISALLOW_COPY_AND_ASSIGN(CameraDetection2dComponent);
};

CYBER_REGISTER_COMPONENT(CameraDetection2dComponent);
