...
#ifdef DEBUG_NCUT
    AINFO << "normalized_cut done, #segments " << segment_clusters.size() << ", time: " << omp_get_wtime() - start_t;
    start_t = omp_get_wtime();
#endif
    // .4 _segment_pids;
...
void NCut::SuperPixelsFloodFill(
...
void NCut::PrecomputeAllSkeletonAndBbox() {
    ...
    for (size_t i = 0; i < _cluster_points.size(); ++i) {
        base::PointFCloudPtr pc = base::PointFCloudPtr(new base::PointFCloud(*_cloud_obstacles, _cluster_points[i]));
...
void NCut::SampleByGrid(
    ...
    base::PointFCloudPtr pc = base::PointFCloudPtr(new base::PointFCloud(*_cloud_obstacles, point_gids));
    ...
    for (int i = 0; i < num_points; ++i) {
        ...
        for (int r = 0; r < patch.rows; ++r) {
            for (int c = 0; c < patch.cols; ++c) {
                ...
                features.coeffRef(i, p++) = static_cast<float>((std::isnan(val) || std::isinf(val)) ? 1.e-50 : val);
...
#ifdef DEBUG_NCUT
            AINFO << "N cut cost is " << cost << ", seg1 size " << seg1->size() << ", seg2 size " << seg2->size();
            if (curr->size() < 50) {
                std::cout << "seg1: ";
...
// visualize_graph_cut(seg1, seg2);
    ...
    LOG_DEBUG << "normalized_cut: " << segment_clusters.size() << " segments from " << _cluster_points.size()
              << " clusters";
    ...
    for (int i = 0; i < num_clusters; ++i) {
        ...
        for (int j = i + 1; j < num_clusters; ++j) {
            ...
            ComputeSquaredSkeletonDistance(
                    _cluster_skeleton_points[i],
                    _cluster_skeleton_features[i],
                    _cluster_skeleton_points[j],
                    _cluster_skeleton_features[j],
                    &dist_point,
                    &dist_feature);
            if (dist_point > radius2) {
                ...
                weights.coeffRef(i, j) = static_cast<float>(exp(-dist_point / hs2) * exp(-dist_feature / hf2));
...
// std::cout << "eigen vectors:\n " << eigenvectors << std::endl << std::endl;
    ...
    const float increment = static_cast<float>((maxval - minval) / (static_cast<float>(_num_cuts) + 1.0f));
    int num_seg1 = 0;
    int num_seg2 = 0;
    float opt_split = 0.0;
...
#ifdef DEBUG_NCUT
        LOG_DEBUG << "seg1: " << num_seg1 << ", seg2: " << num_seg2 << ", split " << split << ", cut " << cut
                  << ", assoc1 " << assoc1 << ", assoc2 " << assoc2 << ", cost " << cost;
#endif
        // .2.3 find best cost
        if (cost < opt_cost) {
            opt_cost = cost;
...
void NCut::LaplacianDecomposition(const Eigen::MatrixXf &weights, Eigen::MatrixXf *eigenvectors_in) {
#ifdef DEBUG_NCUT
...
bool NCut::ComputeSquaredSkeletonDistance(
...
std::string NCut::GetClustersLabel(const std::vector<int> &cluster_ids) {
    ...
    base::PointFCloudPtr cloud = base::PointFCloudPtr(new base::PointFCloud(*_cloud_obstacles, point_ids));
...
void NCut::GetSegmentRoughSize(const base::PointFCloudPtr &cloud, float *length, float *width, float *height) {
    float x_max = -std::numeric_limits<float>::max();
    float y_max = -std::numeric_limits<float>::max();
    float z_max = -std::numeric_limits<float>::max();
    float x_min = std::numeric_limits<float>::max();
