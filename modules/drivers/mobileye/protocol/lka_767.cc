...
const int Lka767::ID = 0x767;

void Lka767::Parse(const uint8_t* bytes, int32_t length,
                   Mobileye* mobileye) const {
  mobileye->mutable_lka_767()->set_heading_angle(heading_angle(bytes, length));
  mobileye->mutable_lka_767()->set_view_range(view_range(bytes, length));
  mobileye->mutable_lka_767()->set_view_range_availability(
...
double Lka767::heading_angle(const uint8_t* bytes, int32_t length) const {
  Byte t0(bytes + 1);
...
// 'physical_unit': '"meter"'}
double Lka767::view_range(const uint8_t* bytes, int32_t length) const {
  Byte t0(bytes + 3);
...
// 'physical_unit': '""'}
bool Lka767::is_view_range_availability(const uint8_t* bytes,
