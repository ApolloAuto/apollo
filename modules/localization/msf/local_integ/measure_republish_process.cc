...
bool MeasureRepublishProcess::GnssHeadingProcess(const drivers::gnss::Heading& heading_msg,
  ...
  if (solution_status != 0) {
    LOG(INFO) << "the heading solution_status is not computed: " << solution_status;
    return false;
  }
  if ((position_type == 0) || (position_type == 1)) {
    LOG(INFO) << "the heading position_type is invalid or fixed: " << position_type;
    return false;
  }
  ...
  if (imu_ant_yaw_angle == 0.0) {
    imu_ant_yaw_angle = -atan2(
          ...
          imu_gnssant_extrinsic_.transform_1.translation()[0],
          imu_gnssant_extrinsic_.transform_2.translation()[1] -
          imu_gnssant_extrinsic_.transform_1.translation()[1]) * 57.295779513082323;
    LOG(INFO) << "the yaw between double ant yaw and vehicle: " << imu_ant_yaw_angle;
  }
  LOG(INFO) << "novatel heading is: " << std::setprecision(15) << measure.time << " "
  ...
  measure.is_have_variance = true;
  LOG(INFO) << "the novatel heading std: " << std::setprecision(15) << measure.time 
                                           << " " << heading_std;
  measure.variance[8][8] = heading_std * heading_std * 3.046174197867086e-04;
  measure.gnss_att.yaw = -gnss_yaw * 0.017453292519943;
  LOG(INFO) << "measure data heading is: " << std::setprecision(15) << measure.time
                                           << std::setprecision(6) << measure.gnss_att.yaw;
  return true;
}

bool MeasureRepublishProcess::LoadImuGnssAntennaExtrinsic(std::string file_path, 
  ...
  if (config["leverarm"]) {
    ...
    if (config["leverarm"]["secondary"]["offset"]) {
      ...
      if (config["leverarm"]["secondary"]["rotation"]) {
        double qx = config["leverarm"]["secondary"]["rotation"]["x"].as<double>();
        double qy = config["leverarm"]["secondary"]["rotation"]["y"].as<double>();
        double qz = config["leverarm"]["secondary"]["rotation"]["z"].as<double>();
        double qw = config["leverarm"]["secondary"]["rotation"]["w"].as<double>();
        ...
        double yaw = atan2(
          ...
          extrinsic.transform_1.translation()[1]);
        extrinsic.transform_1.linear() =
            Eigen::Quaterniond(cos(yaw / 2.0), 0.0, 0.0, sin(yaw / 2.0)).toRotationMatrix();
      }
      return true;
    } else {
      extrinsic.ant_num = 1;
