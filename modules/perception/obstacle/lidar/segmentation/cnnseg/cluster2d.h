...
class Cluster2D {
 public:
  ...
  void FilterNoise(std::vector<int>* grids, const float* count_pt_ptr,
    ...
    if (max_pt_num < enable_filter_thresh) return;
  ...
  void Filter(const caffe::Blob<float>& confidence_pt_blob,
              ...
              const float* count_pt_ptr, const float filter_thresh,
              const float enable_filter_thresh) {
    ...
    for (size_t obstacle_id = 0; obstacle_id < obstacles_.size();
      ...
      FilterNoise(&grids, count_pt_ptr, enable_filter_thresh, filter_thresh);
