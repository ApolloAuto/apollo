...
bool SequenceTypeFuser::init() {
    ...
    if (!load_single_matrix_file(transition_property_file_path, 
                &_transition_matrix)) {
        return false;
    } 
    _transition_matrix += Matrixd::Ones() * 1e-6;
    for (std::size_t i = 0; i < VALID_OBJECT_TYPE; ++i) {
        normalize_row(&_transition_matrix);
    }
    ...
    if (!load_multiple_matrices_file(classifiers_property_file_path, &_smooth_matrices)) {
        return false;
    }
    for (auto& pair : _smooth_matrices) {
        normalize_row(&pair.second);
...
bool SequenceTypeFuser::fuse_type(
    ...
    if (options.timestamp > 0.0) {
        ...
        // sequence fusion
        _sequence.add_tracked_frame_objects(*objects, options.timestamp);
        ...
        for (auto& object : *objects) {
            ...
            _sequence.get_track_in_temporal_window(
            ...
            if (!fuse_with_ccrf(&tracked_objects)) {
...
bool SequenceTypeFuser::fuse_with_ccrf(
    ...
    for (auto& pair : *tracked_objects) {
        ...
        if (!rectify_object_type(object, &_fused_oneshot_probs[i++])) {
    ...
    recover_from_log_prob(&_fused_sequence_probs.back(), 
    ...
    if (_ccrf_debug) {
        ...
        //     sequence_type_fuser::print_probability(object->lidar_supplement->raw_probs[i],
        //             object->lidar_supplement->raw_classification_methods[i]);
        // }
        std::vector<float> prob_t;
        sequence_type_fuser::to_exp(&_fused_oneshot_probs.back());
        sequence_type_fuser::normalize(&_fused_oneshot_probs.back());
        sequence_type_fuser::from_eigen_vector(_fused_oneshot_probs.back(), &prob_t);
        sequence_type_fuser::print_probability(prob_t, "Oneshot");
        sequence_type_fuser::print_probability(object->type_probs, "Sequence");
        std::cout << "@@@@@@@@@@@@@@@@@" << std::endl;
    }
...
bool SequenceTypeFuser::rectify_object_type(const ObjectPtr& object, 
        Vectord* log_prob) {
    using namespace sequence_type_fuser;
    ...
    Vectord single_prob;
    from_std_vector(object->type_probs, &single_prob);
...
bool SequenceTypeFuser::recover_from_log_prob(Vectord* prob, 
        std::vector<float>* dst, ObjectType* type) {
    using namespace sequence_type_fuser;

    to_exp(prob);
    normalize(prob);
    from_eigen_vector(*prob, dst);
