 ...
 * @file
 */
#include "modules/planning/planning_open_space/trajectory_smoother/distance_approach_ipopt_relax_end_slack_interface.h"
...
bool DistanceApproachIPOPTRelaxEndSlackInterface::eval_f(int n, const double* x, bool new_x, double& obj_value) {
    eval_obj(n, x, &obj_value);
    return true;
}

bool DistanceApproachIPOPTRelaxEndSlackInterface::eval_grad_f(int n, const double* x, bool new_x, double* grad_f) {
    gradient(tag_f, n, x, grad_f);
    return true;
}

bool DistanceApproachIPOPTRelaxEndSlackInterface::eval_g(int n, const double* x, bool new_x, int m, double* g) {
    ...
    for (int i = 0; i < horizon_ + 1; ++i) {
        ...
        for (int j = 0; j < obstacles_num_; ++j) {
            ...
            g[constraint_index + 3] = tmp3 + (x[state_index] + cos(x[state_index + 2]) * offset_) * tmp1
                    + (x[state_index + 1] + sin(x[state_index + 2]) * offset_) * tmp2 - tmp4 + x[s_index];
...
bool DistanceApproachIPOPTRelaxEndSlackInterface::check_g(int n, const double* x, int m, const double* g) {
    int kN = n;
    int kM = m;
    double x_u_tmp[kN];
    double x_l_tmp[kN];
...
void DistanceApproachIPOPTRelaxEndSlackInterface::generate_tapes(int n, int m, int* nnz_jac_g, int* nnz_h_lag) {
