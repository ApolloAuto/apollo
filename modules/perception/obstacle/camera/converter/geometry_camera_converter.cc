...
bool GeometryCameraConverter::Convert(std::vector<VisualObjectPtr> *objects) {
  ...
  for (auto &obj : *objects) {
    ...
    instance_transform(obj->height, obj->width, obj->length, deg_alpha,
    ...
    //                _camera_model.project(_corners[i] + obj->center);
...
bool GeometryCameraConverter::ConvertSingle(
  ...
  Eigen::Matrix<double, 3, 1> mass_center_v =
      _camera_model.unproject(mass_center_pixel);
  ...
  for (size_t i = 0; i < 2; ++i) {
    // Mass center search
    mass_center_search(target_box_center_pixel, distance_h, mass_center_v,
                       mass_center_pixel);
...
void GeometryCameraConverter::Rotate(
  ...
  Eigen::Matrix3d rotation = yaw.toRotationMatrix() * pitch.toRotationMatrix() *
...
void GeometryCameraConverter::SearchCenterDirection(
  ...
  while (depth < max_mass_center_search_depth) {
    ...
    for (size_t i = 0; i < _corners.size(); ++i) {
      Eigen::Vector2d point_2d =
          _camera_model.project(_corners[i] + new_center_v);
      min_pixel_x = std::min(min_pixel_x, point_2d.x());
      max_pixel_x = std::max(max_pixel_x, point_2d.x());
      min_pixel_y = std::min(min_pixel_y, point_2d.y());
      max_pixel_y = std::max(max_pixel_y, point_2d.y());
