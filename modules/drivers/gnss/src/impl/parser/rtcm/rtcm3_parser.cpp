...
void Rtcm3Parser::fill_keppler_orbit(eph_t &eph,
                       apollo::drivers::gnss::KepplerOrbit &keppler_orbit) {
...
void Rtcm3Parser::fill_glonass_orbit(geph_t &eph,
  ...
  for (int i = 0; i < _rtcm.obs.n; ++i) {
    ...
    for (j = 0; j < NFREQ + NEXOBS; ++j) {
      ...
      if (_rtcm.obs.data[i].code[i] == CODE_L1C) {
        ...
        _rtcm.obs.data[i].code[i], j, static_cast<int>(gnss_type));
      }
...
bool Rtcm3Parser::process_station_parameters() {
  ...
  ROS_INFO("Station pose (%f, %f, %f).", _rtcm.sta.pos[0],
                                         _rtcm.sta.pos[1],
                                         _rtcm.sta.pos[2]);
