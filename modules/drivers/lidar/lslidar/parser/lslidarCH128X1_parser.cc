...
//  const unsigned char* pkt_ptr = (const unsigned char*)pkt.data().c_str();
//  uint64_t time_stamp = pkt_ptr[1203] + pkt_ptr[1202] * pow(2, 8) + pkt_ptr[1201] * pow(2, 16) + pkt_ptr[1200] * pow(2, 24);
 
//  if (!config_.time_synchronization())
//		packet_end_time = gps_base_usec_ * 1000 - (packets_size - num - 1)*868*POINTS_PER_PACKET;
//  else
//	    packet_end_time = (gps_base_usec_ - (gps_base_usec_ %1000000) + time_stamp % 1000000)*1000;
//
...
//		  packet_end_time = packet_end_time + 1000000000;
//	  else if(packet_end_time - last_packet_time > 1000000000)
//		  packet_end_time = packet_end_time - 1000000000;
//  }
//
//  last_packet_time = packet_end_time;
  ...
  for (size_t point_idx = 0; point_idx < POINTS_PER_PACKET; point_idx++) 
  {
	...
	for (size_t point_idx = 0; point_idx < POINTS_PER_PACKET; point_idx++) 
	{
		...
		double _R_ = cosTheta_2[firings[point_idx].vertical_line] * cosTheta_1[firings[point_idx].vertical_line] *
        ...
        if((y >= config_.bottom_left_x() && y <= config_.top_right_x())
           && (-x >= config_.bottom_left_y() && -x <= config_.top_right_y()))  return;

		// Compute the time of the point
//		point_time = packet_end_time - 868 * (POINTS_PER_PACKET - point_idx - 1);
          // modify
          if(previous_packet_time >1e-6) {
            point_time = current_packet_time - (POINTS_PER_PACKET - point_idx - 1) * (current_packet_time - previous_packet_time)/(POINTS_PER_PACKET);
          } else{              //fist packet
		...
		if (config_.calibration()) {
			...
			ComputeCoords2(firings[point_idx].vertical_line, CH128X1, firings[point_idx].distance, corrections,
