...
MapNodeIndex get_map_index_from_map_folder(const std::string& map_folder) {
    MapNodeIndex index;
    char buf[100];
    sscanf(map_folder.c_str(), "/%03u/%05s/%02d/%08u/%08u",
            &index._resolution_id, buf, &index._zone_id, &index._m, &index._n);
    std::string zone = buf;
...
bool get_all_map_index(const std::string &src_map_folder, 
        const std::string &dst_map_folder, std::list<MapNodeIndex>& buf) {
...
int main(int argc, char **argv) {
    ...
    boost_desc.add_options()
            ...
            ("dstdir", boost::program_options::value<std::string>(), 
    ...
    for (; itr != buf.end(); ++itr, ++index) {
        ...
        LossyMapNode* lossy_node = static_cast<LossyMapNode*>(lossy_map.get_map_node_safe(*itr));
        LossyMapMatrix& lossy_matrix = static_cast<LossyMapMatrix&>(lossy_node->get_map_cell_matrix());
