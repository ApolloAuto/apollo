...
void Fbs3237::Parse(const std::uint8_t* bytes, int32_t length,
  ...
  chassis->mutable_wey()->mutable_fbs3_237()->set_engspd(
                                          engspd(bytes, length));
...
Fbs3_237::CurrentgearType Fbs3237::currentgear(
                         const std::uint8_t* bytes, int32_t length) const {
  Byte t0(bytes + 4);
  int32_t x = t0.get_byte(6, 2);
...
double Fbs3237::epsdrvinputtrqvalue(
                          const std::uint8_t* bytes, int32_t length) const {
  Byte t0(bytes + 5);
  int32_t x = t0.get_byte(0, 8);
...
double Fbs3237::epsconsumedcurrvalue(
                           const std::uint8_t* bytes, int32_t length) const {
  Byte t0(bytes + 6);
  int32_t x = t0.get_byte(0, 8);
...
// 'type': 'enum', 'order': 'motorola', 'physical_unit': ''}
Fbs3_237::EpscurrmodType Fbs3237::epscurrmod(
                            const std::uint8_t* bytes, int32_t length) const {
  Byte t0(bytes + 7);
