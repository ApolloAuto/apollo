 ...
 *****************************************************************************/
#include "modules/perception/common/camera/common/camera_ground_plane.h"

#include <utility>

#include "modules/perception/common/algorithm/i_lib/da/i_ransac.h"
...
GroundPlaneTracker::GroundPlaneTracker(int track_length) {
  ...
  for (int i = track_length - 1; i >= 0; --i) {
    const_weight_temporal_.at(i) =
        algorithm::IPow(algorithm::ISqrt(2.0f), track_length - 1 - i);
  }

  // normalize
  float accm_sum = algorithm::ISum(const_weight_temporal_.data(), track_length);
  algorithm::IScale(const_weight_temporal_.data(), track_length,
                 algorithm::IRec(accm_sum));
...
void GroundPlaneTracker::GetGround(float *pitch, float *cam_height) {
  ...
  // normalize factor
  float accm_wei = algorithm::ISum(weight_.data() + head_, (length - head_));
  ...
  ph[0] = algorithm::IDiv(ph[0], accm_wei);
  ph[1] = algorithm::IDiv(ph[1], accm_wei);
  *pitch = ph[0];
  *cam_height = ph[1];
}
...
void CameraGroundPlaneParams::SetDefault() {
  min_nr_samples = 6;   // 40
  nr_frames_track = 3;  // 2
  max_tilt_angle = algorithm::IDegreeToRadians(10.0f);
...
bool CameraGroundPlaneDetector::DetectGroundFromSamples(float *vd, int count_vd,
  ...
  *inlier_ratio = 0.0f;
  ...
  if (!algorithm::RobustBinaryFitRansac<float, 1, 1, 2, 2,
    ...
    *inlier_ratio = static_cast<float>(nr_inliers) *
                    algorithm::IRec(static_cast<float>(count_vd));
  }
  ...
  float l_best[3] = {0};
  algorithm::ILineFit2dTotalLeastSquare(vd, l_best, count);
