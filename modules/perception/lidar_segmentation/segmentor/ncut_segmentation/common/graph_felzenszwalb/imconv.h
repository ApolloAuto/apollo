...
#include "modules/perception/lidar_segmentation/segmentor/ncut_segmentation/common/graph_felzenszwalb/imutil.h"
...
Image<uchar> *image_rgb2gray(Image<rgb> *input) {
    ...
    for (int y = 0; y < height; y++) {
        for (int x = 0; x < width; x++) {
            imRef(output, x, y) = (uchar)(imRef(input, x, y).r * RED_WEIGHT + imRef(input, x, y).g * GREEN_WEIGHT
...
/**
 * @brief Transform uchar to float
 *
 * @param input
 * @return Image<float>*
 */
...
/**
 * @brief Transform float to uchar
 *
 * @param input
 * @param min
 * @param max
...
/**
 * @brief Transform unchar to long
 *
 * @param input
...
/**
 * @brief Transform short to uchar
 *
 * @param input
 * @param min
 * @param max
