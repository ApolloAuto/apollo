...
bool MapNodeIndex::operator == (const MapNodeIndex& index) const { 
    return _resolution_id == index._resolution_id
           && _zone_id == index._zone_id
           && _m == index._m
           && _n == index._n;
}
...
std::string MapNodeIndex::to_string() const {
    ...
    ss << "Map node (Resolution ID: " << _resolution_id << " Zone ID: " << _zone_id
       << " Easting: " << _n << " Northing: " << _m << ")";
    return ss.str();
}
...
//     if (n >= 0 && m >= 0 && n < get_map_index_range_east(option, resolution_id) &&
