...
namespace {
apollo::hdmap::Id CreateHDMapId(const std::string& string_id) {
  apollo::hdmap::Id id;
  id.set_id(string_id);
  return id;
}
...
LaneInfoConstPtr HDMapImpl::GetLaneById(const apollo::hdmap::Id& id) const {
  LaneTable::const_iterator it = _lane_table.find(id.id());
  return it != _lane_table.end() ? it->second : nullptr;
}

JunctionInfoConstPtr HDMapImpl::GetJunctionById(
...
int HDMapImpl::GetRoads(const apollo::common::PointENU& point,
                double distance,
                std::vector<RoadInfoConstPtr>* roads) const {
  return GetRoads({point.x(), point.y()}, distance, roads);
}

int HDMapImpl::GetRoads(const apollo::common::math::Vec2d& point,
                double distance,
                std::vector<RoadInfoConstPtr>* roads) const {
...
int HDMapImpl::GetJunctions(const apollo::common::PointENU& point,
                         double distance,
                         std::vector<JunctionInfoConstPtr>* junctions) const {
  return GetJunctions({point.x(), point.y()}, distance, junctions);
}

int HDMapImpl::GetJunctions(const apollo::common::math::Vec2d& point,
                          double distance,
                          std::vector<JunctionInfoConstPtr>* junctions) const {
...
int HDMapImpl::GetSignals(const apollo::common::PointENU& point,
                       double distance,
                       std::vector<SignalInfoConstPtr>* signals) const {
  return GetSignals({point.x(), point.y()}, distance, signals);
}

int HDMapImpl::GetSignals(const apollo::common::math::Vec2d& point,
                        double distance,
                        std::vector<SignalInfoConstPtr>* signals) const {
  ...
  signals->clear();
  std::vector<std::string> ids;
  const int status = SearchObjects(point, distance, *_signal_segment_kdtree,
                                  &ids);
  if (status < 0) {
      return status;
  }
...
int HDMapImpl::GetCrosswalks(const apollo::common::PointENU& point,
                        double distance,
                        std::vector<CrosswalkInfoConstPtr>* crosswalks) const {
  return GetCrosswalks({point.x(), point.y()}, distance, crosswalks);
}

int HDMapImpl::GetCrosswalks(const apollo::common::math::Vec2d& point,
                        double distance,
                        std::vector<CrosswalkInfoConstPtr>* crosswalks) const {
...
int HDMapImpl::GetStopSigns(const apollo::common::PointENU& point,
                          double distance,
                          std::vector<StopSignInfoConstPtr>* stop_signs) const {
  return GetStopSigns({point.x(), point.y()}, distance, stop_signs);
}

int HDMapImpl::GetStopSigns(const apollo::common::math::Vec2d& point,
                        double distance,
                        std::vector<StopSignInfoConstPtr>* stop_signs) const {
...
int HDMapImpl::GetYieldSigns(const apollo::common::PointENU& point,
                        double distance,
                        std::vector<YieldSignInfoConstPtr>* yield_signs) const {
  return GetYieldSigns({point.x(), point.y()}, distance, yield_signs);
}

int HDMapImpl::GetYieldSigns(const apollo::common::math::Vec2d& point,
                        double distance,
                        std::vector<YieldSignInfoConstPtr>* yield_signs) const {
...
int HDMapImpl::GetNearestLane(const apollo::common::PointENU& point,
                LaneInfoConstPtr* nearest_lane,
                double* nearest_s,
                double* nearest_l) const {
  return GetNearestLane(apollo::common::math::Vec2d(point.x(), point.y()),
            nearest_lane, nearest_s, nearest_l);
}

int HDMapImpl::GetNearestLane(const apollo::common::math::Vec2d &point,
                            LaneInfoConstPtr* nearest_lane,
...
int HDMapImpl::GetNearestLaneWithHeading(const apollo::common::PointENU& point,
                                  ...
                                  double* nearest_s,
                                  double* nearest_l) const {
  return GetNearestLaneWithHeading(apollo::common::math::Vec2d(point.x(),
...
int HDMapImpl::GetLanesWithHeading(const apollo::common::PointENU& point,
                            const double distance,
                            const double central_heading,
                            const double max_heading_difference,
                            std::vector<LaneInfoConstPtr>* lanes) const {
  return GetLanesWithHeading(apollo::common::math::Vec2d(point.x(),
        point.y()), distance, central_heading, max_heading_difference, lanes);
}

int HDMapImpl::GetLanesWithHeading(const apollo::common::math::Vec2d &point,
                                const double distance,
                                const double central_heading,
                                const double max_heading_difference,
                                std::vector<LaneInfoConstPtr> *lanes) const {
...
int HDMapImpl::GetRoadBoundaries(const apollo::common::PointENU& point,
                        double radius,
...
void HDMapImpl::BuildLaneSegmentKDTree() {
  ...
  BuildSegmentKDTree(_lane_table, params, &_lane_segment_boxes,
...
void HDMapImpl::BuildJunctionPolygonKDTree() {
  ...
  BuildPolygonKDTree(_junction_table, params, &_junction_polygon_boxes,
...
void HDMapImpl::BuildCrosswalkPolygonKDTree() {
  ...
  params.max_leaf_dimension = 5.0;  // meters.
  params.max_leaf_size = 1;
  BuildPolygonKDTree(_crosswalk_table, params, &_crosswalk_polygon_boxes,
...
void HDMapImpl::BuildSignalSegmentKDTree() {
  ...
  BuildSegmentKDTree(_signal_table, params, &_signal_segment_boxes,
...
int HDMapImpl::SearchObjects(const apollo::common::math::Vec2d& center,
                            const double radius,
                            const KDTree& kdtree,
