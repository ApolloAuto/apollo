 ...
 *****************************************************************************/
#include "modules/perception/camera_location_estimation/transformer/multicue/multicue_transformer.h"
...
bool MultiCueTransformer::Init(const TransformerInitOptions &options) {
  std::string config_file =
      GetConfigFile(options.config_path, options.config_file);

  if (!cyber::common::GetProtoFromFile(config_file, &multicue_param_)) {
    AERROR << "Read config failed: " << config_file;
    return false;
  }
  AINFO << multicue_param_.DebugString();

  // Init object template
  object_template_manager_ = ObjectTemplateManager::Instance();
  ACHECK(object_template_manager_->Init());
  mapper_.reset(new ObjMapper);

  return true;
}

void MultiCueTransformer::SetObjMapperOptions(
...
int MultiCueTransformer::MatchTemplates(base::ObjectSubType sub_type,
                                        float *dimension_hwl) {
  ...
  switch (sub_type) {
    ...
    case base::ObjectSubType::MOTORCYCLIST: {
      ...
      std::sort(diff_hs.begin(), diff_hs.end(),
    ...
    default:
      if (min_dimension_val < multicue_param_.min_dimension_val()) {
        algorithm::IScale3(dimension_hwl,
                           multicue_param_.min_dimension_val() *
                               algorithm::IRec(min_dimension_val));
      }
      break;
  }
...
void MultiCueTransformer::FillResults(float object_center[3],
                                      float dimension_hwl[3], float rotation_y,
...
bool MultiCueTransformer::Transform(onboard::CameraFrame *frame) {
